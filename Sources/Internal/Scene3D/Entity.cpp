...
DAVA_VIRTUAL_REFLECTION_IMPL(Entity)
{
    ...
    .Field("Children", &Entity::children)[M::HiddenField()]
...
Component* Entity::GetComponentByIndex(uint32 index) const
{
    DVASSERT(index < components.size());
...
void Entity::AddNode(Entity* node)
{
    if (node)
    {
        ...
        if (render != nullptr && render->GetRenderObject()->GetType() == RenderObject::TYPE_MESH)
        {
            ...
            if (node->GetComponent(Component::STATIC_BODY_COMPONENT) == nullptr &&
                node->GetComponent(Component::DYNAMIC_BODY_COMPONENT) == nullptr &&
                node->GetComponent(Component::BOX_SHAPE_COMPONENT) == nullptr &&
                node->GetComponent(Component::CAPSULE_SHAPE_COMPONENT) == nullptr &&
                node->GetComponent(Component::SPHERE_SHAPE_COMPONENT) == nullptr &&
                node->GetComponent(Component::PLANE_SHAPE_COMPONENT) == nullptr &&
                node->GetComponent(Component::MESH_SHAPE_COMPONENT) == nullptr &&
                node->GetComponent(Component::CONVEX_HULL_SHAPE_COMPONENT) == nullptr &&
                node->GetComponent(Component::HEIGHT_FIELD_SHAPE_COMPONENT) == nullptr &&
                node->GetComponent(Component::VEHICLE_CAR_COMPONENT) == nullptr &&
                node->GetComponent(Component::VEHICLE_TANK_COMPONENT) == nullptr &&
                node->GetComponent(Component::VEHICLE_CHASSIS_COMPONENT) == nullptr &&
                node->GetComponent(Component::VEHICLE_WHEEL_COMPONENT) == nullptr)
            {
                StaticBodyComponent* sb = new StaticBodyComponent();
                MeshShapeComponent* ch = new MeshShapeComponent();

                node->AddComponent(sb);
                node->AddComponent(ch);
            }
        }*/
...
void Entity::Load(KeyedArchive* archive, SerializationContext* serializationContext)
{
    ...
    if (nullptr != serializationContext)
    {
