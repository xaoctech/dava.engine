...
void Entity::AddEntity(Entity* entity)
{
    ...
    {
#if defined(__DAVAENGINE_PHYSICS_ENABLED__)
        DAVA::MeshShapeComponent* shape = entity->GetComponent<MeshShapeComponent>();
        if (shape != nullptr)
            shape->SetTypeMask(1);
#endif
...
void Entity::BakeTransforms()
{
    ...
    bool needPropagate = GetComponent<RenderComponent>() != nullptr && GetComponent<ParticleEffectComponent>() != nullptr && GetComponent<AnimationComponent>() != nullptr && GetComponent<ReflectionComponent>() != nullptr;
...
AABBox3 Entity::GetWTMaximumBoundingBoxSlow()
{
    ...
    {
        AABBox3 wtBox;
        for (RenderObject* ro : GetRenderObjects(this))
        {
            ro->GetBoundingBox().GetTransformedBox(transformComponent->GetWorldTransform(), wtBox);
