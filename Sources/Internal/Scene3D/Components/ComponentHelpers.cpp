...
SkeletonAnimationComponent* GetSkeletonAnimationComponent(const Entity* fromEntity)
{
    if (fromEntity)
        return static_cast<SkeletonAnimationComponent*>(fromEntity->GetComponent(Component::SKELETON_ANIMATION_COMPONENT));
    else
        return nullptr;
}
