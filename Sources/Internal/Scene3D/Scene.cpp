...
#define __TEMPORARY_VISUAL_SCRIPT__
#if defined(__TEMPORARY_VISUAL_SCRIPT__)
...
void Scene::CreateSystems()
{
    ...
    if (SCENE_SYSTEM_PHYSICS_FLAG & systemsMask)
    {
        ...
        AddSingletonComponent(collisionSingleComponent);
...
// TODO: Replace with generic addition of every system.
// Remove include of visual script component.
#if defined(__TEMPORARY_VISUAL_SCRIPT__)
    const EngineContext* engineContext = GetEngineContext();
    auto systemsWithPriority = engineContext->systemManager->GetRegisteredSceneSystems();

    for (auto it : systemsWithPriority)
    {
        const Type* type = it.first;
        int32 priority = it.second;
        const ReflectedType* rType = ReflectedTypeDB::GetByType(type);

        Any systemAny = rType->CreateObject(ReflectedType::CreatePolicy::ByPointer, this);
        SceneSystem* system = systemAny.Cast<SceneSystem*>();
        if (priority != 0)
        {
            AddSystem(system, ComponentUtils::MakeMask<VisualScriptComponent>(), SCENE_SYSTEM_REQUIRE_PROCESS);
        }
    }
#endif
}
...
#if defined(__DAVAENGINE_PHYSICS_ENABLED__)
    collisionSingleComponent = nullptr;
#endif
