...
GeoDecalSystem::GeoDecalSystem(Scene* scene)
    : SceneSystem(scene, ComponentUtils::MakeMask<GeoDecalComponent>())
{
}
...
void GeoDecalSystem::BuildDecal(Entity* entityWithDecal, GeoDecalComponent* component)
{
    ...
    {
        SkeletonComponent* skeletonComponent = e.entity->GetComponent<SkeletonComponent>();
        if (skeletonComponent != nullptr && (e.renderObject->GetType() == RenderObject::TYPE_MESH))
        {
            // update mesh before creating decal in order to have valid skinning
            GetScene()->skeletonSystem->UpdateMeshJointTransforms(skeletonComponent, static_cast<Mesh*>(e.renderObject));
        }
