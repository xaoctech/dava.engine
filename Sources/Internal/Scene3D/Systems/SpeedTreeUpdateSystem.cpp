...
SpeedTreeUpdateSystem::SpeedTreeUpdateSystem(Scene* scene)
    : SceneSystem(scene, ComponentUtils::MakeMask<SpeedTreeComponent>())
{
...
void SpeedTreeUpdateSystem::Process(float32 timeElapsed)
{
    ...
    for (auto& pair : tsc->worldTransformChanged.map)
    {
        if (pair.first->GetComponentsCount(Type::Instance<SpeedTreeComponent>()) > 0)
        {
            for (Entity* entity : pair.second)
            {
                ...
                component->GetSpeedTreeObject()->SetInvWorldTransformPtr(&component->worldTransfromInv);
            }
        }
    }
    ...
    for (Entity* tree : allTrees)
    {
        ...
        SpeedTreeComponent* treeComponent = tree->GetComponent<SpeedTreeComponent>();
        ...
        treeComponent->prevWind = treeComponent->wind;
        treeComponent->wind = Vector4(wind, windForce);
        float32 prevFlexibility = treeComponent->leafsFlexibility.x;
        treeComponent->leafsFlexibility = Vector4(treeComponent->GetLeavesFlexibility() / (0.5f * PI), prevFlexibility, 0.0f, 0.0f);
