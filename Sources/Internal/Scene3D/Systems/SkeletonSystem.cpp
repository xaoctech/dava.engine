...
SkeletonSystem::SkeletonSystem(Scene* scene)
    : SceneSystem(scene, ComponentUtils::MakeMask<SkeletonComponent>())
{
...
void SkeletonSystem::Process(float32 timeElapsed)
{
    ...
    {
        SkeletonComponent* component = entity->GetComponent<SkeletonComponent>();
        if (component != nullptr)
        {
            ...
            if (component->startJoint != SkeletonComponent::INVALID_JOINT_INDEX)
            {
                ...
                uint32 meshCount = entity->GetComponentCount<MeshComponent>();
                for (uint32 meshIndex = 0; meshIndex < meshCount; ++meshIndex)
                {
                    Mesh* mesh = entity->GetComponent<MeshComponent>(meshIndex)->GetMesh();
                    UpdateMeshJointTransforms(component, mesh);
                    UpdateMeshBoundingBox(component, mesh);
                }

                uint32 speedTreeCount = entity->GetComponentCount<SpeedTreeComponent>();
                for (uint32 treeIndex = 0; treeIndex < speedTreeCount; ++treeIndex)
                {
                    SpeedTreeObject* speedTreeObject = entity->GetComponent<SpeedTreeComponent>(treeIndex)->GetSpeedTreeObject();
                    UpdateMeshJointTransforms(component, speedTreeObject);
                }
            }
        }
    }
...
void SkeletonSystem::UpdateMeshJointTransforms(SkeletonComponent* skeleton, Mesh* mesh)
{
    DVASSERT(!skeleton->configUpdated);
    mesh->UpdateJointTransforms(skeleton->finalTransforms);
}

void SkeletonSystem::UpdateMeshBoundingBox(SkeletonComponent* skeleton, Mesh* mesh)
{
    DVASSERT(!skeleton->configUpdated);
    ...
    if (count > 0)
    {
        ...
        mesh->SetBoundingBox(resBox); // TODO: *Skinning* decide on bbox calculation
    }

    GetScene()->GetRenderSystem()->MarkForUpdate(mesh);
}
