...
struct Matrix4
{
    ...
    void Dump();
    float64 GetHash() const;

    void Identity();
    void Zero();

    void BuildOrtho(float32 left, float32 right, float32 bottom, float32 top, float32 near, float32 far, bool zeroBaseClipRange);
    void BuildPerspective(float32 left, float32 right, float32 bottom, float32 top, float32 near, float32 far, bool zeroBaseClipRange);
    ...
    bool Decomposition(Vector3& position, Vector3& scale, Vector3& orientation) const;
    void Decomposition(Vector3& position, Vector3& scale, Quaternion& orientation) const;

    Quaternion GetRotation() const;

    //!
    static Matrix4 MakeTranslation(const Vector3& translationVector);
    static Matrix4 MakeRotation(const Vector3& axis, float32 angleInRadians);
    static Matrix4 MakeScale(const Vector3& scaleVector);

    //! build translation matrix
    void BuildTranslation(const Vector3& vector);

    //! build rotation matrix
    void BuildRotation(const Vector3& axis, float32 angleInRadians);

    //! build scale matrix
    void BuildScale(const Vector3& vector);

    Vector3 GetTranslationVector() const;
    void SetTranslationVector(const Vector3& vector);
...
inline float64 Matrix4::GetHash() const
{
    float64 result = 0;
