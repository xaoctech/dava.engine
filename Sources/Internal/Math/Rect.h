...
struct Rect
{
    ...
    Rect();
    Rect(float32 _x, float32 _y, float32 _dx, float32 _dy);
    Rect(const Vector2& point, const Vector2& size);
    Rect(const Size2f& size);
    Rect(const Rect& rect) = default;

    bool PointInside(const Vector2& point) const;
    Rect Intersection(const Rect& rect) const;
    bool RectIntersects(const Rect& rect) const;
    bool RectContains(const Rect& rect) const;
    void ClampToRect(Vector2& point) const;
    void ClampToRect(Rect& rect) const;
    ...
    bool operator==(const Rect& _s) const;
    bool operator!=(const Rect& _s) const;

    Rect& operator+=(const Vector2& point);
    Rect& operator-=(const Vector2& point);

    Rect operator+(const Vector2& Point) const;
    Rect operator-(const Vector2& Point) const;
};
