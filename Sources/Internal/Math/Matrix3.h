...
struct Matrix3
{
    ...
    Matrix3();
    Matrix3(float32 __00, float32 __01, float32 __02,
    ...
    // Helpers
    void Identity();
    void CreateRotation(const Vector3& r, float32 angleInRadians);
    void BuildRotation(float32 cosA, float32 sinA);
    void BuildRotation(float32 angle);
    void BuildTranslation(float32 x, float32 y);
    void BuildTranslation(const Vector2& vec);
    void BuildScale(const Vector2& vec);
    bool GetInverse(Matrix3& out, float32 fTolerance = 1e-06) const;
    void Transpose();
