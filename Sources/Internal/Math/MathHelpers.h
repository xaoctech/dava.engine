...
//! Test whether two floats are equal with given precision
inline bool TestFloatsAreEqual(float32 v1, float32 v2, float32 epsilon = EPSILON)
{
...
/**
 Get number of bits required to represent given value, e.g. 0b0100'0110 requires 7 bits.
 Return 0 if value is zero.
*/
template <typename T>
constexpr uint32 GetBitsRequired(T value)
{
    static_assert(std::is_integral<T>::value && !std::is_same<T, bool>::value, "GetBitsRequired is applied only for integral types except bool");
