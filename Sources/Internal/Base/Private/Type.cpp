...
Type::~Type()
{
    for (size_t i = 0; i < typeUserDataStorageIndex; ++i)
    {
        if (userDataStorage[i].deleter != nullptr)
        {
            (*userDataStorage[i].deleter)(userDataStorage[i].data);
            userDataStorage[i].data = nullptr;
        }
    }
}
...
void Type::SetUserData(uint32_t index, void* data, void (*deleter)(void*)) const
{
    size_t i = static_cast<size_t>(index);

    DVASSERT(i < typeUserDataStorageIndex);
    userDataStorage[i].data = data;
    userDataStorage[i].deleter = deleter;
}
