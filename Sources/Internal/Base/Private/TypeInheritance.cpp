...
Vector<TypeInheritance::Info> TypeInheritance::CollectRecursiveDerivedTypes() const
{
    Vector<TypeInheritance::Info> allTypes;
    const Vector<Info>& derivedTypes = GetDerivedTypes();
    for (const TypeInheritance::Info& info : derivedTypes)
    {
        CollectDerivedTypes(info, allTypes);
    }
    return allTypes;
}

void TypeInheritance::CollectDerivedTypes(const TypeInheritance::Info& info, Vector<TypeInheritance::Info>& resultVector) const
{
    resultVector.push_back(info);
    const TypeInheritance* inheritance = info.type->GetInheritance();

    if (inheritance != nullptr)
    {
        for (const TypeInheritance::Info& info : inheritance->GetDerivedTypes())
        {
            CollectDerivedTypes(info, resultVector);
        }
    }
}
