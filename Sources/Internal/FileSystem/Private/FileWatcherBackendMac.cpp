                      ...
                      const FSEventStreamEventFlags eventFlags[], const FSEventStreamEventId eventIds[])
{
    ...
    for (size_t i = 0; i < numEvents; ++i)
    {
        ...
        if (path == info->node->checkDirectory)
        {
            continue;
        }
        FSEventStreamEventFlags flag = eventFlags[i];
        FSEventStreamEventId id = eventIds[i];
        bool renamed = (flag & kFSEventStreamEventFlagItemRenamed) == kFSEventStreamEventFlagItemRenamed;
        bool created = (flag & kFSEventStreamEventFlagItemCreated) == kFSEventStreamEventFlagItemCreated;
        bool removed = (flag & kFSEventStreamEventFlagItemRemoved) == kFSEventStreamEventFlagItemRemoved;
        bool modif = (flag & kFSEventStreamEventFlagItemModified) == kFSEventStreamEventFlagItemModified;

        if (renamed == true && (created == modif))
        {
            int ret = access(rawPath, R_OK);
            if (ret < 0)
            {
...
void FileWatcherBackend::Add(const String& directory, bool recursive)
{
    ...
    node->checkDirectory = directory;
    node->checkDirectory.pop_back();
...
void FileWatcherBackend::CreateFSEvents(WatchNode* node)
{
    ...
    node->stream = FSEventStreamCreate(nullptr, &FSEventsCallback, &context, pathsToWatch,
                                       FSEventsGetCurrentEventId(), 0.0f, kFSEventStreamCreateFlagFileEvents | kFSEventStreamCreateFlagNoDefer);
