...
VarTable YamlNode::AsVarTable() const
{
    VarTable result;
    const UnorderedMap<String, RefPtr<YamlNode>>& mapFromNode = AsMap();
...
Any YamlNode::AsAny(const Type* type, const M::Enum* emeta, const M::Flags* fmeta) const
{
    if (nullptr != emeta)
    {
        int32 val = 0;
...
Any YamlNode::AsAny(const Type* type) const
{
    ...
    else if (type == Type::Instance<VarTable>())
        return Any(AsVarTable());

    DVASSERT(false, "Unexpected type");
    return Any();
}
...
bool YamlNode::InitStringFromAny(const Any& any)
{
    DVASSERT(GetType() == TYPE_STRING);
...
bool YamlNode::InitArrayFromAny(const Any& any)
{
    DVASSERT(GetType() == TYPE_ARRAY);
    bool result = true;
    const Type* type = any.GetType();

    if (type == Type::Instance<Vector2>())
    {
        const Vector2& vector = any.Get<Vector2>();
        InternalSetVector(vector.data, Vector2::AXIS_COUNT);
    }
    else if (type == Type::Instance<Vector3>())
    {
        const Vector3& vector = any.Get<Vector3>();
        InternalSetVector(vector.data, Vector3::AXIS_COUNT);
    }
    else if (type == Type::Instance<Vector4>())
    {
        const Vector4& vector = any.Get<Vector4>();
        InternalSetVector(vector.data, Vector4::AXIS_COUNT);
    }
    else if (type == Type::Instance<Matrix2>())
    {
        uint32 dimension = 2;
        const Matrix2& matrix = any.Get<Matrix2>();
        const float32* array = &matrix._data[0][0];
        InternalSetMatrix(array, dimension);
    }
    else if (type == Type::Instance<Matrix3>())
    {
        uint32 dimension = 3;
        const Matrix3& matrix = any.Get<Matrix3>();
        const float32* array = &matrix._data[0][0];
        InternalSetMatrix(array, dimension);
    }
    else if (type == Type::Instance<Matrix4>())
    {
        uint32 dimension = 4;
        const Matrix4& matrix = any.Get<Matrix4>();
        const float32* array = &matrix._data[0][0];
        InternalSetMatrix(array, dimension);
    }
    else if (type == Type::Instance<Color>())
    {
        const Color& color = any.Get<Color>();
        InternalSetVector(color.color, Color::CHANNEL_COUNT);
    }
    else
    {
...
bool YamlNode::InitMapFromAny(const Any& any)
{
    DVASSERT(GetType() == TYPE_MAP);
    DVASSERT(false);
    return false;
...
RefPtr<YamlNode> YamlNode::CreateNodeFromAny(const Any& any)
{
    eType nodeType = TypeToYamlNodeType(any.GetType());
    RefPtr<YamlNode> node = nullptr;
    switch (nodeType)
    {
    case TYPE_STRING:
    {
        node = CreateStringNode();
        const bool initResult = node->InitStringFromAny(any);
        DVASSERT(initResult);
    }
    break;
    case TYPE_ARRAY:
    {
        node = CreateArrayNode();
        const bool initResult = node->InitArrayFromAny(any);
        DVASSERT(initResult);
    }
    break;
    case TYPE_MAP:
    {
        node = CreateMapNode();
        const bool initResult = node->InitMapFromAny(any);
        DVASSERT(initResult);
    }
    break;
    }
...
DAVA::YamlNode::eType YamlNode::TypeToYamlNodeType(const Type* type)
{
    if (type == Type::Instance<bool>()
        || type == Type::Instance<int8>()
        || type == Type::Instance<uint8>()
        || type == Type::Instance<int16>()
        || type == Type::Instance<uint16>()
        || type == Type::Instance<int32>()
        || type == Type::Instance<uint32>()
        || type == Type::Instance<int64>()
        || type == Type::Instance<uint64>()
        || type == Type::Instance<float32>()
        || type == Type::Instance<float64>()
        || type == Type::Instance<FilePath>()
        || type == Type::Instance<FastName>()
        || type == Type::Instance<String>()
        || type == Type::Instance<WideString>())
        return TYPE_STRING;

    if (type == Type::Instance<Vector2>()
        || type == Type::Instance<Vector3>()
        || type == Type::Instance<Vector4>()
        || type == Type::Instance<Matrix2>()
        || type == Type::Instance<Matrix3>()
        || type == Type::Instance<Matrix4>()
        || type == Type::Instance<Color>())
        return TYPE_ARRAY;
