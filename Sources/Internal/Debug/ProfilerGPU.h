...
#define DAVA_PROFILER_GPU_RENDER_PASS(passDesc, marker_name) {passDesc.name = marker_name;DAVA::ProfilerGPU::globalProfiler->AddMarker(reinterpret_cast<rhi::HPerfQuery*>(&passDesc.perfQueryStart), reinterpret_cast<rhi::HPerfQuery*>(&passDesc.perfQueryEnd), marker_name);}

#else

#define DAVA_PROFILER_GPU_PACKET(packet, marker_name) 
#define DAVA_PROFILER_GPU_RENDER_PASS(pass, marker_name) pass.name = marker_name;
