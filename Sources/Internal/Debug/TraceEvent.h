...
struct TraceEvent
{
    ...
    */
    template <class Container>
    static void DumpJSON(const Container& trace, std::ostream& stream, bool writeHeader = true);

    /**
        Dump `trace` from any type container with value type `TraceEvent` to `stream` in JSON-format,
        appending it to previosly existed file.
     */
    template <class Container>
    static void AppendJSON(const Container& trace, const FilePath& filePath);
};
...
template <class Container>
void TraceEvent::AppendJSON(const Container& trace, const FilePath& filePath)
{
    FileSystem* fs = FileSystem::Instance();
    fs->CreateDirectory(filePath.GetDirectory(), true);
    File* file = nullptr;
    if (!fs->Exists(filePath))
    {
        ...
        if (file)
        {
            std::stringstream stream;
            DumpJSON<Container>(trace, stream);
            file->WriteNonTerminatedString(stream.str());
        }
    }
    else
    {
        file = File::Create(filePath, File::OPEN | File::READ | File::WRITE);
        if (file)
        {
            uint64 pos = file->GetPos();
            DVASSERT(file->Seek(-5, File::SEEK_FROM_END) == true);
            uint64 npos = file->GetPos();

            std::stringstream stream;
            DumpJSON<Container>(trace, stream, false);
            file->WriteNonTerminatedString(stream.str());
        }
    }
    SafeRelease(file);
...
template <class Container>
void TraceEvent::DumpJSON(const Container& trace, std::ostream& stream, bool writeHeader)
{
