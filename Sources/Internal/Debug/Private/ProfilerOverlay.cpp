...
void ProfilerOverlay::AddCustomMetric(const FastName& metric, float64 value)
{
...
void ProfilerOverlay::Draw()
{
    ...
    rect.y += rect.dy;
    for (const auto& customMetric : customMetrics)
    {
        DbgDraw::Text2D(rect.x + ProfilerOverlayDetails::OVERLAY_RECT_MARGIN,
                        rect.y + ProfilerOverlayDetails::OVERLAY_RECT_MARGIN,
                        rhi::NativeColorRGBA(1.f, 1.f, 1.f, 1.f),
                        "%s: %.2f", customMetric.first.c_str(), customMetric.second);
        rect.dy = DbgDraw::NormalCharH;
        rect.y += rect.dy;
    }
    ...
    passConfig.priority = PRIORITY_MAIN_2D - 200;
