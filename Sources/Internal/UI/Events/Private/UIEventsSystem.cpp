...
bool UIEventsSystem::ProcessEventOnContol(UIControl* sceneControl, const FastName& event, bool checkFlow)
{
    ...
    if (!processed)
    {
        for (UIEventProcessorDelegate* p : processors)
        {
            processed = p->ProcessEvent(sceneControl, event);
            if (processed)
            {
                break;
            }
...
void UIEventsSystem::AddProcessor(UIEventProcessorDelegate* delegate)
{
    processors.insert(delegate);
}

void UIEventsSystem::RemoveProcessor(UIEventProcessorDelegate* delegate)
{
    processors.erase(delegate);
}
}
