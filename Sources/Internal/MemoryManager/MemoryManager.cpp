...
void MemoryManager::SetCallbacks(Function<void()> updateCallback_, Function<void(uint32, bool)> tagCallback_, Function<void()> finishCallback_)
{
...
bool MemoryManager::TryEnterNestedCall()
{
    uintptr_t value = reinterpret_cast<uintptr_t>(tlsNestedCall.Get());
    if (value == 0)
    {
        tlsNestedCall.Reset(reinterpret_cast<int*>(1));
        return true;
    }
    return false;
}

void MemoryManager::LeaveNestedCall()
{
    tlsNestedCall.Reset(reinterpret_cast<int*>(0));
}
#else
bool MemoryManager::TryEnterNestedCall()
{
