...
Reflection StructureWrapperClass::GetField(const ReflectedObject& object, const ValueWrapper* vw, const Any& key) const
{
    ...
    {
        ReflectedStructure::Key name = key.CastSafely<ReflectedStructure::Key>(ReflectedStructure::Key());
        if (!name.empty())
        {
...
Reflection::Field StructureWrapperClass::GetField(const ReflectedObject& object, const ValueWrapper* vw, size_t index) const
{
    if (index < fieldsCache.size())
    {
        const ReflectedStructure::Field* field = fieldsCache[index].field;
        return Reflection::Field(field->name, Reflection(vw->GetValueObject(object), field->valueWrapper.get(), nullptr, field->meta.get()), fieldsCache[index].inheritFrom);
    }

    return Reflection::Field();
}

Vector<Reflection::Field> StructureWrapperClass::GetFields(const ReflectedObject& object, const ValueWrapper* vw, Reflection::MetaPredicate pred) const
{
    ...
    for (const CachedFieldEntry& fieldEntry : fieldsCache)
    {
        ...
        if (nullptr == pred || (*pred)(field->meta.get()))
        {
            Any key(field->name);
...
{
    FastName name = key.CastSafely<FastName>(FastName());
    if (name.IsValid())
    {
...
Vector<Reflection::Method> StructureWrapperClass::GetMethods(const ReflectedObject& object, const ValueWrapper* vw, Reflection::MetaPredicate pred) const
{
    ...
    for (const CachedMethodEntry& methodEntry : methodsCache)
    {
        ...
        if (nullptr == pred || (*pred)(method->meta.get()))
        {
