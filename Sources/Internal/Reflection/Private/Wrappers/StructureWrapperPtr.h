...
public:
    ...
    Reflection::Field GetField(const ReflectedObject& obj, const ValueWrapper* vw, size_t index) const override
    {
        ...
        if (nullptr != sw)
        {
            return sw->GetField(derefObj, &ptrVW, index);
        }

        return StructureWrapperDefault::GetField(obj, vw, index);
    }
    ...
    Vector<Reflection::Field> GetFields(const ReflectedObject& obj, const ValueWrapper* vw, Reflection::MetaPredicate pred) const override
    {
        ...
        if (nullptr != sw)
        {
            return sw->GetFields(derefObj, &ptrVW, pred);
        }

        return StructureWrapperDefault::GetFields(obj, vw, pred);
    }
    ...
    Vector<Reflection::Method> GetMethods(const ReflectedObject& obj, const ValueWrapper* vw, Reflection::MetaPredicate pred) const override
    {
        ReflectedObject derefObj = Deref(vw->GetValueObject(obj));
        const StructureWrapper* sw = GetInternalWrapper(derefObj);
        if (nullptr != sw)
        {
            return sw->GetMethods(derefObj, &ptrVW, pred);
        }

        return StructureWrapperDefault::GetMethods(obj, vw, pred);
    }
