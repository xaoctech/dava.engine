...
struct LoggerEnabler : public std::false_type
{
};
...
public:
    ...
    */
    ...
    /* Conditional debug syntax */
    /*
    When you use this syntax your log messages can be disabled and enabled in runtime for your specific module.
    Normally you can use those for messages you want to see during feature development phase.
    */
    ...
    static typename std::enable_if<!LoggerEnabler<T, Logger::LEVEL_DEBUG>::value>::type Debug(const char8* text, Args... args)
    {
    }
    ...
    static typename std::enable_if<LoggerEnabler<T, Logger::LEVEL_DEBUG>::value>::type Debug(const char8* text, Args... args)
    {
        Logger::Debug(text, std::forward<Args>(args)...);
    }
...
#define LoggerEnableLevel(Type, Level) template <> struct LoggerEnabler<Type, Level> : public std::true_type{};

#define LoggerEnable(Type)\
LoggerEnableLevel(Type, Logger::LEVEL_FRAMEWORK) \
    LoggerEnableLevel(Type, Logger::LEVEL_DEBUG) \
    LoggerEnableLevel(Type, Logger::LEVEL_INFO) \
    LoggerEnableLevel(Type, Logger::LEVEL_WARNING) \
    LoggerEnableLevel(Type, Logger::LEVEL_ERROR)
