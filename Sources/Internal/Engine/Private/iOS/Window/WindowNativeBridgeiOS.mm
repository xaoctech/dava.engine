...
bool WindowNativeBridge::CreateWindow()
{
    ...
    mainDispatcher->PostEvent(MainDispatcherEvent::CreateWindowCreatedEvent(window, rect.size.width, rect.size.height, rect.size.width * scale, rect.size.height * scale, dpi, Fullscreen::On));
...
void WindowNativeBridge::ViewWillTransitionToSize(float32 w, float32 h)
{
    ...
    mainDispatcher->PostEvent(MainDispatcherEvent::CreateWindowSizeChangedEvent(window, w, h, w * scale, h * scale, Fullscreen::On));
}
