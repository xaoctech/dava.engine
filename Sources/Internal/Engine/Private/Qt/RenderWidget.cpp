...
RenderWidget::RenderWidget(IWindowDelegate* widgetDelegate, uint32 width, uint32 height)
{
    ...
    {
        bool renderInThread = options->GetBool("qt_platform_render_in_thread", false);
        RenderWidgetOGL* oglWidget = new RenderWidgetOGL(widgetDelegate, width, height, renderInThread, this);
    ...
    renderWidgetBackend->mouseEntered.Connect(&mouseEntered, &Signal<>::Emit);
    renderWidgetBackend->mouseLeaved.Connect(&mouseLeaved, &Signal<>::Emit);
}
