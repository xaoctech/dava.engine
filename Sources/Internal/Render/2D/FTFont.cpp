                                               ...
                                               bool contentScaleIncluded)
{
    ...
    int32 faceBboxYMin = int32(FT_MulFix_Wrapper(face->bbox.yMin, face->size->metrics.y_scale) * descendScale);
    int32 faceBboxYMax = int32(FT_MulFix_Wrapper(face->bbox.yMax, face->size->metrics.y_scale) * ascendScale);
    ...
    int32 baseSize = int32(ceilf(UIControlSystem::Instance()->vcs->ConvertVirtualToPhysicalX(bboxSize)));
    ...
    metrics.baseline = int32(ceilf(UIControlSystem::Instance()->vcs->ConvertVirtualToPhysicalX(faceBboxYMax / ftToPixelScale)));
    ...
    if (!contentScaleIncluded)
    {
        metrics.drawRect.x = int32(floorf(UIControlSystem::Instance()->vcs->ConvertPhysicalToVirtualX(float32(metrics.drawRect.x))));
        metrics.drawRect.y = int32(floorf(UIControlSystem::Instance()->vcs->ConvertPhysicalToVirtualY(float32(metrics.drawRect.y))));
        metrics.drawRect.dx = int32(ceilf(UIControlSystem::Instance()->vcs->ConvertPhysicalToVirtualX(float32(metrics.drawRect.dx))));
        metrics.drawRect.dy = int32(ceilf(UIControlSystem::Instance()->vcs->ConvertPhysicalToVirtualY(float32(metrics.drawRect.dy))));
        metrics.baseline = int32(ceilf(UIControlSystem::Instance()->vcs->ConvertPhysicalToVirtualX(float32(metrics.baseline))));
        metrics.height = int32(ceilf(UIControlSystem::Instance()->vcs->ConvertPhysicalToVirtualY(float32(metrics.height))));
        metrics.width = int32(ceilf(UIControlSystem::Instance()->vcs->ConvertPhysicalToVirtualX(totalWidth)));
    }
    else
    {
