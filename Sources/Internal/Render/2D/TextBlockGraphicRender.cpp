...
void TextBlockGraphicRender::Draw(const Color& textColor, const Vector2* offset)
{
    ...
    offsetMatrix.CreateTranslation(Vector3(float32(xOffset) - textBlock->pivot.x, float32(yOffset) - textBlock->pivot.y, 0.f));

    Matrix4 rotateMatrix;
    rotateMatrix.CreateRotation(Vector3(0.f, 0.f, 1.f), textBlock->angle);

    Matrix4 scaleMatrix;
    //recalculate x scale - for non-uniform scale
    const float difX = 1.0f - (textBlock->scale.dy - textBlock->scale.dx);
    scaleMatrix.CreateScale(Vector3(difX, 1.f, 1.0f));

    Matrix4 worldMatrix;
    worldMatrix.CreateTranslation(Vector3(textBlock->position.x, textBlock->position.y, 0.f));
