...
float32 GetGlobalLuminanceScale()
{
    return 25000.0f;
}

float32 GetLuminanceFromEV(float32 ev)
{
    return std::exp2(ev - 3.0f) / GetGlobalLuminanceScale();
}

Vector2 GetLuminanceRangeFromEVRange(float32 base, const Vector2& range)
{
    return Vector2(GetLuminanceFromEV(base - range.x), GetLuminanceFromEV(base + range.y));
}

float32 GetEVFromCameraSettings(float32 invShutterTime, float32 apertureSize, float32 iso)
{
    return std::log2((100.0f / iso) * (apertureSize * apertureSize) * invShutterTime);
}

float32 GetEVFromLuminance(float32 lum)
{
    return std::log2(lum * GetGlobalLuminanceScale()) + 3.0f;
}
