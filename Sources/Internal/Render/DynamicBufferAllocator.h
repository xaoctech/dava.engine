...
static const uint32 DEFAULT_INSTANCE_PAGE_SIZE = 100 * 16 * 4; //typical instancing is 16 floats (4 bytes each)
...
struct AllocResultInstnceBuffer
{
    rhi::HVertexBuffer buffer;
    uint8* data;
    uint32 allocatedInstances;
};
...
AllocResultInstnceBuffer AllocateInstanceBuffer(uint32 instanceSize, uint32 count); //uses paged allocator
