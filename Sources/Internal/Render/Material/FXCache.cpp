...
const FXDescriptor& LoadOldTempalte(const FastName& fxName, const FastName& quality)
{
    ...
    target.materialType = NMaterial::TYPE_LEGACY;
    const YamlNode* materialTypeNode = stateNode->Get("MaterialType");
    if (materialTypeNode)
    {
        int32 value = -1;
        ...
        {
            target.materialType = NMaterial::eType(value);
        }
        else
        {
    ...
    {
        if (stateNode->GetItemKeyName(k).substr(0, 10) == "RenderPass") //GFX_COMPLETE - later rethink passes in yamls
        {
            ...
            const YamlNode* renderPassLayerNode = renderPassNode->Get("Layer");
            if (renderPassLayerNode)
            {
                passDescriptor.renderLayer = RenderLayer::GetLayerIDByName(renderPassLayerNode->AsFastName());
            }
        ...
        #if defined(GFX_COMPLETE)
            passDescriptor.shaderFileName = shaderNode->AsFastName();
        #else
            String shaderFileName = shaderNode->AsString();
            passDescriptor.shaderFileName = FastName(shaderFileName);
        #endif
            ...
            const YamlNode* pinsNode = renderPassNode->Get("UniquePins");
            if (pinsNode)
            {
                int32 count = pinsNode->GetCount();
                for (int32 k = 0; k < count; ++k)
                {
                    const YamlNode* singlePinNode = pinsNode->Get(k);
                    FastName pinName = FastName(singlePinNode->Get(0)->AsString().c_str());
                    int32 pinValue = singlePinNode->Get(1)->AsInt32();
                    passDescriptor.templateDefines[pinName] = pinValue;
                }
            }
