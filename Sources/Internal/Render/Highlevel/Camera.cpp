...
Vector3 Camera::GetOnScreenPositionAndDepth(const Vector3& forPoint, const Rect& viewport)
{
    ...
    Vector4 pv(forPoint);
    pv = pv * GetViewProjMatrix(false, false);

    return Vector3(((pv.x / pv.w) * 0.5f + 0.5f) * viewport.dx + viewport.x,
                   (1.0f - ((pv.y / pv.w) * 0.5f + 0.5f)) * viewport.dy + viewport.y, pv.w + pv.z);
}

const Matrix4& Camera::GetViewProjMatrix(bool invertProjection, bool invertZ)
{
    if (flags & REQUIRE_REBUILD)
    {
        RebuildCameraFromValues();
    }
    if ((flags & REQUIRE_REBUILD_PROJECTION) || GetReverseZEnabled() != invertZ)
    {
        RebuildProjectionMatrix(invertProjection, invertZ);
    }
...
void Camera::RebuildProjectionMatrix(bool invertProjection, bool invertZ)
{
    flags &= ~REQUIRE_REBUILD_PROJECTION;
    flags |= REQUIRE_REBUILD_UNIFORM_PROJ_MODEL;

    if (invertZ)
        flags |= USES_REVERSED_Z;
    else
        flags &= ~USES_REVERSED_Z;
...
void Camera::ExtractCameraToValues()
{
    ...
    target.x = cameraTransform._02;
    target.y = cameraTransform._12;
    target.z = cameraTransform._22;

    Matrix4 invMx;
    cameraTransform.GetInverse(invMx);

    position = invMx.GetTranslationVector();
}

void Camera::PrepareDynamicParameters(bool invertProjection, bool invertZ, Vector4* externalClipPlane)
{
    ...
    projMatrix.GetInverse(invProjMatrix);
...
{
    PrepareDynamicParameters(invertProjection, invertZ, externalClipPlane);
    ...
    Renderer::GetDynamicBindings().SetDynamicParam(DynamicBindings::PARAM_INV_PROJ, &invProjMatrix, DynamicBindings::UPDATE_SEMANTIC_ALWAYS);
...
Vector3 Camera::UnProject(float32 winx, float32 winy, float32 winz, const Rect& viewport)
{
    ...
    Matrix4 finalMatrix = GetViewProjMatrix(false, false);
    finalMatrix.Inverse();
...
{
    Quaternion orient;
    orient.Construct(cameraTransform);
    orient.Normalize();
    return orient;
}
...
void Camera::Rotate(const Quaternion& rot)
{
    ...
    backup->CopyMathOnly(*this);
    SetDirection(rot.ApplyToVectorFast(GetDirection()));
    RebuildCameraFromValues();
}
