...
DAVA_VIRTUAL_REFLECTION_IMPL(RenderBatch)
{
    ...
    .Field("aabbbox", &RenderBatch::aabbox)[M::DisplayName("Bounding box"), M::Group("Bounding box")]
    .Field("material", &RenderBatch::GetMaterial, &RenderBatch::SetMaterial)[M::DisplayName("Material")]
...
void RenderBatch::Load(KeyedArchive* archive, SerializationContext* serializationContext)
{
    if (NULL != archive)
    {
        ...
        if (mat != nullptr)
        {
            mat->PreBuildMaterial(PASS_FORWARD);
        }
    }
