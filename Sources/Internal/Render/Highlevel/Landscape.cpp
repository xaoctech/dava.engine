...
Landscape::Landscape()
{
    ...
    PrepareMaterial(landscapeRuntimeMaterial);

    decorationRuntimeMaterial = new NMaterial();
    decorationRuntimeMaterial->SetMaterialName(FastName("Decoration_Runtime_Material"));
    decorationRuntimeMaterial->AddFlag(NMaterialFlagName::FLAG_DECORATION_DRAW_LEVELS, 0);
    decorationRuntimeMaterial->AddTexture(TEXTURE_DECORATION, decorationVTexture->GetLayerTexture(0));
    decorationRuntimeMaterial->SetParent(landscapeRuntimeMaterial);
    decorationRuntimeMaterial->SetRuntime(true);

    AddFlag(RenderObject::CUSTOM_PREPARE_TO_RENDER);

    SetDynamicProperty(DynamicBindings::PARAM_LANDSCAPE_HEIGHTMAP_SIZE_POW2, &heightmapSizePow2f, pointer_size(&heightmapSizePow2f));
    SetDynamicProperty(DynamicBindings::PARAM_LANDSCAPE_HEIGHTMAP_SIZE, &heightmapSizef, pointer_size(&heightmapSizef));
    SetDynamicProperty(DynamicBindings::PARAM_TESSELLATION_HEIGHT, &tessellationHeight, pointer_size(&tessellationHeight));
...
void Landscape::ReleaseGeometryData()
{
    ...
    if (landscapeRuntimeMaterial->HasLocalTexture(NMaterialTextureName::TEXTURE_HEIGHTMAP))
        landscapeRuntimeMaterial->RemoveTexture(NMaterialTextureName::TEXTURE_HEIGHTMAP);

    if (landscapeRuntimeMaterial->HasLocalTexture(NMaterialTextureName::TEXTURE_TANGENTMAP))
        landscapeRuntimeMaterial->RemoveTexture(NMaterialTextureName::TEXTURE_TANGENTMAP);
...
{
    landscapeRuntimeMaterial->SetFlag(NMaterialFlagName::FLAG_LANDSCAPE_USE_INSTANCING, (renderMode == RENDERMODE_NO_INSTANCING) ? 0 : 1);
    landscapeRuntimeMaterial->SetFlag(NMaterialFlagName::FLAG_LANDSCAPE_LOD_MORPHING, (renderMode == RENDERMODE_INSTANCING_MORPHING) ? 1 : 0);
...
void Landscape::RequestPages(const LandscapeSubdivision::SubdivisionPatch* subdivPatch)
{
    ...
    if (subdivPatch->level <= decoration->GetBaseLevel())
...
void Landscape::SetPageMaterial(uint32 layerIndex, uint32 materialIndex, NMaterial* material) const
{
    DVASSERT(layerIndex < layersCount);
    landscapeLayerRenderers[layerIndex]->SetTerrainLODMaterial(materialIndex, material);
    landscapeLayerRenderers[layerIndex]->GetDecorationLODMaterial(materialIndex)->SetParent(material);
}
...
RenderObject* Landscape::Clone(RenderObject* newObject)
{
    ...
    newLandscape->decoration->CopyParameters(decoration);
...
void Landscape::SetDrawDecorationLevels(bool draw)
{
    if (debugDrawDecorationLevels != draw)
    {
        ...
        decorationRuntimeMaterial->SetFlag(NMaterialFlagName::FLAG_DECORATION_DRAW_LEVELS, debugDrawDecorationLevels ? 1 : 0);
    }
}
