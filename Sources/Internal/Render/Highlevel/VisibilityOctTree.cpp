...
void VisibilityOctTree::Clip(Camera* _camera, RenderHierarchy::ClipResult& visibilityArray, uint32 _visibilityCriteria)
{
    camera = _camera;
    ...
    InternalClip(rootCoord, worldBBox, 0x3f);
    ...
    for (uint32 k = 0; k < size; ++k)
        if (visibleObjects.At(k))
        {
            ...
            visibilityArray.AddObject(roIndices[k]);
...
void VisibilityOctTree::InternalClip(VoxelCoord voxelCoord, const AABBox3& nodeBox, uint8 clipMask)
{
    ...
    if (currentNode.children)
    {
        ...
        for (uint32 childBitIndex = 0; childBitIndex < 8; ++childBitIndex)
        {
            if ((currentNode.children >> childBitIndex) & 1)
            {
                ...
                InternalClip(childCoord, childBox, clipMask);
            }
        }
    }
}
