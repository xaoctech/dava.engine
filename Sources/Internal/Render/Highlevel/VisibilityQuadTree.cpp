...
void QuadTree::ProcessNodeClipping(uint16 nodeId, uint8 clippingFlags, RenderHierarchy::ClipResult& visibilityArray)
{
    QuadTreeNode& currNode = nodes[nodeId];
...
void QuadTree::Clip(Camera* camera, RenderHierarchy::ClipResult& visibilityArray, uint32 visibilityCriteria)
{
    DVASSERT(worldInitialized);
    currCamera = camera;
...
void QuadTree::DebugDraw(const Matrix4& cameraMatrix, RenderHelper* renderHelper)
{
    ...
    DebugDrawNode(0, renderHelper);
}

void QuadTree::DebugDrawNode(uint16 nodeId, RenderHelper* renderHelper)
{
    ...
    {
        renderHelper->DrawAABox(nodes[nodeId].objects[i]->GetWorldBoundingBox(), colorObject, RenderHelper::DRAW_WIRE_DEPTH);
    }

    renderHelper->DrawAABox(nodes[nodeId].bbox, colorNode, RenderHelper::DRAW_WIRE_DEPTH);
    for (int32 i = 0; i < QuadTreeNode::NODE_NONE; ++i)
    {
        if (nodes[nodeId].children[i] != INVALID_TREE_NODE_INDEX)
        {
            DebugDrawNode(nodes[nodeId].children[i], renderHelper);
        }
    }
}
