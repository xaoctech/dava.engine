...
void DecorationData::ReloadDecoration()
{
    ...
    if (decorationPath.IsEqualToExtension(".sc2"))
    {
        ...
        for (uint32 l = 0; l < layersCount; ++l)
        {
            ...
            layersData[l].variations.resize(variationCount);
            layersParams[l].variations.resize(variationCount);
        }
    ...
    Asset<Prefab> decorationPrefab = GetEngineContext()->assetManager->LoadAsset<Prefab>(decorationPath);
    ...
    for (uint32 l = 0; l < layersCount; ++l)
    {
        Entity* layerEntity = decorationEntities[l];
        ...
        for (uint32 v = 0; v < variationCount; ++v)
        {
            ...
            MeshComponent* meshComponent = varEntity->GetComponent<MeshComponent>();

            if (meshComponent == nullptr)
                continue;

            const Vector<MeshLODDescriptor>& lodDescriptors = meshComponent->GetMeshDescriptor();
            if (!lodDescriptors.empty() && !lodDescriptors[0].batchDescriptors.empty())
            {
                const MeshLODDescriptor& lodDescriptor = lodDescriptors[0];
                const MeshBatchDescriptor& batchDescriptor = lodDescriptor.batchDescriptors[0];

                layerMaterial = batchDescriptor.materialAsset->GetMaterial();
                varGeometry = lodDescriptor.geometryAsset->GetPolygonGroup(batchDescriptor.geometryIndex);
            }
            else
            {
                Logger::Error("[Decoration loading] Error: each variation entity must have mesh");
            }
...
void DecorationData::CopyParameters(DecorationData* from)
{
