...
struct DynamicPropertyParam
{
    DynamicBindings::eUniformSemantic propertySemantic;
    pointer_size updateSemantic;
    const void* data;
};

struct DynamicTextureParam
{
    DynamicBindings::eTextureSemantic textureSemantic;
    rhi::HTexture handle;
};
...
struct RenderBatchWithOptions : public InspBase
{
    ...
    Vector<DynamicPropertyParam> dynamicProps;
    Vector<DynamicTextureParam> dynamicTextures;
    int32 lodIndex = -2;
    int32 switchIndex = -1;
...
public:
    ...
    static const uint32 SERIALIZATION_CRITERIA = VISIBLE | VISIBLE_REFLECTION | VISIBLE_REFRACTION | ALWAYS_CLIPPING_VISIBLE | VISIBLE_SHADOW_CASTER;
...
public:
    ...
    DAVA_DEPRECATED(virtual void Save(KeyedArchive* archive, SerializationContext* serializationContext));
    DAVA_DEPRECATED(virtual void Load(KeyedArchive* archive, SerializationContext* serializationContext));

    void SaveFlags(KeyedArchive* archive, SerializationContext* serializationContext);
    void LoadFlags(KeyedArchive* archive, SerializationContext* serializationContext);
    ...
    void SetDynamicProperty(DynamicBindings::eUniformSemantic shaderSemantic, const void* value, pointer_size _updateSemantic);
    void SetDynamicProperty(RenderBatch* batch, DynamicBindings::eUniformSemantic shaderSemantic, const void* value, pointer_size _updateSemantic);

    void RemoveDynamicProperty(DynamicBindings::eUniformSemantic shaderSemantic);
    void RemoveDynamicProperty(RenderBatch* batch, DynamicBindings::eUniformSemantic shaderSemantic);

    void SetDynamicTexture(DynamicBindings::eTextureSemantic textureSemantic, rhi::HTexture handle);
    void SetDynamicTexture(RenderBatch* batch, DynamicBindings::eTextureSemantic textureSemantic, rhi::HTexture handle);

    void RemoveDynamicTexture(DynamicBindings::eTextureSemantic textureSemantic);
    void RemoveDynamicTexture(RenderBatch* batch, DynamicBindings::eTextureSemantic textureSemantic);
    ...
    inline void SetShadowCaster(bool visible);
    ...
    void SetDiffuseGIProbe(Texture* diffuseGIProbe);

    void SetLocalReflectionProbe(ReflectionProbe* probe);
    ReflectionProbe* GetLocalReflectionProbe() const;
    void SetGlobalReflectionProbe(ReflectionProbe* probe);
    ReflectionProbe* GetGlobalReflectionProbe() const;
...
protected:
    ...
    RenderBatchWithOptions* FindRenderBatchWithOptions(RenderBatch* batch);
    static void SetDynamicPropertyInternal(Vector<DynamicPropertyParam>* dynamicProps, DynamicBindings::eUniformSemantic shaderSemantic, const void* value, pointer_size _updateSemantic);
    static void RemoveDynamicPropertyInternal(Vector<DynamicPropertyParam>* dynamicProps, DynamicBindings::eUniformSemantic shaderSemantic);
    static void SetDynamicTextureInternal(Vector<DynamicTextureParam>* dynamicTextures, DynamicBindings::eTextureSemantic textureSemantic, rhi::HTexture handle);
    static void RemoveDynamicTextureInternal(Vector<DynamicTextureParam>* dynamicTextures, DynamicBindings::eTextureSemantic textureSemantic);
...
protected:
    ...
    Vector<DynamicPropertyParam> objectDynamicProps;
    Vector<DynamicTextureParam> objectDynamicTextures;

    Light* lights[MAX_LIGHT_COUNT];
    ReflectionProbe* localReflectionProbe = nullptr;
    ReflectionProbe* globalReflectionProbe = nullptr;
    // Texture* diffuseGIProbe = nullptr;
...
inline bool RenderObject::GetShadowCaster() const
{
    return (flags & VISIBLE_SHADOW_CASTER) == VISIBLE_SHADOW_CASTER;
}

inline void RenderObject::SetShadowCaster(bool visible)
{
    if (visible)
        flags |= VISIBLE_SHADOW_CASTER;
    else
        flags &= ~VISIBLE_SHADOW_CASTER;
}

/*
inline void RenderObject::SetDiffuseGIProbe(Texture* diffuseGIProbe_)
{
    diffuseGIProbe = diffuseGIProbe_;
}
// */

inline ReflectionProbe* RenderObject::GetLocalReflectionProbe() const
{
    return localReflectionProbe;
}

inline ReflectionProbe* RenderObject::GetGlobalReflectionProbe() const
{
    return globalReflectionProbe;
}
