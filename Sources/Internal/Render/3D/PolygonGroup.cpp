...
void PolygonGroup::UpdateDataPointersAndStreamsLegacy()
{
    DAVA_MEMORY_PROFILER_CLASS_ALLOC_SCOPE();
...
void PolygonGroup::AllocateData(int32 _meshFormat, int32 _vertexCount, int32 _indexCount, int32 _primitiveCount)
{
    ...
    indexFormat = (vertexCount < 65535) ? EIF_16 : EIF_32;
    ...
    indexArray = new uint8[indexCount * INDEX_FORMAT_SIZE[indexFormat]];
...
void PolygonGroup::BuildBuffers()
{
    ...
    ibDesc.indexSize = (indexFormat == EIF_16) ? rhi::INDEX_SIZE_16BIT : rhi::INDEX_SIZE_32BIT;
...
void PolygonGroup::Save(KeyedArchive* keyedArchive, SerializationContext* serializationContext)
{
    ...
    keyedArchive->SetInt32("indexFormat", int32(indexFormat));
    keyedArchive->SetByteArray("indices", indexArray, indexCount * INDEX_FORMAT_SIZE[indexFormat]);
...
void PolygonGroup::LoadPolygonData(KeyedArchive* keyedArchive, SerializationContext* serializationContext, int32 requiredFlags, bool cutUnusedStreams)
{
    ...
    memcpy(indexArray, archiveData, indexDataSize);
