...
PolygonGroup* BuildAABox(const Vector3& e)
{
    PolygonGroup* unityCube = new PolygonGroup();
    unityCube->AllocateData(EVF_VERTEX, 8, 36);
    unityCube->SetCoord(0, Vector3(-e.x, -e.y, e.z));
    unityCube->SetCoord(1, Vector3(e.x, -e.y, e.z));
    unityCube->SetCoord(2, Vector3(e.x, e.y, e.z));
    unityCube->SetCoord(3, Vector3(-e.x, e.y, e.z));

    unityCube->SetCoord(4, Vector3(-e.x, -e.y, -e.z));
    unityCube->SetCoord(5, Vector3(e.x, -e.y, -e.z));
    unityCube->SetCoord(6, Vector3(e.x, e.y, -e.z));
    unityCube->SetCoord(7, Vector3(-e.x, e.y, -e.z));

    int32 i = 0;

    //z+
    unityCube->SetIndex(i + 0, 0);
    unityCube->SetIndex(i + 1, 1);
    unityCube->SetIndex(i + 2, 2);
    unityCube->SetIndex(i + 3, 2);
    unityCube->SetIndex(i + 4, 3);
    unityCube->SetIndex(i + 5, 0);
    i += 6;

    //z-
    unityCube->SetIndex(i + 0, 4);
    unityCube->SetIndex(i + 1, 7);
    unityCube->SetIndex(i + 2, 6);
    unityCube->SetIndex(i + 3, 6);
    unityCube->SetIndex(i + 4, 5);
    unityCube->SetIndex(i + 5, 4);
    i += 6;

    //y-
    unityCube->SetIndex(i + 0, 4);
    unityCube->SetIndex(i + 1, 5);
    unityCube->SetIndex(i + 2, 1);
    unityCube->SetIndex(i + 3, 1);
    unityCube->SetIndex(i + 4, 0);
    unityCube->SetIndex(i + 5, 4);
    i += 6;

    //y+
    unityCube->SetIndex(i + 0, 7);
    unityCube->SetIndex(i + 1, 3);
    unityCube->SetIndex(i + 2, 2);
    unityCube->SetIndex(i + 3, 2);
    unityCube->SetIndex(i + 4, 6);
    unityCube->SetIndex(i + 5, 7);
    i += 6;

    //x+
    unityCube->SetIndex(i + 0, 1);
    unityCube->SetIndex(i + 1, 5);
    unityCube->SetIndex(i + 2, 6);
    unityCube->SetIndex(i + 3, 6);
    unityCube->SetIndex(i + 4, 2);
    unityCube->SetIndex(i + 5, 1);
    i += 6;

    //x-
    unityCube->SetIndex(i + 0, 4);
    unityCube->SetIndex(i + 1, 0);
    unityCube->SetIndex(i + 2, 3);
    unityCube->SetIndex(i + 3, 3);
    unityCube->SetIndex(i + 4, 7);
    unityCube->SetIndex(i + 5, 4);

    unityCube->BuildBuffers();
...
void RebuildMeshTangentSpace(PolygonGroup* group, bool precomputeBinormal /*=true*/)
{
    ...
    for (uint32 f = 0; f < faceCount; ++f)
    {
        ...
        Vector2 texCoord[3];
        Vector3 avgNormal;
        Vector3 currNormal; //out of for to prevent constructor
        for (uint32 i = 0; i < 3; ++i)
        {
            ...
            group->GetNormal(originIndex, currNormal);
            avgNormal += currNormal;
        ...
        if (tangent.Length() < EPS)
            tangent = binormal.CrossProduct(avgNormal);
        if (binormal.Length() < EPS)
            binormal = avgNormal.CrossProduct(tangent);
        tangent.Normalize();
        binormal.Normalize();
    ...
    bool hardcodedNormals = false;
    bool hardcodedTangents = false;
    bool hardcodedBinormals = false;
    // copy vertices
    for (uint32 i = 0, sz = static_cast<uint32>(verticesOrigin.size()); i < sz; ++i)
    {
    ...
    if (hardcodedNormals || hardcodedTangents || hardcodedBinormals)
    {
        Logger::Error("Failed to rebuild tangent space.\nNormals: [%s]\nTangents: [%s]\nBinormals: [%s]",
                      !hardcodedNormals ? "OK" : "SOME HARDCODED",
                      !hardcodedTangents ? "OK" : "SOME HARDCODED",
                      !hardcodedBinormals ? "OK" : "SOME HARDCODED");
    }
