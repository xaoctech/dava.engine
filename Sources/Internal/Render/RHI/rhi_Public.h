...
struct RenderDeviceCaps
{
    ...
    bool isDepthTextureSupported = false;
    bool isReverseDepthSupported = false;
...
const InitParam& GetInitializeParams();
...
bool IsInvertedProjectionRequired(bool renderToTexture, bool renderTargetIsCubemap);
...
void* MapTexture(HTexture tex, uint32 level = 0, TextureFace face = TEXTURE_FACE_NEGATIVE_X);
...
void ReadTextureRegion(HTexture tex, void* data, uint32 dataSize, const Rect2i& rect, uint32 level = 0, TextureFace face = TEXTURE_FACE_NONE);
...
struct TextureSetDescriptor
{
    ...
    /*
        The purpose of these flags are define textures that can be interchanged from call to call. 
     */
    union
    {
        ...
        struct
        {
            uint32 fragmentTextureDynamic : 16;
            uint32 vertexTextureDynamic : 16;
        };
    };
};
...
// void TextureSetMakeDynamic(HTextureSet textureSetHandle, uint32 isDynamic); GFX_COMPLETE
