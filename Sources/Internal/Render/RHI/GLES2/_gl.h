...
#define GL_CALL(expr) { \
    ...
    if (err != GL_NO_ERROR) { \
        DAVA::Logger::Error("OpenGL BEFORE call %s failed with %s", #expr, glErrorToString(err)); /*DVASSERT(!"Before GL_CALL");*/ \
    } \
    expr; \
    err = glGetError(); \
    if (err != GL_NO_ERROR) { \
        DAVA::Logger::Error("OpenGL AFTER call %s failed with %s", #expr, glErrorToString(err)); /*DVASSERT(!"After GL_CALL");*/ \
    } \
}
...
GLenum GetHalfFloatEnumValue();
GLenum GetR16FEnumValue();
GLenum GetR32FEnumValue();
GLenum GetRG16FEnumValue();
GLenum GetRG32FEnumValue();
GLenum GetRGBA16FEnumValue();
GLenum GetRGBA32FEnumValue();
GLenum GetD16EnumValue();
...
namespace rhi
{
inline GLenum GetComparisonFunctionValue(CmpFunc func)
{
    switch (func)
    {
    case CMP_NEVER:
    ...
    case CMP_ALWAYS:
        return GL_ALWAYS;
    default:
        DVASSERT(!"Invalid CmpFunc provided");
        return GL_ALWAYS;
    }
}
}
