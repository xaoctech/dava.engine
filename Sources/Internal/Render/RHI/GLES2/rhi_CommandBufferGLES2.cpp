...
#include "rhi_OpenGLState.h"
    ...
    #define ACQUIRE_CONTEXT() do { _GLES2_AcquireContext(); glState.InvalidateState(); } while (0)
    #define RELEASE_CONTEXT() do { _GLES2_ReleaseContext(); } while (0)
#else
...
static Handle gles2_RenderPass_Allocate(const RenderPassConfig& passConf, uint32 cmdBufCount, Handle* cmdBuf)
{
    ...
    for (uint32 i = 0; i != cmdBufCount; ++i)
    {
        ...
        ((passConf.depthStencilBuffer.texture == InvalidHandle) || (passConf.depthStencilBuffer.texture == DefaultDepthBuffer));
...
void CommandBufferGLES2_t::Execute()
{
    ...
    for (const uint8 *c = cmdData, *c_end = cmdData + curUsedSize; c != c_end;)
    {
        ...
        case CMD_BEGIN:
        {
            ...
            if (isFirstInPass)
            {
                SetGLClipControlToReverseDepthEnabled(passCfg.usesReverseDepth);
                ...
                for (uint32 i = 0; i != countof(passCfg.colorBuffer); ++i)
                {
                    ...
                    {
                        if ((passCfg.depthStencilBuffer.texture != DefaultDepthBuffer) && (passCfg.depthStencilBuffer.texture != InvalidHandle))
                        {
                            Size2i sz = TextureGLES2::Size(passCfg.depthStencilBuffer.texture);
                            def_viewport[2] = sz.dx;
                            def_viewport[3] = sz.dy;
                            rt[0] = passCfg.depthStencilBuffer.texture;
                            rt_count = 0;
                            apply_fb = true;
                        }
        ...
        case CMD_END:
        {
            ...
            if (isLastInPass)
            {
                ...
                if (_GLES2_Bound_FrameBuffer != _GLES2_Default_FrameBuffer) // defualt framebuffer is discard once after frame
                {
                    ...
                    if (discardColor || discardDepthStencil)
...
static void _GLES2_ExecuteQueuedCommands(const CommonImpl::Frame& frame)
{
...
#if RHI_GL_DUMP_PASS_ORDER
    static int dumpPassOrderCounter = 0;

    const char* loadActionNames[] =
    {
      "LOADACTION_NONE ",
      "LOADACTION_CLEAR",
      "LOADACTION_LOAD "
    ...
    if ((dumpPassOrderCounter % 100) == 0)
    {
        Logger::Info("Frame %u pass order: ", frame_n);
        for (std::vector<RenderPassGLES2_t *>::iterator p = pass.begin(), p_end = pass.end(); p != p_end; ++p)
        {
            CommandBufferGLES2_t* cb = CommandBufferPoolGLES2::Get((*p)->cmdBuf[0]);

            Logger::Info("  *  %d  -  %s, ColorTarget: 0x%08x, %s %s, DepthTarget: 0x%08x, %s %s", (*p)->priority, (*p)->name,
                         cb->passCfg.colorBuffer[0].texture, loadActionNames[cb->passCfg.colorBuffer[0].loadAction], storeActionNames[cb->passCfg.colorBuffer[0].storeAction],
                         cb->passCfg.depthStencilBuffer.texture, loadActionNames[cb->passCfg.depthStencilBuffer.loadAction], storeActionNames[cb->passCfg.depthStencilBuffer.storeAction]);
        }
    }
    dumpPassOrderCounter++;    
#endif
