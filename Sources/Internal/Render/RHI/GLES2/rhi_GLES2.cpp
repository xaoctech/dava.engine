...
static bool PackedFloat_Supported = false;
...
static void gles_check_GL_extensions()
{
    ...
    if (!IsEmptyString(ext))
    {
        ...
        PackedFloat_Supported = (strstr(ext, "GL_EXT_packed_float") != nullptr);
        ...
        MutableDeviceCaps::Get().isReverseDepthSupported = strstr(ext, "GL_ARB_clip_control") != nullptr;
        ...
        {
            float32 value = 0.0f;
            GL_CALL(glGetFloatv(GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT, &value));
...
#else
        MutableDeviceCaps::Get().isDepthTextureSupported = (strstr(ext, "OES_depth_texture") != nullptr);
#endif
    ...
    if (MutableDeviceCaps::Get().isReverseDepthSupported)
    {
        ...
        impl_glClipControl = reinterpret_cast<PFNGLCLIPCONTROLPROC_T>(wglGetProcAddress("glClipControl"));
        if (impl_glClipControl == nullptr)
            impl_glClipControl = reinterpret_cast<PFNGLCLIPCONTROLPROC_T>(wglGetProcAddress("glClipControlARB"));
        
#elif defined(__DAVAENGINE_MACOS__) && defined(GL_ARB_clip_control)
...
{
#define IF_CASE(ptr, X) case X: ptr = #X; break
    const char* sourceString = "???";
    switch (source)
    {
        IF_CASE(sourceString, GL_DEBUG_SOURCE_API);
        IF_CASE(sourceString, GL_DEBUG_SOURCE_WINDOW_SYSTEM);
        IF_CASE(sourceString, GL_DEBUG_SOURCE_SHADER_COMPILER);
        IF_CASE(sourceString, GL_DEBUG_SOURCE_THIRD_PARTY);
        IF_CASE(sourceString, GL_DEBUG_SOURCE_APPLICATION);
        IF_CASE(sourceString, GL_DEBUG_SOURCE_OTHER);
    default:
        break;
    }
    ...
    switch (type)
    {
        IF_CASE(typeString, GL_DEBUG_TYPE_ERROR);
        IF_CASE(typeString, GL_DEBUG_TYPE_DEPRECATED_BEHAVIOR);
        IF_CASE(typeString, GL_DEBUG_TYPE_UNDEFINED_BEHAVIOR);
        IF_CASE(typeString, GL_DEBUG_TYPE_PORTABILITY);
        IF_CASE(typeString, GL_DEBUG_TYPE_PERFORMANCE);
        IF_CASE(typeString, GL_DEBUG_TYPE_OTHER);
    default:
        break;
    }
    ...
    switch (severity)
    {
        IF_CASE(severityString, GL_DEBUG_SEVERITY_HIGH);
        IF_CASE(severityString, GL_DEBUG_SEVERITY_MEDIUM);
        IF_CASE(severityString, GL_DEBUG_SEVERITY_LOW);
        IF_CASE(severityString, GL_DEBUG_SEVERITY_NOTIFICATION);
    default:
        break;
    }

    if (type == GL_DEBUG_TYPE_ERROR)
        DAVA::Logger::Error("%s [%s, %s]: %s", sourceString, typeString, severityString, message);
    else
        DAVA::Logger::Info("%s [%s, %s]: %s", sourceString, typeString, severityString, message);
}
...
void gles2_Initialize(const InitParam& param)
{
    ...
    _GLES2_Present = (param.presentFunc) ? param.presentFunc : &win32_gl_end_frame;
#elif defined(__DAVAENGINE_MACOS__)
    macos_gl_init(param);
    ...
    _GLES2_Present = (param.presentFunc) ? param.presentFunc : &android_gl_end_frame;
#endif
    ...
    if (param.maxPerfQueryCount)
        PerfQueryGLES2::Init(param.maxPerfQueryCount);
...
void SetGLClipControlToReverseDepthEnabled(bool enabled)
{
    if (rhi::MutableDeviceCaps::Get().isReverseDepthSupported)
    {
        GL_CALL(impl_glClipControl(GL_LOWER_LEFT, enabled ? GL_ZERO_TO_ONE : GL_NEGATIVE_ONE_TO_ONE));
    }
}
...
GLint GetGLRenderTargetFormat(rhi::TextureFormat rhiFormat)
{
    ...
    case rhi::TEXTURE_FORMAT_RGB10A2:
        return GL_RGB10_A2;

    case rhi::TEXTURE_FORMAT_RGBA16F:
        return GL_RGBA16F;

    case rhi::TEXTURE_FORMAT_RGBA32F:
        return GL_RGBA32F;

    case rhi::TEXTURE_FORMAT_D16:
        return GetD16EnumValue();

    case rhi::TEXTURE_FORMAT_D24S8:
    ...
    case rhi::TEXTURE_FORMAT_R11G11B10F:
        return GL_R11F_G11F_B10F;

    case rhi::TEXTURE_FORMAT_D32F:
...
IMPL_GET_ENUM_VALUE(GetHalfFloatEnumValue, GL_HALF_FLOAT_OES, GL_HALF_FLOAT);
IMPL_GET_ENUM_VALUE(GetR16FEnumValue, GL_RED, GL_R16F);
IMPL_GET_ENUM_VALUE(GetR32FEnumValue, GL_RED, GL_R32F);
IMPL_GET_ENUM_VALUE(GetRG16FEnumValue, GL_RG, GL_RG16F);
IMPL_GET_ENUM_VALUE(GetRG32FEnumValue, GL_RG, GL_RG32F);
IMPL_GET_ENUM_VALUE(GetRGBA16FEnumValue, GL_RGBA, GL_RGBA16F);
IMPL_GET_ENUM_VALUE(GetRGBA32FEnumValue, GL_RGBA, GL_RGBA32F);
IMPL_GET_ENUM_VALUE(GetD16EnumValue, GL_DEPTH_COMPONENT, GL_DEPTH_COMPONENT16);

#undef IMPL_GET_ENUM_VALUE
...
bool GetGLTextureFormat(rhi::TextureFormat rhiFormat, GLint* internalFormat, GLint* format, GLenum* type, bool* compressed)
{
    ...
    case TEXTURE_FORMAT_R16F:
        *internalFormat = GetR16FEnumValue();
        *format = GL_RED;
        *type = GetHalfFloatEnumValue();
        *compressed = false;
        success = true;
        break;

    case TEXTURE_FORMAT_RG16F:
        *internalFormat = GetRG16FEnumValue();
        *format = GL_RG;
        *type = GetHalfFloatEnumValue();
        *compressed = false;
        success = true;
    ...
    case TEXTURE_FORMAT_RGBA16F:
        *internalFormat = GetRGBA16FEnumValue();
        *format = GL_RGBA;
        *type = GetHalfFloatEnumValue();
        *compressed = false;
        success = true;
    ...
    case TEXTURE_FORMAT_R32F:
        *internalFormat = GetR32FEnumValue();
        *format = GL_RED;
        *type = GL_FLOAT;
        *compressed = false;
        success = true;
    ...
    case TEXTURE_FORMAT_RG32F:
        *internalFormat = GetRG32FEnumValue();
        *format = GL_RG;
        *type = GL_FLOAT;
        *compressed = false;
        success = true;
    ...
    case TEXTURE_FORMAT_RGBA32F:
        *internalFormat = GetRGBA32FEnumValue();
        *format = GL_RGBA;
        *type = GL_FLOAT;
        *compressed = false;
        success = true;
    ...
    case TEXTURE_FORMAT_D16:
        *internalFormat = GetD16EnumValue();
