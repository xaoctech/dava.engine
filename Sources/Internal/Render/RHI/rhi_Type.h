...
enum
{
    ...
    MAX_RENDER_TARGET_COUNT = 6, //GFX_COMPLETE - 6 is enough for now (using fbFetch);
    MAX_FRAGMENT_TEXTURE_SAMPLER_COUNT = 16,
    MAX_VERTEX_TEXTURE_SAMPLER_COUNT = 4,
...
inline const char* VertexSemanticsName(VertexSemantics vs)
{
    switch (vs)
    {
    case VS_POSITION:
...
inline const char* VertexDataTypeName(VertexDataType t)
{
    switch (t)
    {
    case VDT_FLOAT:
...
struct Descriptor
{
    struct Sampler
    {
        ...
        uint32 comparisonEnabled : 1;
        uint32 comparisonFunction : 4;
        uint32 pad : 7;
...
struct RenderPassConfig
{
    ...
    struct ColorBuffer : public RenderTarget
    {
    ...
    struct DepthStencilBuffer : public RenderTarget
    {
    ...
    enum : uint32
    {
        ColorBuffersCountAutoDeduction = uint32(-1)
    };
    ...
    uint32 explicitColorBuffersCount = ColorBuffersCountAutoDeduction;
    ...
    bool IsRenderTargetPass() const
    {
        bool result = (depthStencilBuffer.texture != DefaultDepthBuffer);
        for (uint32_t i = 0; i < MAX_RENDER_TARGET_COUNT; ++i)
        {
            result |= colorBuffer[i].texture != InvalidHandle;
        }
        return result;
    }

    bool IsCubeRenderTargetPass() const
    {
        bool result = (depthStencilBuffer.texture != DefaultDepthBuffer) && (depthStencilBuffer.textureFace != TEXTURE_FACE_NONE);
        for (uint32_t i = 0; i < MAX_RENDER_TARGET_COUNT; ++i)
        {
            result |= (colorBuffer[i].texture != InvalidHandle) && (colorBuffer[i].textureFace != TEXTURE_FACE_NONE);
        }
        return result;
    }
};
...
struct ScissorRect
{
    ...
    ScissorRect(uint16 ax, uint16 ay, uint16 aw, uint16 ah)
        : x(ax)
        , y(ay)
        , width(aw)
        , height(ah)
