...
static void dx11_ExecImmediateCommand(CommonImpl::ImmediateCommand* command)
{
    ...
    for (DX11Command *cmd = commandData, *cmdEnd = commandData + command->cmdCount; cmd != cmdEnd; ++cmd)
    {
        ...
        case DX11Command::COPY_SUBRESOURCE_REGION:
            dx11.context->CopySubresourceRegion((ID3D11Resource*)(arg[0]), UINT(arg[1]), UINT(arg[2]), UINT(arg[3]), UINT(arg[4]), (ID3D11Resource*)(arg[5]), UINT(arg[6]), (const D3D11_BOX*)(arg[7]));
            break;
