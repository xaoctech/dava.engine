...
void dx11_Texture_ReadRegion(Handle tex, void* data, uint32 dataSize, const Rect2i& rect, uint32 level, TextureFace face)
{
    TextureDX11_t* self = TextureDX11Pool::Get(tex);
    TextureFormat format = self->descriptor.format;

    DVASSERT(TextureSize(format, rect.dx, rect.dy, level) == dataSize);

    ID3D11Texture2D* tex2d_copy = nullptr;
    D3D11_TEXTURE2D_DESC copydesc2d = {};
    copydesc2d.Width = rect.dx;
    copydesc2d.Height = rect.dy;
    copydesc2d.MipLevels = 1;
    copydesc2d.ArraySize = 1;
    copydesc2d.Format = DX11_TextureFormat(format);
    copydesc2d.SampleDesc.Count = 1;
    copydesc2d.SampleDesc.Quality = 0;
    copydesc2d.Usage = D3D11_USAGE_STAGING;
    copydesc2d.BindFlags = 0;
    copydesc2d.CPUAccessFlags = D3D11_CPU_ACCESS_READ;

    if (DX11DeviceCommand(DX11Command::CREATE_TEXTURE_2D, &copydesc2d, NULL, &tex2d_copy))
    {
        D3D11_MAPPED_SUBRESOURCE res = {};
        D3D11_BOX copyBox = { UINT(rect.x), UINT(rect.y), 0, UINT(rect.x + rect.dx), UINT(rect.y + rect.dy), 0 };
        DX11Command cmd1[] =
        {
          { DX11Command::COPY_SUBRESOURCE_REGION, uint64(tex2d_copy), 0, 0, 0, 0, uint64(self->tex2d), uint64(level), uint64(&copyBox) },
          { DX11Command::MAP, tex2d_copy, 0, D3D11_MAP_READ, 0, &res }
        };
        ExecDX11(cmd1, countof(cmd1));
        DX11Check(cmd1[1].retval);

        memcpy(data, res.pData, dataSize);

        DX11Command cmd2(DX11Command::UNMAP, tex2d_copy, 0);
        ExecDX11(&cmd2, 1);
        DX11Check(cmd2.retval);

        DAVA::SafeRelease(tex2d_copy);

        if (format == TEXTURE_FORMAT_R8G8B8A8)
        {
            _SwapRB8(data, data, dataSize);
        }
        else if (format == TEXTURE_FORMAT_R4G4B4A4)
        {
            _SwapRB4(data, data, dataSize);
        }
        else if (format == TEXTURE_FORMAT_R5G5B5A1)
        {
            _SwapRB5551(data, data, dataSize);
        }
    }
}
...
void SetupDispatch(Dispatch* dispatch)
{
    ...
    dispatch->impl_Texture_ReadRegion = &dx11_Texture_ReadRegion;
