...
const char* HLSLGenerator::TranslateSemantic(const char* semantic, bool output, Target target) const
{
    ...
    else if (target == TARGET_FRAGMENT)
    {
        ...
        else if (!stricmp(semantic, "SV_Target4"))
            sem = (mode == MODE_DX11) ? "SV_TARGET4" : "COLOR4";
        else if (!stricmp(semantic, "SV_Target5"))
            sem = (mode == MODE_DX11) ? "SV_TARGET5" : "COLOR5";
    }
...
bool HLSLGenerator::Generate(HLSLTree* tree_, Mode mode_, Target target_, const char* entryName_, std::string* code)
{
    ...
    if (mode == MODE_DX11)
        ...
        writer.WriteLine(0, "#define centerPixelMapping float2(0.5, 0.5)");
    writer.WriteLine(0, "");
    ...
    if ((function != nullptr) && (function->semantic != nullptr))
    {
        function->sv_semantic = TranslateSemantic(function->semantic, /*output=*/true, target);
    }

    if ((function != nullptr) && (function->returnType.baseType == HLSLBaseType_UserDefined))
    {
    ...
    HLSLArgument* argument = (function == nullptr) ? nullptr : function->argument;
    while (argument)
    {
    ...
    //
    HLSLStatement* globalVariable = root->statement;
    HLSLStatement* previousVariable = root->statement;
    while (globalVariable != nullptr)
    {
        HLSLStatement* nextGlobalVariable = globalVariable->nextStatement;
        if (!globalVariable->hidden && (globalVariable->nodeType == HLSLNodeType_Declaration))
        {
            HLSLDeclaration* decl = static_cast<HLSLDeclaration*>(globalVariable);
            if ((decl->type.flags & HLSLTypeFlag_Property) && !decl->type.array)
            {
                DVASSERT(decl->type.flags & HLSLTypeFlag_Static);

                decl->type.flags &= ~HLSLTypeFlag_Static;
                decl->type.flags |= HLSLTypeFlag_Const;

                globalVariable->nextStatement = function->statement;
                function->statement = globalVariable;
                previousVariable->nextStatement = nextGlobalVariable;
            }
            else
            {
                previousVariable = globalVariable;
            }
        }
        else
        {
        ...
        globalVariable = nextGlobalVariable;
    }
    // */
    ...
    else if (expression->nodeType == HLSLNodeType_FunctionCall)
    {
        ...
        if (sampler_call)
        {
            if (sampler_lod)
            {
                ...
                else if (mode == MODE_DX9)
                {
                    ...
                    else if (identifier->expressionType.baseType == HLSLBaseType_SamplerCube)
                    {
                        ...
                        writer.Write("texCUBElod( %s, float4( ", identifier->name);
                        OutputExpression(expr);
                        writer.Write(".x, ");
                        OutputExpression(expr);
                        writer.Write(".y, ");
            ...
            else if (sampler_grad)
            {
                ...
                if (mode == MODE_DX11)
                {
                    writer.Write("%s_texture.SampleGrad( %s_sampler ", identifier->name, identifier->name);
                    for (HLSLExpression* expr = identifier->nextExpression; expr; expr = expr->nextExpression)
                    {
                        writer.Write(", ");
                        OutputExpression(expr);
                ...
                {
                    writer.Write("tex2Dgrad( %s", identifier->name);
                    for (HLSLExpression* expr = identifier->nextExpression; expr; expr = expr->nextExpression)
                    {
                        writer.Write(", ");
                        OutputExpression(expr);
...
void HLSLGenerator::OutputDeclaration(HLSLDeclaration* declaration)
{
    ...
    if (declaration->annotation && strstr(declaration->annotation, "bigarray"))
    {
        //GFX_COMPLETE - bigarrays in dx are referenced as defines
        return;
    }
