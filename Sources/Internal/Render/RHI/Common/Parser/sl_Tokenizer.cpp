...
{
    bool result = false;
    if ((m_buffer != nullptr) && ((m_buffer + 1) < m_bufferEnd) && (m_buffer[0] == '/'))
    {
        if (m_buffer[1] == '/')
        {
            // Single line comment.
...
bool HLSLTokenizer::ScanNumber()
{
    ...
    {
        m_buffer = fEnd;
        m_token = *(fEnd - 1) == 'h' ? HLSLToken_HalfLiteral : HLSLToken_FloatLiteral;
...
void HLSLTokenizer::GetTokenName(char buffer[s_maxIdentifier]) const
{
    ...
    {
        sprintf(buffer, "%ff", m_fValue);
    }
    else if (m_token == HLSLToken_HalfLiteral)
    {
        sprintf(buffer, "%fh", m_fValue);
    }
