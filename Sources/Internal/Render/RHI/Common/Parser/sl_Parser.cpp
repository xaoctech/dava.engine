...
struct IntrinsicRegister
{
    const char* name;
    HLSLType type;
    explicit IntrinsicRegister(const char* name_, HLSLBaseType type_)
        : name(name_)
        , type(type_)
    {
    }
...
const Intrinsic _intrinsic[] =
{
  ...
  Intrinsic("equal", HLSLBaseType_Bool, HLSLBaseType_Float),
  ...
  Intrinsic("equal", HLSLBaseType_Bool, HLSLBaseType_Float, HLSLBaseType_Float),
  Intrinsic("equal", HLSLBaseType_Int2, HLSLBaseType_Float2, HLSLBaseType_Float2),
  Intrinsic("equal", HLSLBaseType_Int3, HLSLBaseType_Float3, HLSLBaseType_Float3),
  Intrinsic("equal", HLSLBaseType_Int4, HLSLBaseType_Float4, HLSLBaseType_Float4),
  Intrinsic("equal", HLSLBaseType_Bool, HLSLBaseType_Half, HLSLBaseType_Half),
  Intrinsic("equal", HLSLBaseType_Int2, HLSLBaseType_Half2, HLSLBaseType_Half2),
  Intrinsic("equal", HLSLBaseType_Int3, HLSLBaseType_Half3, HLSLBaseType_Half3),
  Intrinsic("equal", HLSLBaseType_Int4, HLSLBaseType_Half4, HLSLBaseType_Half4),
  Intrinsic("equal", HLSLBaseType_Bool, HLSLBaseType_Bool),
  Intrinsic("equal", HLSLBaseType_Bool, HLSLBaseType_Int, HLSLBaseType_Int),
  Intrinsic("equal", HLSLBaseType_Int2, HLSLBaseType_Int2, HLSLBaseType_Int2),
  Intrinsic("equal", HLSLBaseType_Int3, HLSLBaseType_Int3, HLSLBaseType_Int3),
  Intrinsic("equal", HLSLBaseType_Int4, HLSLBaseType_Int4, HLSLBaseType_Int4),
  Intrinsic("equal", HLSLBaseType_Bool, HLSLBaseType_Uint, HLSLBaseType_Uint),
  Intrinsic("equal", HLSLBaseType_Int2, HLSLBaseType_Uint2, HLSLBaseType_Uint2),
  Intrinsic("equal", HLSLBaseType_Int3, HLSLBaseType_Uint3, HLSLBaseType_Uint3),
  Intrinsic("equal", HLSLBaseType_Int4, HLSLBaseType_Uint4, HLSLBaseType_Uint4),
  ...
  Intrinsic("tex2Dcmp", HLSLBaseType_Float4, HLSLBaseType_Sampler2DShadow, HLSLBaseType_Float3),
  ...
  Intrinsic("texCUBElod", HLSLBaseType_Float4, HLSLBaseType_SamplerCube, HLSLBaseType_Float3, HLSLBaseType_Float),
  ...
  Intrinsic("FP_SHADOW", HLSLBaseType_Float, HLSLBaseType_Float3),
  Intrinsic("FP_SHADOW", HLSLBaseType_Float, HLSLBaseType_Float4),
  Intrinsic("FP_SHADOW", HLSLBaseType_Half, HLSLBaseType_Half3),
  Intrinsic("FP_SHADOW", HLSLBaseType_Half, HLSLBaseType_Half4),
...
const int _numIntrinsicsRegisters = sizeof(_intrinsicRegisters) / sizeof(IntrinsicRegister);
...
bool HLSLParser::AcceptFloat(float& value, HLSLToken& token)
{
    token = HLSLToken(m_tokenizer.GetToken());
    if ((token == HLSLToken_FloatLiteral) || (token == HLSLToken_HalfLiteral))
    {
...
bool HLSLParser::ParseTerminalExpression(HLSLExpression*& expression, bool& needsEndParen)
{
    ...
    else
    {
        ...
        float fValue = 0.0f;
        int iValue = 0;
        HLSLToken floatToken = HLSLToken(-1);
        if (AcceptFloat(fValue, floatToken))
        {
            HLSLLiteralExpression* literalExpression = m_tree->AddNode<HLSLLiteralExpression>(fileName, line);
            literalExpression->type = (floatToken == HLSLToken_FloatLiteral) ? HLSLBaseType_Float : HLSLBaseType_Half;
    ...
    while (!done)
    {
        ...
        while (Accept('.'))
        {
            ...
            memberAccess->expressionType = expression->expressionType;
...
bool HLSLParser::ParseStateValue(const EffectState* state, HLSLStateAssignment* stateAssignment)
{
    ...
    else
    {
        ...
        {
            HLSLToken floatToken = HLSLToken(-1);
            if (!AcceptFloat(stateAssignment->fValue, floatToken))
            {
    ...
    if (Accept('='))
    {
        ...
        if (AcceptIdentifier(msk))
        {
            ...
            if (stricmp(msk, "none") == 0)
                mask = COLORMASK_NONE;
            else if ((stricmp(msk, "all") == 0) || (stricmp(msk, "rgba") == 0))
                mask = COLORMASK_ALL;
            else if (stricmp(msk, "rgb") == 0)
                mask = COLORMASK_RGB;
            else if (stricmp(msk, "a") == 0)
                mask = COLORMASK_A;
            else if (stricmp(msk, "r") == 0)
                mask = COLORMASK_R;
            else if (stricmp(msk, "g") == 0)
                mask = COLORMASK_G;
            else if (stricmp(msk, "b") == 0)
                mask = COLORMASK_B;
            else if (stricmp(msk, "rg") == 0)
                mask = COLORMASK_RG;
            else
...
const HLSLType* HLSLParser::FindVariable(const char* name, bool& global) const
{
    ...
    for (int i = 0; i < _numIntrinsicsRegisters; i++)
    {
        if (String_Equal(name, _intrinsicRegisters[i].name))
        {
            global = true;
            return &_intrinsicRegisters[i].type;
        }
    }
    return NULL;
}
