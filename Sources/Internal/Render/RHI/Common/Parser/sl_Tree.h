...
enum HLSLTypeFlags
{
    ...
    HLSLTypeFlag_Reference = 0x200000,
    HLSLTypeFlag_ThreadConts = 0x400000,
};
...
struct HLSLNode
{
    HLSLNodeType nodeType = HLSLNodeType_Root;
...
{
    HLSLStatement* nextStatement = nullptr; // Next statement in the block.
...
struct HLSLAttribute : public HLSLNode
{
    ...
    HLSLAttributeType attributeType = HLSLAttributeType_Custom;
    const char* attrText = nullptr;
    HLSLExpression* argument = nullptr;
    HLSLAttribute* nextAttribute = nullptr;
};
...
enum ColorMask
{
    ...
    COLORMASK_B = (1 << 2),
    COLORMASK_A = (1 << 3),
    COLORMASK_RG = COLORMASK_R | COLORMASK_G, //GFX_COMPLETE - TEMP to make decals work
    COLORMASK_RGB = COLORMASK_R | COLORMASK_G | COLORMASK_B,
...
extern void PruneTree(HLSLTree* tree, const char* entryName);
