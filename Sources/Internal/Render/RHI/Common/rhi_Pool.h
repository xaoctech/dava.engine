...
private:
    struct Entry
    {
        ...
        Entry()
            : allocated(0)
            , generation(0)
            , nextObjectIndex(InvalidObjectIndex)
            , pad(0)
        {
        }
...
inline Handle ResourcePool<T, RT, DT, nr>::Alloc()
{
    ...
    if (Object == nullptr)
    {
        ...
        for (uint32 i = 0; i < ObjectCount; ++i)
            Object[i].nextObjectIndex = i + 1;

        (Object + ObjectCount - 1)->nextObjectIndex = Entry::InvalidObjectIndex;
    }

    if (HeadIndex != Entry::InvalidObjectIndex)
    {
        ...
        ++e->generation;
        handle =
        ((uint32(e - Object) << HANDLE_INDEX_SHIFT) & HANDLE_INDEX_MASK) |
    ...
    {
        RHI_POOL_ASSERT(0, "Next object allocation will fail");
    }
