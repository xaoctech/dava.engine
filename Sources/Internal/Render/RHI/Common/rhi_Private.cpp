...
bool IsInvertedProjectionRequired(bool renderToTexture, bool renderTargetIsCubemap)
{
    ...
    if (renderToTexture)
    {
        bool isUpperLeftOrigin = DeviceCaps().isUpperLeftRTOrigin;
        result = (isUpperLeftOrigin && renderTargetIsCubemap) || (!isUpperLeftOrigin && !renderTargetIsCubemap);
    }
    return result;
}
...
void ReadRegion(Handle tex, void* data, uint32 dataSize, const Rect2i& rect, uint32 level, TextureFace face)
{
    return (*_Impl.impl_Texture_ReadRegion)(tex, data, dataSize, rect, level, face);
}
