...
struct TextureMetal_t : public ResourceImpl<TextureMetal_t, Texture::Descriptor>
{
    TextureMetal_t()
        : mappedData(nullptr)
        , width(0)
...
static MTLPixelFormat MetalTextureFormat(TextureFormat format)
{
    switch (format)
    {
    ...
    default:
        DVASSERT(!"Unsupported texture format");
        return MTLPixelFormatInvalid;
    }
}

static MTLPixelFormat MetalRenderableTextureFormat(TextureFormat format)
{
    switch (format)
    {
    case TEXTURE_FORMAT_R8G8B8A8:
...
static bool _Construct(TextureMetal_t* tex, const Texture::Descriptor& texDesc)
{
    ...
    if (texDesc.isRenderTarget && !(texDesc.cpuAccessRead || texDesc.cpuAccessWrite))
    {
        desc.resourceOptions = MTLResourceStorageModePrivate;
        desc.storageMode = MTLStorageModePrivate;
    }
    else if (texDesc.cpuAccessRead)
    {
    ...
    {
        desc.resourceOptions = MTLResourceCPUCacheModeWriteCombined;
        desc.storageMode = MTLStorageModeShared;
    }

    if (texDesc.memoryless)
    {
...
void metal_Texture_ReadRegion(Handle tex, void* data, uint32 dataSize, const Rect2i& rect, uint32 level, TextureFace face)
{
    TextureMetal_t* self = TextureMetalPool::Get(tex);
    ...
    rgn.origin.y = rect.y;
    rgn.origin.z = 0;
    rgn.size.width = rect.dx;
    rgn.size.height = rect.dy;
    rgn.size.depth = 1;

    DVASSERT(TextureSize(self->format, uint32(rect.dx), uint32(rect.dy), level) == dataSize);

    uint32 stride = TextureStride(self->format, Size2i([self->uid width], [self->uid height]), level);

    [self->uid getBytes:data bytesPerRow:stride fromRegion:rgn mipmapLevel:level];
...
void SetupDispatch(Dispatch* dispatch)
{
    ...
    dispatch->impl_Texture_ReadRegion = &metal_Texture_ReadRegion;
