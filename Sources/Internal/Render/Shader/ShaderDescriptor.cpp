...
namespace ShaderDescriptorDetail
{
...
{
    return ShaderDescriptorDetail::propertyLayoutSet.GetUnique(layout).props;
}
...
ShaderDescriptor::ShaderDescriptor(const Any& assetKey)
    : AssetBase(assetKey)
{
}
...
void ShaderDescriptor::UpdateConfigFromSource(rhi::ShaderSource* vSource, rhi::ShaderSource* fSource)
{
    ...
    for (uint32 i = 0, sz = static_cast<uint32>(constBuffers.size()); i < sz; ++i)
    {
        ...
        constBuffers[i].propertyLayoutId = ShaderDescriptorDetail::propertyLayoutSet.MakeUnique(bufferPropertyLayouts[i]);
    }
