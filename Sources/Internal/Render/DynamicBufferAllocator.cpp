...
uint32 instancePageSize = DEFAULT_PAGE_SIZE;
...
struct PagedBufferAllocator
{
    ...
    BufferAllocateResult AllocateData(uint32 size, uint32 count)
    {
        ...
        if (freeBuffers.empty())
        {
            currentBuffer = new BufferInfo();
            currentBuffer->allocatedSize = instancePageSize;
            currentBuffer->buffer = BufferProxy<HBuffer>::CreateBuffer(pageSize);
        }
        else
        {
        ...
        uint8* currentlyMappedData = BufferProxy<HBuffer>::MapBuffer(currentBuffer->buffer, 0, currentBuffer->allocatedSize);
        currentBuffer->readySync = rhi::GetCurrentFrameSyncObject();
        buffersToUnmap.push_back(currentBuffer);
        usedBuffers.push_back(currentBuffer);

        BufferAllocateResult res;
        res.buffer = currentBuffer->buffer;
        res.data = currentlyMappedData;
        res.count = Min(count, pageSize / size);
...
PagedBufferAllocator<rhi::HVertexBuffer> instanceBufferAllocator;
...
AllocResultInstnceBuffer AllocateInstanceBuffer(uint32 instanceSize, uint32 count)
{
    PagedBufferAllocator<rhi::HVertexBuffer>::BufferAllocateResult result = instanceBufferAllocator.AllocateData(instanceSize, count);
    return AllocResultInstnceBuffer{ result.buffer, result.data, result.count };
}
...
void BeginFrame()
{
    ...
    instanceBufferAllocator.BeginFrame();
}
...
void EndFrame()
{
    ...
    instanceBufferAllocator.EndFrame();
}
...
void SetInstancePageSize(uint32 size)
{
    instancePageSize = size;
    instanceBufferAllocator.Clear();
}

uint32 GetCurrentPageSize()
{
    return pageSize;
}
uint32 GetCurrentInstancePageSize()
{
    return instancePageSize;
}
}
}
