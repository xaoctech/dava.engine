...
bool Texture::LoadImages(eGPUFamily gpu, Vector<Image*>* images)
{
    ...
    if (!IsLoadAvailable(gpu))
    {
        ...
        //GFX_COMPLETE -- kostyl to load hdr textures on devices
        //return false;
        gpu = GPU_ORIGIN;
    }
...
void Texture::FlushDataToRenderer(Vector<Image*>* images)
{
    ...
    autoGeneratedMips = descriptor.autoGenMipmaps;
    levelsCount = descriptor.levelCount;
...
Texture* Texture::CreateFBO(const Texture::FBODescriptor& fboDesc)
{
    ...
    descriptor.levelCount = fboDesc.mipLevelsCount;
    descriptor.cpuAccessRead = fboDesc.needPixelReadback;
    ...
    if (needDepth)
    {
        descriptor.format = rhi::TEXTURE_FORMAT_D16;
...
/* NEED THOSE FUNCTIONS TO SAVE CUBEMAP
void Texture::CreateCubemapImages(Vector<Image *> & images)
{
    ...
    if (isRenderTarget)
    {
        for (uint32 i = 0; i < Texture::CUBE_FACE_COUNT; ++i)
        {
            Image * image = ReadDataToImageFace(i);
            image->mipmapLevel = 0;
            image->cubeFaceID = i;
            images.push_back(image);
        }
...
bool Texture::CreateDescriptor(const FilePath& texturePath, std::unique_ptr<TextureDescriptor>& descriptor, bool descriptorChanged)
{
...
bool Texture::CreateDescriptor(const FilePath& texturePath)
{
    std::unique_ptr<TextureDescriptor> descriptor;
    bool descriptorChanged = false;
    return CreateDescriptor(texturePath, descriptor, descriptorChanged);
}
};
