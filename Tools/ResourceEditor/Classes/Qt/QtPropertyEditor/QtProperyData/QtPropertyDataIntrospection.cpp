...
QVariant QtPropertyDataIntrospection::GetValue()
{
	QVariant v;
	...
	{
		DAVA::Vector2 vec2 = member->Value(object).AsVector2();
		v = QString().sprintf("[%g, %g]", vec2.x, vec2.y);
	}
	else if(member->Type() == DAVA::MetaInfo::Instance<DAVA::Vector3>())
	{
		DAVA::Vector3 vec3 = member->Value(object).AsVector3();
		v = QString().sprintf("[%g, %g, %g]", vec3.x, vec3.y, vec3.z);
	}
	else if(member->Type() == DAVA::MetaInfo::Instance<DAVA::Vector4>())
	{
		DAVA::Vector4 vec4 = member->Value(object).AsVector4();
		v = QString().sprintf("[%g, %g, %g, %g]", vec4.x, vec4.y, vec4.z, vec4.w);
	}

	return v;
}

void QtPropertyDataIntrospection::SetValue(const QVariant &value)
{
	DAVA::VariantType v;
	bool ok = false;
