...
bool FillGeometryInstance(FULogFile& fileOut, FCDGeometryInstance* instance)
{
    ...
    FailIf(e == NULL);
    if (e->GetType() == FCDEntity::CONTROLLER)
...
bool CheckGeometryInstance(FULogFile& fileOut, FCDGeometryInstance* instance)
{
    ...
    for (size_t i = 0; i < 3; ++i)
    {
        ...
        if (b->semantic->empty())
        {
            PassIf(!foundEmpty);
            foundEmpty = true;
            PassIf(b->target->empty());
        }
        else if (IsEquivalent(b->semantic, "Toto"))
        {
            PassIf(!foundToto);
            foundToto = true;
            PassIf(IsEquivalent(b->target, "Targeted"));
        }
        else if (IsEquivalent(b->semantic, "Gallant"))
        {
            PassIf(!foundGallant);
            foundGallant = true;
            PassIf(IsEquivalent(b->target, "Pierto"));
        }
        else
            Fail;
    }
...
bool CheckLayers(FULogFile& fileOut, FCDocument* doc)
{
    ...
    for (size_t i = 0; i < 2; ++i)
    {
        ...
        if (l->objects.empty())
        {
            FailIf(emptyLayer != NULL);
            emptyLayer = l;
        }
        else
        {
            FailIf(fullLayer != NULL);
            fullLayer = l;
        }
    }
