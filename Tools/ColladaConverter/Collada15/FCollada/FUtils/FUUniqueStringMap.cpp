...
void TrickLinkerFUUniqueStringMap()
{
    ...
    FUUniqueStringMap map1;
    FUSUniqueStringMap map2;
    fstring test1(FC("Test1"));
    fm::string test2("Test2");
    map1.insert(test1);
    map2.insert(test2);
    if (map1.contains(test1))
        map1.erase(test1);
    if (map2.contains(test2))
        map2.erase(test2);
}
...
inline void SplitString(const fm::stringT<CH>& str, fm::stringT<CH>& prefix, uint32& suffix)
{
    ...
    if (offset == str.length())
        ...
        suffix = FUStringConversion::ToUInt32(str.c_str() + offset);
}

template <class CH>
void FUUniqueStringMapT<CH>::insert(fm::stringT<CH>& wantedStr)
{
    fm::stringT<CH> prefix;
    uint32 suffix;
...
bool FUUniqueStringMapT<CH>::contains(const fm::stringT<CH>& str) const
{
    ...
    if (itV == values.end())
        ...
        return itV->second.find(suffix) != itV->second.end();
}
