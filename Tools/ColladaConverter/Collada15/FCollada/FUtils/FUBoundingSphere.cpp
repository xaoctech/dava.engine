...
bool FUBoundingSphere::Overlaps(const FUBoundingBox& boundingBox, FMVector3* overlapCenter) const
{
    ...
    {
        float rx, ry, rz;
        if (center.x > boundingBox.GetMax().x)
            rx = boundingBox.GetMax().x - center.x;
        else if (center.x > boundingBox.GetMin().x)
            rx = 0.0f;
        else
            rx = boundingBox.GetMin().x - center.x;
        if (center.y > boundingBox.GetMax().y)
            ry = boundingBox.GetMax().y - center.y;
        else if (center.y > boundingBox.GetMin().y)
            ry = 0.0f;
        else
            ry = boundingBox.GetMin().y - center.y;
        if (center.z > boundingBox.GetMax().z)
            rz = boundingBox.GetMax().z - center.z;
        else if (center.z > boundingBox.GetMin().z)
            rz = 0.0f;
        else
            rz = boundingBox.GetMin().z - center.z;
