...
template <>
fstring FUStringBuilder::ToString() const
{
    return fstring(ToCharPtr());
}

template <>
void FUStringBuilder::append(uint32 i)
{
    fchar sz[128];
...
template <>
void FUStringBuilder::append(uint64 i)
{
    fchar sz[128];
    fsnprintf(sz, 128, FC("%u"), (unsigned int)i);
    append(sz);
...
template <>
void FUStringBuilder::append(int32 i)
{
    fchar sz[128];
...
template <>
void FUSStringBuilder::append(uint32 i)
{
    char sz[128];
...
template <>
void FUSStringBuilder::append(uint64 i)
{
    char sz[128];
    snprintf(sz, 128, "%u", (unsigned int)i);
    append(sz);
...
template <>
void FUSStringBuilder::append(int32 i)
{
    char sz[128];
...
{
    {
        ...
        b5.append(1.0);
        d5.append(-1.0);
        b5.append(FMVector2::Zero);
        d5.append(FMVector2::Zero);
        b5.append(FMVector3::Zero);
        d5.append(FMVector3::Zero);
        b5.append(FMVector4::Zero);
        d5.append(FMVector4::Zero);
        b5.back();
        d5.back();
        b1.appendLine("Test");
        d1.appendLine(FC("Test"));
        b1.appendHex((uint8)4);
        d1.appendHex((uint8)4);
        b1.remove(0);
        d1.remove(0);
        b2.remove(0, 2);
        d2.remove(0, 2);
...

    {
        ...
        x = a.rfind("X", y);
        y = v.rfind(FC("A"), x);
        x = a.find_first_of("XSA", y);
        y = r.find_first_of(FC("DNA"), x);
        x = a.find_last_of("XSA", y);
        y = r.find_last_of(FC("DNA"), x);
        a.insert(x, "XX");
        r.insert(y, FC("XX"));
        a.insert(y, c);
        r.insert(x, v);
        a.erase(0, fm::string::npos);
        t.erase(1, fstring::npos);
        a.append('c');
        v.append('w');
    }
