...
public:
    ...
    inline FMQuaternion& operator=(const float* v)
    {
        x = *v;
        y = *(v + 1);
        z = *(v + 2);
    ...
    inline FMQuaternion Normalize() const
    {
        float l = Length();
        return FMQuaternion(x / l, y / l, z / l, w / l);
    }
    ...
    inline FMQuaternion operator~() const
    {
        return FMQuaternion(-x, -y, -z, w);
    }
    ...
    inline FMQuaternion& operator*=(const FMQuaternion& q)
    {
        return (*this) = (*this) * q;
    }
