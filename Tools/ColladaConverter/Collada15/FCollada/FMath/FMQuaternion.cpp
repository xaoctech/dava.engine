...
static void PatchEuler(float* pval, float* val)
{
    ...
    for (int i = 0; i < 3; ++i) distanceSq += (val[i] - pval[i]) * (val[i] - pval[i]);
    ...
    float alternateDistanceSq = 0.0f;
    for (int i = 0; i < 3; ++i) alternateDistanceSq += (alternative[i] - pval[i]) * (alternative[i] - pval[i]);
