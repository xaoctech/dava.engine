...
public:
    ...
    inline FCDSceneNode* GetJoint(size_t index)
    {
        FUAssert(index < GetJointCount(), return NULL);
        return joints.at(index);
    }
    inline const FCDSceneNode* GetJoint(size_t index) const
    {
        FUAssert(index < GetJointCount(), return NULL);
        return joints.at(index);
    } /**< See above. */
    ...
    inline void RemoveJoint(size_t index)
    {
        joints.erase(index);
    }
    ...
    inline void RemoveJoint(FCDSceneNode* joint)
    {
        joints.erase(joint);
    }
...
private:
    ...
    inline FCDSkinController* FindSkin(FCDEntity* entity)
    {
        return const_cast<FCDSkinController*>(const_cast<const FCDControllerInstance*>(this)->FindSkin(entity));
    }
};
