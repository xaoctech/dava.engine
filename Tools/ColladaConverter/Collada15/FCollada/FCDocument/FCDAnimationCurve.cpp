...
float FindT(float cp0x, float cp1x, float cp2x, float cp3x, float input, float initialGuess)
{
    ...
    float midT = 0.5f;
    if (initialGuess <= 0.1)
        midT = 0.1f; //clamp to 10% or 90%, because if miss, the cost is too high.
    else if (initialGuess >= 0.9)
        midT = 0.9f;
    else
        midT = initialGuess;
    bool once = true;
...
static void ComputeTCBTangent(const FCDAnimationKey* previousKey, const FCDAnimationKey* currentKey, const FCDAnimationKey* nextKey, float tens, float cont, float bias, FMVector2& leftTangent, FMVector2& rightTangent)
{
    ...
    {
        if (nextKey)
            pCurrentMinusPrevious.x = nextKey->input - currentKey->input;
        else
            pCurrentMinusPrevious.x = 0.5f; //Case where there is only one TCB key.. should not happen.
    ...
    {
        if (previousKey)
            pNextMinusCurrent.x = currentKey->input - previousKey->input;
        else
            pNextMinusCurrent.x = 0.5f; //Case where there is only one TCB key.. ?
...
FCDAnimationKey* FCDAnimationCurve::AddKey(FUDaeInterpolation::Interpolation interpolation)
{
    ...
    case FUDaeInterpolation::STEP:
        key = new FCDAnimationKey;
        break;
    case FUDaeInterpolation::LINEAR:
        key = new FCDAnimationKey;
        break;
    case FUDaeInterpolation::BEZIER:
        key = new FCDAnimationKeyBezier;
        break;
    case FUDaeInterpolation::TCB:
        key = new FCDAnimationKeyTCB;
        break;
    default:
        FUFail(key = new FCDAnimationKey; break;);
    }
...
{
    if (clone == NULL)
        clone = new FCDAnimationCurve(const_cast<FCDocument*>(GetDocument()), parent);
...
float FCDAnimationCurve::Evaluate(float input) const
{
    ...
    if (input < inputStart)
    {
        ...
        switch (preInfinity)
        {
        case FUDaeInfinity::CONSTANT:
            return outputStart;
        case FUDaeInfinity::LINEAR:
            return outputStart + inputDifference * (keys[1]->output - outputStart) / (keys[1]->input - inputStart);
    ...
    else if (input >= inputEnd)
    {
        ...
        case FUDaeInfinity::CONSTANT:
            return outputEnd;
        case FUDaeInfinity::LINEAR:
            return outputEnd + inputDifference * (keys[keys.size() - 2]->output - outputEnd) / (keys[keys.size() - 2]->input - inputEnd);
        case FUDaeInfinity::CYCLE:
        {
            float cycleCount = ceilf(inputDifference / inputSpan);
            input -= cycleCount * inputSpan;
    ...
    case FUDaeInterpolation::BEZIER:
    {
        ...
        if (is2DEvaluation)
            t = FindT(bkey1->input, bkey1->outTangent.x, inTangent.x, endKey->input, input, t);
