...
FCDocument::~FCDocument()
{
    ...
    {
        animatedValues.begin()->first->Release();
    }
...
const FCDPhysicsScene* FCDocument::GetPhysicsSceneInstance(size_t index) const
{
    ...
    if (entity != NULL && entity->HasType(FCDPhysicsScene::GetClassType()))
        return (const FCDPhysicsScene*)entity;
    else
        return NULL;
}
...
FCDSceneNode* FCDocument::AddVisualScene()
{
    ...
    if (visualSceneRoot->GetEntity() == NULL)
        visualSceneRoot->SetEntity(visualScene);
    return visualScene;
}
...
void FCDocument::SetCurrentTime(float time)
{
    ...
    for (size_t i = 0; i < externalReferenceManager->GetPlaceHolderCount(); i++)
    {
        ...
        if (placeHolder->IsTargetLoaded())
            placeHolder->GetTarget()->SetCurrentTime(time);
    }
}
