...
private:
    ...
    {
        if (IsEquivalent(v, FMVector3::XAxis))
            return X;
        else if (IsEquivalent(v, FMVector3::YAxis))
            return Y;
        else if (IsEquivalent(v, FMVector3::ZAxis))
            return Z;
        else
            return UNKNOWN;
    }
...
public:
    ...
    inline bool HasConversion()
    {
        return functor != Identity;
    }
    ...
    void operator()(FMVector3& data, bool isScale = false)
    {
        (*functor)(data, (isScale ? 1 : -1));
    }

private:
    void PrepareFunctor()
    {
        if (target == UNKNOWN || current == UNKNOWN || target == current)
            functor = Identity;
        else
        {
            switch (target)
            {
            case X:
                functor = (current == Y) ? XtoY : XtoZ;
                break;
            case Y:
                functor = (current == X) ? YtoX : YtoZ;
                break;
            case Z:
                functor = (current == X) ? ZtoX : ZtoY;
                break;
            case UNKNOWN:
            default:
                FUFail(break);
    ...
    static void XtoY(FMVector3& data, int32 sign)
    {
        float t = sign * data.x;
        data.x = data.y;
        data.y = t;
    }
    static void XtoZ(FMVector3& data, int32 UNUSED(sign))
    {
        float t = data.x;
        data.x = data.y;
        data.y = data.z;
    ...
    static void YtoX(FMVector3& data, int32 sign)
    {
        float t = data.x;
        data.x = sign * data.y;
        data.y = t;
    }
    static void YtoZ(FMVector3& data, int32 sign)
    {
        float t = sign * data.y;
        data.y = data.z;
        data.z = t;
    }
    static void ZtoX(FMVector3& data, int32 UNUSED(sign))
    {
        float t = data.z;
        data.z = data.y;
        data.y = data.x;
    ...
    static void ZtoY(FMVector3& data, int32 sign)
    {
        float t = data.y;
        data.y = sign * data.z;
        data.z = t;
...
public:
    ...
    bool HasConversion()
    {
        return !IsEquivalent(factor, 1.0f);
    }
    void SetCurrent(float current)
    {
        factor = ((current <= 0.0f) ? 1.0f : (current / target));
    }
...
void GetAssetFunctors(FCDEntity* entity, FCDAsset* libraryAsset, FCDConversionUnitFunctor& lengthFunctor, FCDConversionSwapFunctor& upAxisFunctor)
{
    ...
    if (libraryAsset != NULL)
        assets.push_back(libraryAsset);
    bool hasLength = false;
    bool hasAxis = false;
    for (FCDAssetList::iterator it = assets.begin(); it != assets.end(); ++it)
    {
        if (!hasLength && (*it)->GetHasUnitsFlag())
        {
            hasLength = true;
            lengthFunctor.SetCurrent((*it)->GetUnitConversionFactor());
        }
        if (!hasAxis && (*it)->GetHasUpAxisFlag())
        {
            hasAxis = true;
            upAxisFunctor.SetCurrent((*it)->GetUpAxis());
        }
    }
    if (!hasLength)
        lengthFunctor.SetCurrent(-1.0f);
    if (!hasAxis)
        upAxisFunctor.SetCurrent(FMVector3::Origin);
}
...
void ConvertAnimationVector3(FCDAnimated* animated1, FCDAnimated* animated2, FCDAnimated* animated3, float* v1, float* v2, float* v3, FCDocument* document, FCDConversionUnitFunctor& lengthFunctor, FCDConversionSwapFunctor& upAxisFunctor, bool convertLength, bool isScale = false)
{
    ...
    if (indices[0] == ~(size_t)0 && indices[1] == ~(size_t)0 && indices[2] == ~(size_t)0)
        return; // no animations on the vector
...
void ConvertAnimationFloat(FCDAnimated* animated, float& f, FCDocument* document, FCDConversionUnitFunctor& lengthFunctor, FCDConversionSwapFunctor& upAxisFunctor)
{
    if (animated != NULL)
    {
        ...
        if (index == ~(size_t)0)
            return; // no animations on the float

        FCDAnimationCurveListList& curvesListList = animated->GetCurves();
        if (index >= curvesListList.size())
            return; // no animations on the float
...
void StandardizeUpAxisAndLength(FCDocument* document, const FMVector3& upAxis, float unitInMeters, bool handleTargets)
{
    ...
    {
        if (baseAsset->GetHasUnitsFlag())
            unitInMeters = baseAsset->GetUnitConversionFactor();
        else
            unitInMeters = 1.0f; // COLLADA specification default.
    }
    FMVector3 _upAxis = upAxis;
    if (!IsEquivalent(upAxis, FMVector3::Origin))
        _upAxis = upAxis;
    else
    {
        if (baseAsset->GetHasUpAxisFlag())
            _upAxis = baseAsset->GetUpAxis();
        else
            _upAxis = FMVector3::YAxis; // COLLADA specification default.
    }
    ...
    {
        FCDocument* document = documentQueue.back();
        documentQueue.pop_back();
        ...
        while (!visualSceneNodeIt.IsDone())
        {
            ...
            if (lengthFunctor.HasConversion() || upAxisFunctor.HasConversion())
            {
                ...
                for (size_t t = 0; t < transformCount; ++t)
                {
                    ...
                    if (transform->HasType(FCDTTranslation::GetClassType()))
                        CONVERT_VECT3L(((FCDTTranslation*)transform)->GetTranslation(), ((FCDTTranslation*)transform)->GetTranslation())
                    else if (transform->HasType(FCDTRotation::GetClassType()))
                        CONVERT_VECT3(((FCDTRotation*)transform)->GetAngleAxis(), ((FCDTRotation*)transform)->GetAxis())
                    else if (transform->HasType(FCDTScale::GetClassType()))
                        CONVERT_SCALE(((FCDTScale*)transform)->GetScale(), ((FCDTScale*)transform)->GetScale())
                    else if (transform->HasType(FCDTSkew::GetClassType()))
                    {
                        CONVERT_VECT3(((FCDTSkew*)transform)->GetSkew(), ((FCDTSkew*)transform)->GetRotateAxis());
                        CONVERT_VECT3(((FCDTSkew*)transform)->GetSkew(), ((FCDTSkew*)transform)->GetAroundAxis());
                    }
                    else if (transform->HasType(FCDTLookAt::GetClassType()))
                    {
                        CONVERT_VECT3L(((FCDTLookAt*)transform)->GetLookAt(), ((FCDTLookAt*)transform)->GetPosition());
                        CONVERT_VECT3L(((FCDTLookAt*)transform)->GetLookAt(), ((FCDTLookAt*)transform)->GetTarget());
                        CONVERT_VECT3(((FCDTLookAt*)transform)->GetLookAt(), ((FCDTLookAt*)transform)->GetUp());
                    }
                    else if (transform->HasType(FCDTMatrix::GetClassType()))
                    {
                        CONVERT_MAT44(((FCDTMatrix*)transform)->GetTransform(), ((FCDTMatrix*)transform)->GetTransform());
                    }
                }
            }
