...
public:
    ...
    inline size_t GetPairCount() const
    {
        return pairs.size();
    }
    ...
    inline FCDJointWeightPair* GetPair(size_t index)
    {
        FUAssert(index < pairs.size(), return NULL);
        return &(pairs.at(index));
    }
    inline const FCDJointWeightPair* GetPair(size_t index) const
    {
        FUAssert(index < pairs.size(), return NULL);
        return &(pairs.at(index));
    } /**< See above. */
...
public:
    ...
    inline const fm::string& GetId() const
    {
        return id;
    }
    ...
    inline const FMMatrix44& GetBindPoseInverse() const
    {
    ...
    inline void SetBindPoseInverse(const FMMatrix44& inverseBindPose)
    {
        bindPoseInverse = inverseBindPose;
    }
};
...
public:
    ...
    inline FCDController* GetParent()
    {
        return parent;
    }
    inline const FCDController* GetParent() const
    {
        return parent;
    } /**< See above. */
    ...
    inline FCDSkinControllerJoint* GetJoints()
    {
        return !joints.empty() ? &(joints.front()) : NULL;
    }
    inline const FCDSkinControllerJoint* GetJoints() const
    {
        return !joints.empty() ? &(joints.front()) : NULL;
    } /**< See above. */
    ...
    inline FCDSkinControllerJoint* GetJoint(size_t index)
    {
        FUAssert(index < joints.size(), return NULL);
        return &joints.at(index);
    }
    inline const FCDSkinControllerJoint* GetJoint(size_t index) const
    {
        FUAssert(index < joints.size(), return NULL);
        return &joints.at(index);
    } /**< See above. */
    ...
    inline size_t GetInfluenceCount() const
    {
    ...
    inline FCDSkinControllerVertex* GetVertexInfluences()
    {
        return influences.size() > 0 ? &(influences.front()) : NULL;
    }
    inline const FCDSkinControllerVertex* GetVertexInfluences() const
    {
        return influences.size() > 0 ? &(influences.front()) : NULL;
    } /**< See above. */
    ...
    inline FCDSkinControllerVertex* GetVertexInfluence(size_t index)
    {
        FUAssert(index < influences.size(), return NULL);
        return &influences.at(index);
    }
    inline const FCDSkinControllerVertex* GetVertexInfluence(size_t index) const
    {
        FUAssert(index < influences.size(), return NULL);
        return &influences.at(index);
    } /**< See above. */
