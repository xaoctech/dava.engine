...
public:
    ...
    inline size_t GetParentCount() const
    {
        return parents.size();
    };
    ...
    inline FCDSceneNode* GetParent(size_t index = 0)
    {
        FUAssert(index == 0 || index < parents.size(), return NULL);
        return (!parents.empty()) ? parents.at(index) : NULL;
    }
    inline const FCDSceneNode* GetParent(size_t index = 0) const
    {
        FUAssert(index == 0 || index < parents.size(), return NULL);
        return (!parents.empty()) ? parents.at(index) : NULL;
    } /**< See above. */
    ...
    inline const FCDSceneNode** GetParents() const
    {
    ...
    inline FCDSceneNode* GetChild(size_t index)
    {
        FUAssert(index < children.size(), return NULL);
        return children.at(index);
    }
    inline const FCDSceneNode* GetChild(size_t index) const
    {
        FUAssert(index < children.size(), return NULL);
        return children.at(index);
    } /**< See above. */
    ...
    inline const FCDSceneNode** GetChildren() const
    {
    ...
    inline FCDEntityInstance* GetInstance(size_t index)
    {
        FUAssert(index < instances.size(), return NULL);
        return instances.at(index);
    }
    inline const FCDEntityInstance* GetInstance(size_t index) const
    {
        FUAssert(index < instances.size(), return NULL);
        return instances.at(index);
    } /**< See above. */
    ...
    inline const FCDEntityInstance** GetInstances() const
    {
    ...
    inline size_t GetTransformCount() const
    {
    ...
    inline FCDTransform* GetTransform(size_t index)
    {
        FUAssert(index < transforms.size(), return NULL);
        return transforms.at(index);
    }
    inline const FCDTransform* GetTransform(size_t index) const
    {
        FUAssert(index < transforms.size(), return NULL);
        return transforms.at(index);
    } /**< See above. */
    ...
    inline const FCDTransform** GetTransforms() const
    {
    ...
    inline void GetHierarchicalAssets(FCDAssetList& assets)
    {
        GetHierarchicalAssets(*(FCDAssetConstList*)&assets);
    }
    ...
    {
        return const_cast<FCDEntity*>(const_cast<const FCDSceneNode*>(this)->FindDaeId(daeId));
    }
    ...
    inline const fm::string& GetSubId() const
    {
        return daeSubId;
    }
    ...
    inline FCDEntity* FindSubId(const fm::string& subId)
    {
        return const_cast<FCDEntity*>(const_cast<const FCDSceneNode*>(this)->FindSubId(subId));
    }
    ...
    inline FCDParameterAnimatableFloat& GetVisibility()
    {
        return visibility;
    }
    inline const FCDParameterAnimatableFloat& GetVisibility() const
    {
        return visibility;
    } /**< See above. */
    ...
    {
        visibility = isVisible ? 1.0f : 0.0f;
    }
    ...
    /** Retrieves the local transform for this visual scene node.
		...
		@return The local transform. */
    inline FMMatrix44 CalculateLocalTransform() const
    {
        return ToMatrix();
    }
    ...
    inline void IncrementTargetCount()
    {
        ++targetCount;
    }
    ...
    /** [INTERNAL] Decrements the number of entities target this node.
		To set targets, use the FCDTargetedEntity::SetTarget function. */
    inline void DecrementTargetCount()
    {
        if (targetCount > 0)
            --targetCount;
    }
