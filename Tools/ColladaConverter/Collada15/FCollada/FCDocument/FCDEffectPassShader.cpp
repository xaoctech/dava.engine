...
const FCDEffectPassBind* FCDEffectPassShader::FindBindingReference(const char* reference) const
{
    ...
    {
        if (IsEquivalent((*it)->reference, reference))
            return (*it);
    }
    return NULL;
}
...
const FCDEffectPassBind* FCDEffectPassShader::FindBindingSymbol(const char* symbol) const
{
    for (const FCDEffectPassBind** it = bindings.begin(); it != bindings.end(); ++it)
    {
        if (IsEquivalent((*it)->symbol, symbol))
            return (*it);
    }
    return NULL;
}
...
{
    if (clone == NULL)
        clone = new FCDEffectPassShader(const_cast<FCDocument*>(GetDocument()), parent);
    ...
    if (code != NULL)
    {
        ...
        if (clone->code == NULL)
            clone->code = clone->parent->GetParent()->GetParent()->FindCode(code->GetSubId());
    }
    return clone;
}
