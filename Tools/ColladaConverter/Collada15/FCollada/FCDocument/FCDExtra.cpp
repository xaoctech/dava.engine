...
namespace FCollada
{
extern FColladaPluginManager* pluginManager;
}
...
const FCDEType* FCDExtra::FindType(const char* name) const
{
    ...
    {
        if (IsEquivalent((*itT)->GetName(), name))
            return *itT;
    }
    return NULL;
}
...
{
    if (clone == NULL)
        clone = new FCDExtra(const_cast<FCDocument*>(GetDocument()), NULL);
...
const FCDETechnique* FCDEType::FindTechnique(const char* profile) const
{
    ...
    {
        if (IsEquivalent((*itT)->GetProfile(), profile))
            return *itT;
    }
    return NULL;
}
...
const FCDENode* FCDENode::FindChildNode(const char* name) const
{
    ...
    {
        if (IsEquivalent((*itN)->GetName(), name))
            return (*itN);
    }
    return NULL;
}
...
void FCDENode::FindChildrenNodes(const char* name, FCDENodeList& nodes) const
{
    ...
    {
        if (IsEquivalent((*itN)->GetName(), name))
            nodes.push_back(const_cast<FCDENode*>(*itN));
    }
}
...
const FCDEAttribute* FCDENode::FindAttribute(const char* name) const
{
    ...
    {
        if (IsEquivalent((*itA)->GetName(), name))
            return (*itA);
    }
    return NULL;
}
