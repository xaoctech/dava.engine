...
xmlNode* AddParameter(xmlNode* parent, const char* name, const char* type)
{
    ...
    if (name != NULL && *name != 0)
        AddAttribute(parameterNode, DAE_NAME_ATTRIBUTE, name);
    if (type == NULL)
        type = DAE_FLOAT_TYPE;
...
{
    if (sourceId == NULL || *sourceId == 0 || semantic == NULL || *semantic == 0)
        return NULL;
    ...
    if (offset >= 0)
        AddAttribute(inputNode, DAE_OFFSET_ATTRIBUTE, offset);
    if (set >= 0)
        AddAttribute(inputNode, DAE_SET_ATTRIBUTE, set);
    return inputNode;
}
...
xmlNode* AddSourceMatrix(xmlNode* parent, const char* id, const FMMatrix44List& values)
{
    ...
    FUSStringBuilder arrayId(id);
    arrayId.append("-array");
...
fm::string AddNodeSid(xmlNode* node, const char* wantedSid)
{
    ...
    {
        if (HasNodeProperty(parentNode, DAE_ID_ATTRIBUTE) || HasNodeProperty(parentNode, DAE_SID_ATTRIBUTE))
            break;
    }
    if (parentNode == node)
    {
        if (!HasNodeProperty(parentNode, DAE_SID_ATTRIBUTE))
    ...
    {
        FUSStringBuilder builder(wantedSid);
