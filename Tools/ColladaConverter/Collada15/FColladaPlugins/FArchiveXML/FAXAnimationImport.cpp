...
bool FArchiveXML::LoadAnimationChannel(FCDObject* object, xmlNode* channelNode)
{
    ...
    for (size_t i = 0; i < samplerInputNodes.size(); ++i) // Don't use iterator here because we are possibly appending source nodes in the loop
    {
        ...
        if (sourceSemantic == DAE_INPUT_ANIMATION_INPUT)
            inputSource = sourceNode;
        else if (sourceSemantic == DAE_OUTPUT_ANIMATION_INPUT)
            outputSource = sourceNode;
        else if (sourceSemantic == DAE_INTANGENT_ANIMATION_INPUT)
            inTangentSource = sourceNode;
        else if (sourceSemantic == DAE_OUTTANGENT_ANIMATION_INPUT)
            outTangentSource = sourceNode;
        else if (sourceSemantic == DAEFC_TCB_ANIMATION_INPUT)
            tcbSource = sourceNode;
        else if (sourceSemantic == DAEFC_EASE_INOUT_ANIMATION_INPUT)
            easeSource = sourceNode;
        else if (sourceSemantic == DAE_INTERPOLATION_ANIMATION_INPUT)
            interpolationSource = sourceNode;
        else if (sourceSemantic == DAEMAYA_DRIVER_INPUT)
            inputDriver = sourceId;
    }
    ...
    if (keyCount == 0)
        return true; // Valid although very boring channel.

    UInt32List interpolations;
    interpolations.reserve(keyCount);
    ...
    xmlNodeList mayaParameterNodes;
    StringList mayaParameterNames;
    ...
    if (!inputDriver.empty())
    {
        ...
        if (driverTarget != NULL)
        {
            ...
            if (data.driverQualifier < 0)
                data.driverQualifier = 0;
        }
    }
...
{
    if (animatable == NULL || node == NULL)
        return;
    FCDAnimated* animated = animatable->GetAnimated();
    if (!FArchiveXML::LinkAnimated(animated, node))
    ...
    for (FCDAnimationChannelList::iterator it = channels.begin(); it != channels.end(); ++it)
    {
        ...
        if (!qualifier.empty())
        {
            ...
            if (animated->GetArrayElement() != -1)
            {
                ...
                if (animated->GetArrayElement() != element)
                    continue;
                else
                {
            ...
            else
            {
                ...
                {
                    if (qualifiers[index] == qualifier)
                        break;
                }

                // Check for bracket notation eg -(X)- instead
                if (index == qualifiers.size())
                    index = FUStringConversion::ParseQualifier(qualifier);
                if (index < qualifiers.size())
                {
                ...
                else
                {
                    ...
                    {
                        if (IsEquivalent(qualifier, FCDAnimatedStandardQualifiers::XYZW[index]))
                            break;
                        else if (IsEquivalent(qualifier, FCDAnimatedStandardQualifiers::RGBA[index]))
                            break;
                    }
                    if (index < checkCount)
                    {
