    ...
    else
    {
        ...
        for (size_t c = 0; c < curveCount; ++c)
        {
            ...
            if (curCurve != NULL)
            {
                ...
                {
                    curveId.append('_');
                    curveId.append(curveData.targetElement);
                    curveId.append('_');
                }
        ...
        for (size_t c = 0; c < curveCount; ++c)
        {
            if (animationChannel->GetCurve(c) != NULL)
                FArchiveXML::WriteSamplerFCDAnimationCurve(animationChannel->GetCurve(c), parentNode, ids[c]);
        }
        for (size_t c = 0; c < curveCount; ++c)
        {
            if (animationChannel->GetCurve(c) != NULL)
                FArchiveXML::WriteChannelFCDAnimationCurve(animationChannel->GetCurve(c), parentNode, ids[c], data.targetPointer.c_str());
        }
    }
...
void FArchiveXML::WriteSourceFCDAnimationCurve(FCDAnimationCurve* animationCurve, xmlNode* parentNode, const fm::string& baseId)
{
    ...
    FloatList outputs;
    outputs.reserve(keyCount);
    UInt32List interpolations;
    interpolations.reserve(keyCount);
    FMVector2List inTangents;
    if (hasTangents)
        inTangents.reserve(keyCount);
    FMVector2List outTangents;
    if (hasTangents)
        outTangents.reserve(keyCount);
    FMVector3List tcbs;
    if (hasTCB)
        tcbs.reserve(keyCount);
    FMVector2List eases;
    if (hasTCB)
        eases.reserve(keyCount);
...
xmlNode* FArchiveXML::WriteSamplerFCDAnimationCurve(FCDAnimationCurve* animationCurve, xmlNode* parentNode, const fm::string& baseId)
{
    ...
    if (animationCurve->HasDriver())
    {
        ...
        {
            builder.append('(');
            builder.append(driverElement);
            builder.append(')');
        }
        if (animationCurve->GetDriverIndex() == 0)
        {
            builder.append('(');
            builder.append(animationCurve->GetDriverIndex());
            builder.append(')');
        }
...
xmlNode* FArchiveXML::WriteChannelFCDAnimationCurve(FCDAnimationCurve* animationCurve, xmlNode* parentNode, const fm::string& baseId, const char* targetPointer)
{
    ...
    {
        builder.append('(');
        builder.append(data.targetElement);
        builder.append(')');
    }
...
{
    if (animationMultiCurve->GetDimension() == 0)
        return;
    ...
    inputs.reserve(keyCount);
    FloatList outputs;
    outputs.reserve(keyCount * animationMultiCurve->GetDimension());
    UInt32List interpolations;
    interpolations.reserve(keyCount);
    FloatList inTangents;
    if (hasTangents)
        inTangents.reserve(keyCount * animationMultiCurve->GetDimension() * 2);
    FloatList outTangents;
    if (hasTangents)
        outTangents.reserve(keyCount * animationMultiCurve->GetDimension() * 2);
    FloatList tcbs;
    if (hasTCB)
        tcbs.reserve(keyCount * animationMultiCurve->GetDimension() * 3);
    FloatList eases;
    if (hasTCB)
        eases.reserve(keyCount * animationMultiCurve->GetDimension() * 2);
...
xmlNode* FArchiveXML::WriteChannelFCDAnimationMultiCurve(FCDAnimationMultiCurve* animationMultiCurve, xmlNode* parentNode, const fm::string& baseId, const fm::string& pointer)
{
    ...
    {
        builder.append('(');
        builder.append(animationMultiCurve->GetTargetElement());
        builder.append(')');
    }
    AddAttribute(channelNode, DAE_TARGET_ATTRIBUTE, builder);
    return channelNode;
