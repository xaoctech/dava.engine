...
bool FArchiveXML::LoadGeometryMesh(FCDObject* object, xmlNode* meshNode)
{
    ...
    for (xmlNode* childNode = meshNode->children; childNode != NULL; childNode = childNode->next)
    {
        ...
        if (childNode->type != XML_ELEMENT_NODE)
            continue;
        else if (IsEquivalent(childNode->name, DAE_SOURCE_ELEMENT))
            continue;
        else if (IsEquivalent(childNode->name, DAE_VERTICES_ELEMENT))
            continue;
        else if (IsEquivalent(childNode->name, DAE_POLYGONS_ELEMENT)
                 || IsEquivalent(childNode->name, DAE_TRIANGLES_ELEMENT)
                 || IsEquivalent(childNode->name, DAE_POLYLIST_ELEMENT))
            primType = FCDGeometryPolygons::POLYGONS;
        else if (IsEquivalent(childNode->name, DAE_LINES_ELEMENT))
            primType = FCDGeometryPolygons::LINES;
        else if (IsEquivalent(childNode->name, DAE_LINESTRIPS_ELEMENT))
            primType = FCDGeometryPolygons::LINE_STRIPS;
        else if (IsEquivalent(childNode->name, DAE_TRIFANS_ELEMENT))
            primType = FCDGeometryPolygons::TRIANGLE_FANS;
        else if (IsEquivalent(childNode->name, DAE_TRISTRIPS_ELEMENT))
            primType = FCDGeometryPolygons::TRIANGLE_STRIPS;
        else if (IsEquivalent(childNode->name, DAE_POINTS_ELEMENT))
            primType = FCDGeometryPolygons::POINTS;
        else
            continue;
    ...
    if (verticesExtraNode != NULL)
    {
        ...
        for (size_t i = 0; i < extraInputNodeCount; ++i)
        {
            ...
            if (source == NULL || !geometryMesh->IsVertexSource(source))
                continue;
...
bool FArchiveXML::LoadGeometryPolygons(FCDObject* object, xmlNode* baseNode)
{
    ...
    bool isPolygons = false, isTriangles = false, isPolylist = false;
    if (geometryPolygons->GetPrimitiveType() == FCDGeometryPolygons::POLYGONS)
    {
        if (IsEquivalent(baseNode->name, DAE_POLYGONS_ELEMENT))
        {
            expectedVertexCount = 4;
            isPolygons = true;
        }
        else if (IsEquivalent(baseNode->name, DAE_TRIANGLES_ELEMENT))
        {
            expectedVertexCount = 3 * expectedFaceCount;
            isTriangles = true;
        }
        else
        {
            FUAssert(IsEquivalent(baseNode->name, DAE_POLYLIST_ELEMENT), return false);
    ...
    for (itNode = baseNode->children; itNode != NULL; itNode = itNode->next)
    {
        ...
        if (IsEquivalent(itNode->name, DAE_INPUT_ELEMENT))
        {
            ...
            if (offset >= idxOwners.size())
                idxOwners.resize(offset + 1);
            ...
            if (semantic == FUDaeGeometryInput::UNKNOWN)
            ...
            else if (semantic == FUDaeGeometryInput::VERTEX)
            {
                ...
                for (uint32 i = 0; i < vertexSourceCount; ++i)
                {
                    ...
                    if (vertexInput == NULL)
                        vertexInput = geometryPolygons->AddInput(vertexSource, offset);
                    if (idxOwners[offset] == NULL)
                        idxOwners[offset] = vertexInput;
    ...
    if (!noTessellation)
    {
        ...
        else if (isPolylist)
        {
            ...
            if (vCountDataString != NULL)
                FUStringConversion::ToUInt32List(vCountDataString, vCountData);
    ...
    for (; itNode != NULL; itNode = itNode->next)
    {
        ...
        if (IsEquivalent(itNode->name, DAE_POLYGON_ELEMENT) || IsEquivalent(itNode->name, DAE_POLYGONHOLED_ELEMENT))
        {
            ...
            if (isTriangles)
                for (uint32 i = 0; i < localFaceVertexCount / 3; ++i) geometryPolygons->AddFaceVertexCount(3);
            else if (isPolygons)
                geometryPolygons->AddFaceVertexCount(localFaceVertexCount);
        ...
        else if (IsEquivalent(itNode->name, DAE_VERTEXCOUNT_ELEMENT))
        {
        } // Don't whine at this one.
        else
        {
            FUError::Error(FUError::WARNING_LEVEL, FUError::WARNING_UNKNOWN_POLYGON_CHILD, itNode->line);
        }
...
bool FArchiveXML::LoadGeometrySpline(FCDObject* object, xmlNode* splineNode)
{
    ...
    {
        // is it a spline?
        if (!IsEquivalent(splineNode->name, DAE_SPLINE_ELEMENT))
            continue;
...
bool FArchiveXML::LoadSpline(FCDObject* object, xmlNode* splineNode)
{
    ...
    for (size_t i = 0; i < inputElements.size(); i++)
    {
        ...
        if (sourceId.empty())
        {
            FUError::Error(FUError::ERROR_LEVEL, FUError::ERROR_DEFAULT_ERROR);
            return false;
        }
...
{
    if (!FArchiveXML::LoadSpline(object, splineNode))
        return false;
...
void FArchiveXML::SetTypeFCDGeometrySource(FCDGeometrySource* geometrySource, FUDaeGeometryInput::Semantic type)
{
    ...
    if (geometrySource->GetType() != FUDaeGeometryInput::POSITION && geometrySource->GetType() != FUDaeGeometryInput::COLOR)
        return;
