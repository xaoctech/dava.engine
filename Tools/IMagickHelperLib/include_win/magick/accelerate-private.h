...
inline CLQuantum getBlue(CLPixelType p) { return p.x; } inline void setBlue(CLPixelType* p, CLQuantum value) { (*p).x = value; } inline float getBlueF4(float4 p) { return p.x; } inline void setBlueF4(float4* p, float value) { (*p).x = value; }

inline CLQuantum getGreen(CLPixelType p) { return p.y; } inline void setGreen(CLPixelType* p, CLQuantum value) { (*p).y = value; } inline float getGreenF4(float4 p) { return p.y; } inline void setGreenF4(float4* p, float value) { (*p).y = value; }

inline CLQuantum getRed(CLPixelType p) { return p.z; } inline void setRed(CLPixelType* p, CLQuantum value) { (*p).z = value; } inline float getRedF4(float4 p) { return p.z; } inline void setRedF4(float4* p, float value) { (*p).z = value; }

inline CLQuantum getOpacity(CLPixelType p) { return p.w; } inline void setOpacity(CLPixelType* p, CLQuantum value) { (*p).w = value; } inline float getOpacityF4(float4 p) { return p.w; } inline void setOpacityF4(float4* p, float value) { (*p).w = value; }
...
uint2 MWC_SeedImpl_Mod64(ulong A, ulong M, uint vecSize, uint vecOffset, ulong streamBase, ulong streamGap) {
    ...
    enum
    {
        MWC_BASEID = 4077358422479273989UL
    };
...
float mwcGenerateDifferentialNoise(mwc64x_state_t* r, CLQuantum pixel, NoiseType noise_type, float attenuate) {
    ...
    case ImpulseNoise:
    {
        if (alpha < (SigmaImpulse / 2.0f))
            noise = 0.0f;
        else if (alpha >= (1.0f - (SigmaImpulse / 2.0f)))
    ...
    case MultiplicativeGaussianNoise:
    {
        ...
        beta = mwcReadPseudoRandomValue(r);
        noise = (float)(pixel + pixel * SigmaMultiplicativeGaussian * sigma * cospi((float)(2.0f * beta)) / 2.0f);
        break;
    }
    case PoissonNoise:
    {
...
__kernel void AddNoise(const __global CLPixelType* inputImage, __global CLPixelType* filteredImage, const unsigned int inputPixelCount, const unsigned int pixelsPerWorkItem, const ChannelType channel, const NoiseType noise_type, const float attenuate, const unsigned int seed0, const unsigned int seed1, const unsigned int numRandomNumbersPerPixel) {
...
inline void MagickPixelCompositeBlend(const float4* p,
                                      const float alpha, const float4* q,
                                      const float beta, float4* composite) {
    MagickPixelCompositePlus(p, (float)(QuantumRange - alpha * (QuantumRange - getOpacityF4(*p))), q, (float)(QuantumRange - beta * (QuantumRange - getOpacityF4(*q))), composite);
})

STRINGIFY(
__kernel void Composite(__global CLPixelType* image,
    ...
    const CLPixelType compositePixel = compositeImage[index.y * imageWidth + index.x];
    float4 source;
...
STRINGIFY(
__kernel void ConvolveOptimized(const __global CLPixelType* input, __global CLPixelType* output,
    ...
    if (imageIndex.x >= imageWidth || imageIndex.y >= imageHeight)
    {
        return;
    }
...
STRINGIFY(
__kernel void Convolve(const __global CLPixelType* input, __global CLPixelType* output,
...
__kernel void HullPass1(const __global CLPixelType* inputImage, __global CLPixelType* outputImage, const unsigned int imageWidth, const unsigned int imageHeight, const int2 offset, const int polarity, const int matte) {
    ...
    CLPixelType r = (clampedNeighbor.x == neighbor.x && clampedNeighbor.y == neighbor.y) ? inputImage[clampedNeighbor.y * imageWidth + clampedNeighbor.x] : (CLPixelType)0;
...
__kernel void HullPass2(const __global CLPixelType* inputImage, __global CLPixelType* outputImage, const unsigned int imageWidth, const unsigned int imageHeight, const int2 offset, const int polarity, const int matte) {
    ...
    CLPixelType s = (clampedNeighbor.x == neighbor.x && clampedNeighbor.y == neighbor.y) ? inputImage[clampedNeighbor.y * imageWidth + clampedNeighbor.x] : (CLPixelType)0;
...
CLPixelType ApplyFunction(CLPixelType pixel, const MagickFunction function,
    ...
    case SinusoidFunction:
    {
        ...
        result.x = QuantumRange * (ampl * sin(2.0f * MagickPI *
                                              (freq * QuantumScale * (float)pixel.x + phase / 360.0f)) +
                                   bias);
        result.y = QuantumRange * (ampl * sin(2.0f * MagickPI *
                                              (freq * QuantumScale * (float)pixel.y + phase / 360.0f)) +
                                   bias);
        result.z = QuantumRange * (ampl * sin(2.0f * MagickPI *
                                              (freq * QuantumScale * (float)pixel.z + phase / 360.0f)) +
                                   bias);
        result.w = QuantumRange * (ampl * sin(2.0f * MagickPI *
                                              (freq * QuantumScale * (float)pixel.w + phase / 360.0f)) +
                                   bias);
        break;
    }
    case ArcsinFunction:
...
inline void ConvertRGBToHSL(const CLQuantum red, const CLQuantum green, const CLQuantum blue,
    ...
    *lightness = (tmax + tmin) / 2.0;
    ...
    else if (tmax == (QuantumScale * green))
...
STRINGIFY(
__kernel void MotionBlur(const __global CLPixelType* input, __global CLPixelType* output,
                         const unsigned int imageWidth, const unsigned int imageHeight,
    ...
    if (currentPixel.x >= imageWidth || currentPixel.y >= imageHeight)
        return;
...
__kernel void RadialBlur(const __global CLPixelType* im, __global CLPixelType* filtered_im,
    ...
    if (((channel & OpacityChannel) == 0) || (matte == 0))
    {
        ...
        {
            result += convert_float4(im[ClampToCanvas(blurCenter.x + center_x * cos_theta[i] - center_y * sin_theta[i] + 0.5f, columns) +
    ...
    else
    {
        ...
        for (unsigned int i = 0; i < cossin_theta_size; i += step)
        {
            float4 p = convert_float4(im[ClampToCanvas(blurCenter.x + center_x * cos_theta[i] - center_y * sin_theta[i] + 0.5f, columns) +
                                         ClampToCanvas(blurCenter.y + center_x * sin_theta[i] + center_y * cos_theta[i] + 0.5f, rows) * columns]);
...
__kernel void RandomNumberGenerator(__global uint* seeds, const float normalizeRand, __global float* randomNumbers, const uint init, const uint numRandomNumbers) {
    ...
    for (unsigned int i = 0; i < numRandomNumbersPerItem; i++)
    {
        ...
        unsigned int pos = (get_group_id(0) * get_local_size(0) * numRandomNumbersPerItem) + get_local_size(0) * i + get_local_id(0);
...
float CubicBC(const float x, const __global float* resizeFilterCoefficients) {
    ...
    */
    if (x < 1.0)
        return (resizeFilterCoefficients[0] + x * (x * (resizeFilterCoefficients[1] + x * resizeFilterCoefficients[2])));
    if (x < 2.0)
        return (resizeFilterCoefficients[3] + x * (resizeFilterCoefficients[4] + x * (resizeFilterCoefficients[5] + x * resizeFilterCoefficients[6])));
    return (0.0);
})
...
STRINGIFY(
inline float getResizeFilterWeight(const __global float* resizeFilterCubicCoefficients, const ResizeWeightingFunctionType resizeFilterType, const ResizeWeightingFunctionType resizeWindowType, const float resizeFilterScale, const float resizeWindowSupport, const float resizeFilterBlur, const float x) {
    float scale;
    float xBlur = fabs(x / resizeFilterBlur);
    if (resizeWindowSupport < MagickEpsilon || resizeWindowType == BoxWeightingFunction)
    {
        scale = 1.0f;
    }
    else
    ...
    for (unsigned int chunk = 0; chunk < totalNumChunks; chunk++)
    {
            ...
            if (startStep < n)
            {
                ...
                if (matte == 0)
                {
                    for (unsigned int i = startStep; i < stopStep; i++, cacheIndex++)
                    {
                        ...
                        float weight = getResizeFilterWeight(resizeFilterCubicCoefficients, (ResizeWeightingFunctionType)resizeFilterType, (ResizeWeightingFunctionType)resizeWindowType, resizeFilterScale, resizeFilterWindowSupport, resizeFilterBlur, scale * (start + i - bisect + 0.5));
        ...
        if (itemID < actualNumPixelInThisChunk)
        {
            if (matte == 0)
            {
                ...
                filteredImage[y * filteredColumns + chunkStartX + itemID] = (CLPixelType)(ClampToQuantum(filteredPixel.x), ClampToQuantum(filteredPixel.y), ClampToQuantum(filteredPixel.z), ClampToQuantum(filteredPixel.w));
            }
            else
            {
                float density = densityCache[itemID];
                ...
                CLPixelType fp;
                fp = (CLPixelType)(ClampToQuantum(gamma * filteredPixel.x), ClampToQuantum(gamma * filteredPixel.y), ClampToQuantum(gamma * filteredPixel.z), ClampToQuantum(filteredPixel.w));
    ...
    for (unsigned int chunk = 0; chunk < totalNumChunks; chunk++)
    {
        ...
        if (itemID < actualNumPixelInThisChunk)
        {
            if (matte == 0)
            {
                ...
                filteredImage[(chunkStartY + itemID) * filteredColumns + x] = (CLPixelType)(ClampToQuantum(filteredPixel.x), ClampToQuantum(filteredPixel.y), ClampToQuantum(filteredPixel.z), ClampToQuantum(filteredPixel.w));
            }
            else
            {
                float density = densityCache[itemID];
...
__kernel void UnsharpMaskBlurColumn(const __global CLPixelType* inputImage,
    ...
    if (groupStartY >= 0 && groupStopY < imageRows)
    {
        event_t e = async_work_group_strided_copy(cachedData, blurRowData + groupStartY * imageColumns + groupX, groupStopY - groupStartY, imageColumns, 0);
        wait_group_events(1, &e);
    }
    else
    {
    ...
    if (cy < imageRows)
    {
        ...
        blurredPixel = floor((float4)(ClampToQuantum(blurredPixel.x), ClampToQuantum(blurredPixel.y), ClampToQuantum(blurredPixel.z), ClampToQuantum(blurredPixel.w)));
        ...
        //write back
        filtered_im[cy * imageColumns + groupX] = (CLPixelType)(ClampToQuantum(outputPixel.x), ClampToQuantum(outputPixel.y), ClampToQuantum(outputPixel.z), ClampToQuantum(outputPixel.w));
    }
}
