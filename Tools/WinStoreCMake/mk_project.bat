...
if not exist %CMAKE_DIR%\cmake.exe (echo "Can't find cmake.exe in %CMAKE_DIR%") & (goto exit)

if "%1" == "" (echo "Add path which contains CMakeLists.txt") & (goto exit)

if not exist "%SOURCE_DIR%CMakeLists.txt" (echo "Can't find CMakeLists.txt in %SOURCE_DIR%") & (goto exit)

if not "%2" == "ARM" (
if not "%2" == "Win32" (
if not "%2" == "Win64" (
if not "%2" == "" ( (echo "Add platform Win64 or ARM default Win32") & (goto exit) ) ) ) )
