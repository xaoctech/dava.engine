...
int main(char argc, char** argv)
{
    try
    {
        ...
        bex::apiCall(ILBCreateManager("../../../temp/baking", ILB_CS_LOCAL, bex::getLicenseKey().c_str(), &bmh));
        ...
        bex::apiCall(ILBSetUVTransform(entity, &ILBVec2(0.0f, 0.0f), &ILBVec2(0.5f, 1.0f)));
        bex::apiCall(ILBAddBakeInstance(textureTarget, floorInstance, &entity));
        bex::apiCall(ILBSetUVTransform(entity, &ILBVec2(0.5f, 0.0f), &ILBVec2(0.5f, 1.0f)));

        ILBTargetHandle cameraTarget;
        bex::apiCall(ILBCreateCameraTarget(job, _T("cameraTarget"), camera, 640, 480, &cameraTarget));

        // Add pass to targets
        bex::apiCall(ILBAddPassToTarget(textureTarget, fullShadingPass));
