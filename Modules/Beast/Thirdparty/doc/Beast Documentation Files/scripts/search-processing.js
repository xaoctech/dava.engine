...
top.HlpSys.search = function(){
    ...
    function restoreState(config) {
        ...
        config.caseSensitive = (config.caseSensitive === "true" || config.caseSensitive === true);
            config.filter_prog = (config.filter_prog === "true" || config.filter_prog === true);
        config.filter_api = (config.filter_api === "true" || config.filter_api === true);
        config.filter_headers = (config.filter_headers === "true" || config.filter_headers === true);
}
    ...
    return {
        ...
        restoreState: function(doc) {
            ...
            var filter_progSwitch = searchForm.filter_prog;
            if (filter_progSwitch) {
                var filter_progState = this.config.filter_prog;
                filter_progSwitch.checked = filter_progState;
            }

            var filter_apiSwitch = searchForm.filter_api;
            if (filter_apiSwitch) {
                var filter_apiState = this.config.filter_api;
                filter_apiSwitch.checked = filter_apiState;
            }

            var filter_headersSwitch = searchForm.filter_headers;
            if (filter_headersSwitch) {
                var filter_headersState = this.config.filter_headers;
                filter_headersSwitch.checked = filter_headersState;
            }},
        ...
        config: {
            ...
            setFilterprog : function(option) {
                if (typeof option == "boolean") {
                    this.filter_prog = option;
                } else {
                    this.filter_prog = !this.filter_prog;
                }
                saveState(this);
            },

            setFilterapi : function(option) {
                if (typeof option == "boolean") {
                    this.filter_api = option;
                } else {
                    this.filter_api = !this.filter_api;
                }
                saveState(this);
            },

            setFilterheaders : function(option) {
                if (typeof option == "boolean") {
                    this.filter_headers = option;
                } else {
                    this.filter_headers = !this.filter_headers;
                }
                saveState(this);
            },
		...
		data:{
            filter_prog_inclusions : new Array("(files|shaders)/.*\.(htm|osl)"),
            filter_api_inclusions : new Array("api/((?!8h_source).)*\.html"),
            filter_headers_inclusions : new Array("api/.*8h_source\.html"),
        ...
        SearchString: function(query) {
            ...
            }for (var i_result = 0; i_result < results.getResults().length; i_result++)
			{
				var filename = results.getResults()[i_result].href;
                var refused = false;

                if (refused == false)
				{
					for (var i = 0; i < this.data.filter_prog_inclusions.length; i++)
					{
						if (filename.match(this.data.filter_prog_inclusions[i]))
						{
							if (this.config.filter_prog == false)
							{ refused = true; break; }
						}
					}
				}
                ...
                if (refused == false)
				{
					for (var i = 0; i < this.data.filter_api_inclusions.length; i++)
					{
						if (filename.match(this.data.filter_api_inclusions[i]))
						{
							if (this.config.filter_api == false)
							{ refused = true; break; }
						}
					}
				}
                ...
                if (refused == false)
				{
					for (var i = 0; i < this.data.filter_headers_inclusions.length; i++)
					{
						if (filename.match(this.data.filter_headers_inclusions[i]))
						{
							if (this.config.filter_headers == false)
							{ refused = true; break; }
						}
					}
				}
...
if (refused)
				{
					results.getResults().splice(i_result, 1);
					i_result--;
				}
            }
...
// SIG // MzEwMTQwM1owIwYJKoZIhvcNAQkEMRYEFPoK5wNmfb4m
// SIG // 6mfSD/SrIFbGyGKnMA0GCSqGSIb3DQEBAQUABIIBAFNb
// SIG // 5tAW9sVFO2Tzr1v/fXv9qpZP00aCdfNZGXDPzDlCkDaV
// SIG // i4bjdKmnDheoDiFliJTGcgPjeNGyZ9pag1ht45qf6KY3
// SIG // Ojw4YJqsLZEwv3H8VbmUrtX6PHipXRaUV3dCXQ08EgmP
// SIG // IKlfwMAfxoBXHls+4zxNeISd2dbulORmWlnoffLHOLaH
// SIG // 1EfNQbrjugpY6RIviZ4hlp5LKWFSq0N7P7w53UjNZ9pN
// SIG // UvdA8evdRzoCc9egVNN8KW0PVt6Qk1/Uj2hii+iWPJM8
// SIG // lXZuAj5CfNdfQesjQy3UKi/VXL0ml6zt1rQC0Wg0U633
// SIG // W4XcM5LEKEc9sYeK2xoiH9M3IRiJp24=
