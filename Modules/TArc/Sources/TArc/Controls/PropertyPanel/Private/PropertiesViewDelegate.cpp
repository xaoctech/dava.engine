...
PropertiesViewDelegate::PropertiesViewDelegate(PropertiesTreeView* view_, ReflectedPropertyModel* model_, QObject* parent)
...
QSize PropertiesViewDelegate::sizeHint(const QStyleOptionViewItem& option, const QModelIndex& index) const
{
    ...
    else
    {
        ...
        {
            int width = view->GetIndexWidth(index);
            int height = valueComponent->GetHeightForWidth(width);
...
bool PropertiesViewDelegate::UpdateSizeHints(int section, int newWidth)
{
    ...
    for (auto iter = heightForWidthItems.begin(); iter != heightForWidthItems.end(); ++iter)
    {
        ...
        int width = view->GetIndexWidth(index);
        int heightForWidth = value->GetHeightForWidth(width);
