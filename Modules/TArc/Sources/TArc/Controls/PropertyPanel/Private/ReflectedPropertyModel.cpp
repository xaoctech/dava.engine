...
QMimeData* ReflectedPropertyModel::mimeData(const QModelIndexList& indexes) const
{
    ...
    Vector<ReflectedPropertyItem*> acceptedItems;
    acceptedItems.reserve(indexes.count());
    ...
    foreach (const QModelIndex& index, indexes)
    {
        ...
        if (count == 1)
        {
            std::shared_ptr<PropertyNode> node = item->GetPropertyNode(0);
...
Qt::DropActions ReflectedPropertyModel::supportedDragActions() const
{
    return static_cast<Qt::DropActions>(Qt::MoveAction | Qt::LinkAction);
}
