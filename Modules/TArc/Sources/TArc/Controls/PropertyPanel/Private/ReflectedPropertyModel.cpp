...
DataWrappersProcessor* ReflectedPropertyModel::GetWrappersProcessor(bool isFastWrapper)
{
    if (isFastWrapper == true)
    {
...
void ReflectedPropertyModel::OnItemCreated(ReflectedPropertyItem* item)
{
    item->repaintASAP.Connect([this](ReflectedPropertyItem* item) {
        QModelIndex index = MapItem(item);
        emit dataChanged(index, index.sibling(index.row(), 1));
    });

    if (item->value->RepaintOnUpdateRequire())
    {
        repaintRequire.insert(item);
    }
}
