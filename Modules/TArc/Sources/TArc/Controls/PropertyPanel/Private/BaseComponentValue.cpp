...
public:
    ...
    ButtonModel(BaseComponentValue* valueComponent, const M::CommandProducer::Info& commandInfo_)
        : value(valueComponent)
        , cmd(nullptr)
...
private:
    ...
    {
        return cmd != nullptr && (cmd->OnlyForSingleSelection() == false || value->nodes.size() == 1);
    }
...
class BaseComponentValue::RemoveFieldButtonModel : public BaseComponentValue::ButtonModel
{
public:
    RemoveFieldButtonModel(BaseComponentValue* valueComponent)
        : ButtonModel(valueComponent, M::CommandProducer::Info{ DAVA::SharedIcon(":/TArc/Resources/remove.png"), QStringLiteral("Remove field"), "Field removed" })
    {
    }

    bool IsEnabled() const override
    {
        return !(value->nodes.front()->field.ref.IsReadonly());
    }
...
void BaseComponentValue::EnsureEditorCreated(QWidget* parent)
{
    ...
    auto buttonWidgetCreateFn = [this, parent]() {
        if (buttonsLayout != nullptr)
        {
            return;
        }
    ...
    auto afterButtonsCreatedFn = [this]() {
        if (buttonsLayout == nullptr)
        {
            return;
        }
    ...
    {
        std::shared_ptr<PropertyNode> node = nodes.front();
        std::shared_ptr<PropertyNode> parentNode = node->parent.lock();
        if (parentNode != nullptr &&
            parentNode->field.ref.GetFieldsCaps().canRemoveField == true &&
            parentNode->field.ref.GetMeta<M::MutableCollection>())
        {
            buttonWidgetCreateFn();

            std::unique_ptr<ButtonModel> modelObject = std::make_unique<RemoveFieldButtonModel>(this);
            ...
            ReflectedButton* button = new ReflectedButton(p, GetDataProcessor(), model, buttonsLayout->ToWidgetCast());
            button->ForceUpdate();
...
void BaseComponentValue::RemoveField()
{
    std::shared_ptr<PropertyNode> node = nodes.front();
    std::shared_ptr<PropertyNode> parentNode = nodes.front()->parent.lock();
    DVASSERT(parentNode != nullptr);
    GetModifyInterface()->RemoveField(parentNode->field.ref, node->field.key);
}
