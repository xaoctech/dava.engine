...
Reflection::Field ComponentStructureWrapper::GetField(const ReflectedObject& obj, const ValueWrapper* vw, size_t index) const
{
    const Type* objType = obj.GetReflectedType()->GetType();
    const Type* componentType = Type::Instance<BaseComponentValue>();
    Reflection::Field field = classWrapper->GetField(obj, vw, index);
    if (nullptr != field.ref.GetMeta<M::ProxyMetaRequire>() && TypeInheritance::CanCast(objType, componentType))
    {
        BaseComponentValue* componentValue = obj.GetPtr<BaseComponentValue>();
        if (!componentValue->nodes.empty())
        {
            const Reflection& metaProvider = componentValue->nodes.front()->field.ref;
            field.ref = Reflection::Create(field.ref, metaProvider);
        }
    }
...
Vector<Reflection::Field> ComponentStructureWrapper::GetFields(const ReflectedObject& obj, const ValueWrapper* vw, Reflection::MetaPredicate pred) const
{
    const Type* objType = obj.GetReflectedType()->GetType();
    const Type* componentType = Type::Instance<BaseComponentValue>();
    Vector<Reflection::Field> fields = classWrapper->GetFields(obj, vw, pred);
...
Vector<Reflection::Method> ComponentStructureWrapper::GetMethods(const ReflectedObject& object, const ValueWrapper* vw, Reflection::MetaPredicate pred) const
{
    return classWrapper->GetMethods(object, vw, pred);
}
