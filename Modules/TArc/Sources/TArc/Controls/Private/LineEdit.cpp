...
void LineEdit::UpdateControl(const ControlDescriptor& descriptor)
{
    ...
    if (readOnlyChanged || textChanged)
    {
        ...
        if (textChanged)
        {
            ...
            Any v = fieldValue.GetValue();
            if (v.IsEmpty() == false)
            {
                QString newText = QString::fromStdString(v.Cast<String>());
                if (newText != text())
                {
                    setText(newText);
                }
    ...
    {
        setEnabled(GetFieldValue<bool>(Fields::IsEnabled, isEnabled()));
    }

    if (descriptor.IsChanged(Fields::IsVisible))
    {
        setVisible(GetFieldValue<bool>(Fields::IsVisible, isVisible()));
    }
