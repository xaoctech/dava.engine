...
void BaseSpinBox<TBase, TEditableType>::UpdateControl(const ControlDescriptor& changedFields)
{
    ...
    {
        this->setEnabled(this->template GetFieldValue<bool>(BaseFields::IsEnabled, this->isEnabled()));
    }

    if (changedFields.IsChanged(BaseFields::IsVisible))
    {
        this->setVisible(this->template GetFieldValue<bool>(BaseFields::IsVisible, this->isVisible()));
    }
}
