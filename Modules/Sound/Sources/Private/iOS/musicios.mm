...
bool MusicIOSSoundEvent::Init()
{
    ...
    if (avSound && !(static_cast<AvSound*>(avSound))->initSuccess)
    {
        [static_cast<AvSound*>(avSound) release];
        avSound = 0;
        return false;
    }
    return true;
...
{
    Retain();
    return [static_cast<AvSound*>(avSound) play];
}

void MusicIOSSoundEvent::Stop(bool force /* = false */)
{
    [static_cast<AvSound*>(avSound) stop];
}

void MusicIOSSoundEvent::SetPaused(bool paused)
{
    if (paused)
        [static_cast<AvSound*>(avSound) pause];
    else
        [static_cast<AvSound*>(avSound) play];
}

void MusicIOSSoundEvent::SetVolume(float32 _volume)
{
    volume = _volume;
    (static_cast<AvSound*>(avSound)).audioPlayer.volume = Clamp(_volume, 0.f, 1.f);
}
...
void MusicIOSSoundEvent::SetSpeed(float32 _speed)
{
    ...
    AVAudioPlayer* player = (static_cast<AvSound*>(avSound)).audioPlayer;
...
{
    (static_cast<AvSound*>(avSound)).audioPlayer.numberOfLoops = looping;
}

int32 MusicIOSSoundEvent::GetLoopCount() const
{
    return static_cast<int32>((static_cast<AvSound*>(avSound)).audioPlayer.numberOfLoops);
}

bool MusicIOSSoundEvent::IsActive() const
{
    return [(static_cast<AvSound*>(avSound)).audioPlayer isPlaying];
}
