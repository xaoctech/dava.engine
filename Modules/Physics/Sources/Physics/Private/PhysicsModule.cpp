...
void PhysicsModule::Init()
{
    ...
    PxVehicleSetBasisVectors(PxVec3(0.0f, 0.0f, 1.0f), PxVec3(-1.0f, 0.0f, 0.0f)); // TODO: change forward vector?
    PxVehicleSetUpdateMode(PxVehicleUpdateMode::eVELOCITY_CHANGE);

    static PhysicsModuleDetail::AssertHandler assertHandler;
    PxSetAssertHandler(assertHandler);

    DAVA_REFLECTION_REGISTER_PERMANENT_NAME(PhysicsSystem);
    DAVA_REFLECTION_REGISTER_PERMANENT_NAME(WASDPhysicsControllerSystem);
    ...
    DAVA_REFLECTION_REGISTER_PERMANENT_NAME(CollisionSingleComponent);
}
...
physx::PxScene* PhysicsModule::CreateScene(const PhysicsSceneConfig& config, physx::PxSimulationFilterShader filterShader, physx::PxSimulationEventCallback* callback) const
{
    ...
    sceneDesc.flags = PxSceneFlag::eENABLE_ACTIVE_ACTORS | PxSceneFlag::eENABLE_ENHANCED_DETERMINISM | PxSceneFlag::eENABLE_CCD;
...
physx::PxRigidActor* PhysicsModule::CreateStaticActor() const
{
    return physics->createRigidStatic(physx::PxTransform(physx::PxIDENTITY::PxIdentity));
}

physx::PxRigidActor* PhysicsModule::CreateDynamicActor() const
{
