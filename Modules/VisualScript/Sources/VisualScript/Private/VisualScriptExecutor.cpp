...
{
    VisualScriptFunction* function = node->GetFunction();
    if (!function)
        return;
    ...
    auto& types = function->function.GetInvokeParams().GetArgumentTypes();
    for (size_t ti = 0; ti < size; ++ti)
    {
        if (types[ti] != params[ti].GetType())
        {
            // throw exception. Incorrect param type.
        }
    }
    ...
    Any result;
    if (size == 0)
        result = function->function.Invoke();
    else if (size == 1)
        result = function->function.InvokeWithCast(params[0]);
    else if (size == 2)
        result = function->function.InvokeWithCast(params[0], params[1]);
    else if (size == 3)
        result = function->function.InvokeWithCast(params[0], params[1], params[2]);
    else if (size == 4)
        result = function->function.InvokeWithCast(params[0], params[1], params[2], params[3]);
