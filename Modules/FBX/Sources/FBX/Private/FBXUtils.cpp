...
FilePath GetFBXFilePath(const FbxObject* sceneObject)
{
    DVASSERT(sceneObject != nullptr);
    DVASSERT(sceneObject->GetScene() != nullptr);
    DVASSERT(sceneObject->GetScene()->GetSceneInfo() != nullptr);

    FbxString url = sceneObject->GetScene()->GetSceneInfo()->Url.Get();
    return FilePath(url.Buffer());
}

FilePath GetFBXTexturePath(const FbxSurfaceMaterial* fbxMaterial, FbxLayerElement::EType type)
{
    DVASSERT(type >= FbxLayerElement::sTypeTextureStartIndex && type <= FbxLayerElement::sTypeTextureEndIndex);

    FbxProperty textureProperty = fbxMaterial->FindProperty(FbxLayerElement::sTextureChannelNames[FBXSDK_TEXTURE_INDEX(type)]);
    ...
    else
    {
        if (textureProperty.GetSrcObjectCount<FbxTexture>() > 0)
            fbxTexture = textureProperty.GetSrcObject<FbxTexture>(0);
    }

    FbxFileTexture* fbxFileTexture = FbxCast<FbxFileTexture>(fbxTexture);
    if (fbxFileTexture != nullptr)
        return GetFBXFilePath(fbxMaterial).GetDirectory() + fbxFileTexture->GetRelativeFileName();
