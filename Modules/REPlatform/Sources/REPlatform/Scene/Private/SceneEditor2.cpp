    ...
    , commandStack(new RECommandStack())
{
    ...
    AddSystem(new LightingPresetsSystem(this), transformSystem); // Scene will react if any of this component will appears.
    ...
    AddSystem(new EditorLightmapSystem(this));
...
SceneFileV2::eError SceneEditor2::LoadAsPrefab(const FilePath& path)
{
    ...
    {
        return SceneFileV2::ERROR_FILE_READ_ERROR;
    }
...
void SceneEditor2::Setup3DDrawing()
{
    if (drawCamera)
    {
        drawCamera->SetupDynamicParameters(false, false);
    }
}
