...
CollisionObj CreateBox(bool createCollision, bool recreate, const Matrix4& transform, const Vector3& halfSize, QueryObjectDataComponent* queryData, void* userData)
{
    CollisionObj result;
    result.isValid = true;
    ...
    if (createCollision)
    {
        ...
        physx::PxFilterData queryFilterData = objectFilterData;
        if (queryData != nullptr)
        {
            queryFilterData.word3 = queryData->GetData();
        }
...
CollisionObj CreateMesh(bool createCollision, const Matrix4& transform, RenderObject* ro, PhysicsGeometryCache* cache, QueryObjectDataComponent* queryData, void* userData)
{
    CollisionObj result;
    result.isValid = true;
    if (createCollision)
...
CollisionObj CreateLandscape(bool createCollision, Landscape* landscape, QueryObjectDataComponent* queryData, void* userData)
{
    CollisionObj result;
    result.isValid = true;
    ...
    if (landscape->GetHeightmap() != nullptr)
    {
        if (createCollision)
        {
            ...
            InitBounds(rigidActor, shape);
            physx::PxFilterData queryFilterData = landscapeFilterData;
            if (queryData != nullptr)
            {
                queryFilterData.word3 = queryData->GetData();
            }
    ...
    {
        result = CreateBox(createCollision, true, Matrix4::IDENTITY, Vector3(0.5f, 0.5f, 0.5f), queryData, userData);
    }
    return result;
}
...
class QueryFilterCallback final : public physx::PxQueryFilterCallback
{
    virtual physx::PxQueryHitType::Enum preFilter(const physx::PxFilterData& filterData0, const physx::PxShape* shape, const physx::PxRigidActor* actor, physx::PxHitFlags& queryFlags)
    {
        ...
        if ((filterData0.word0 & filterData1.word0) == 0 &&
            (filterData0.word1 & filterData1.word1) == 0)
        {
            return physx::PxQueryHitType::eNONE;
        }

        if (filterData0.word3 != 0 && (filterData0.word3 & filterData1.word3) == 0)
        {
            return physx::PxQueryHitType::eNONE;
        }
...
Component* QueryObjectDataComponent::Clone(Entity* toEntity)
{
    QueryObjectDataComponent* component = new QueryObjectDataComponent();
...
uint32 QueryObjectDataComponent::GetData() const
{
    return data;
}

void QueryObjectDataComponent::SetData(uint32 newData)
{
    if (data != newData)
    {
        data = newData;
        ...
        if (e != nullptr)
        {
            e->GetScene()->GetSystem<SceneCollisionSystem>()->UpdateActor(this);
        }
    }
}
...
{
    return new QueryObjectDataComponent();
}

DAVA_VIRTUAL_REFLECTION_IMPL(QueryObjectDataComponent)
{
    ReflectionRegistrator<QueryObjectDataComponent>::Begin()[M::NonExportableComponent(),
                                                             M::HiddenField(),
...
bool SceneCollisionSystem::OverlapSphereTest(const DAVA::Vector3& position, DAVA::float32 sphereRadius, DAVA::uint32 addFilterData) const
{
    physx::PxSphereGeometry sphere(sphereRadius);
    physx::PxTransform pose(physx::PxVec3(position.x, position.y, position.z));

    physx::PxQueryFilterData fd;
    fd.data = SceneCollisionSystemDetail::objectFilterData;
    fd.data.word3 = addFilterData;
    fd.flags = physx::PxQueryFlag::eSTATIC;
    fd.flags |= physx::PxQueryFlag::ePREFILTER;
    fd.flags |= physx::PxQueryFlag::eANY_HIT;

    SceneCollisionSystemDetail::QueryFilterCallback filterCallback;
    physx::PxOverlapBuffer hit;
    return physicsScene->overlap(sphere, pose, hit, fd, &filterCallback);
}

void SceneCollisionSystem::OverlapSphereTest(const DAVA::Vector3& position, DAVA::float32 sphereRadius, SelectableGroup::CollectionType& result, DAVA::uint32 addFilterData) const
{
    physx::PxSphereGeometry sphere(sphereRadius);
    ...
    SceneCollisionSystemDetail::QueryFilterCallback filterCallback;
    physx::PxOverlapHit hit[SceneCollisionSystemDetail::maxQueryTouchCount];
    physx::PxOverlapBuffer buffer(hit, SceneCollisionSystemDetail::maxQueryTouchCount);
    if (physicsScene->overlap(sphere, pose, buffer, fd, &filterCallback) == true)
    {
        result.reserve(buffer.nbTouches);
        for (physx::PxU32 i = 0; i < buffer.nbTouches; i++)
        {
            physx::PxOverlapHit& currentHit = hit[i];
            DVASSERT(currentHit.actor != nullptr);

            Any objPtr(currentHit.actor->userData);
            auto iter = objToPhysx.find(objPtr);
            DVASSERT(iter != objToPhysx.end());
...
void SceneCollisionSystem::UpdateActor(QueryObjectDataComponent* component)
{
    DVASSERT(component != nullptr);
    Entity* e = component->GetEntity();

    auto iter = objToPhysx.find(e);
    DVASSERT(iter != objToPhysx.end());
    physx::PxRigidActor* actor = iter->second;
    ...
    for (physx::PxU32 i = 0; i < shapesCount; ++i)
    {
        ...
        filterData.word3 = component->GetData();
...
void SceneCollisionSystem::EnumerateObjectHierarchy(const Selectable& object, bool createCollision, const TCallBack& callback)
{
    ...
    if (object.CanBeCastedTo<Entity>())
    {
        ...
        {
            result = CreateLandscape(createCollision, landscape, queryData, userData);
        }
        ...
        if ((result.isValid == false) && (particleEffect != nullptr))
        {
            ...
            result = CreateBox(createCollision, false, entity->GetWorldTransform(), toVec3Fn(debugBoxParticleScale), queryData, userData);
        }

        GeoDecalComponent* geoDecalComponent = GetGeoDecalComponent(entity);
        if ((result.isValid == false) && (geoDecalComponent != nullptr))
        {
            result = CreateBox(createCollision, true, entity->GetWorldTransform(), geoDecalComponent->GetDimensions(), queryData, userData);
        }

        DecalComponent* decalComponent = GetDecalComponent(entity);
        if ((result.isValid == false) && (decalComponent != nullptr))
        {
            result = CreateBox(createCollision, true, entity->GetWorldTransform(), 2.0f * decalComponent->GetLocalSize(), queryData, userData);
        }
        ...
        if ((result.isValid == false) && (renderObject != nullptr))
        {
            ...
            {
                result = CreateMesh(createCollision, entity->GetWorldTransform(), renderObject, geometryCache, queryData, userData);
            }
        }
        ...
        if ((result.isValid == false) && (camera != nullptr))
        {
            ...
            result = CreateBox(createCollision, true, transform, toVec3Fn(debugBoxScale), queryData, userData);
        }
        ...
        if ((result.isValid == false) && (entity != nullptr))
        {
            ...
            {
                result = CreateBox(createCollision, false, entity->GetWorldTransform(), toVec3Fn(debugBoxUserScale), queryData, userData);
            }
