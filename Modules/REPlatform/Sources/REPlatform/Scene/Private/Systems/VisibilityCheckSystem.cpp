...
VisibilityCheckSystem::VisibilityCheckSystem(Scene* scene)
    : SceneSystem(scene, ComponentUtils::MakeMask<VisibilityCheckComponent>())
...
void VisibilityCheckSystem::Draw()
{
    ...
    GetFinalGatherCamera()->SetupDynamicParameters(false, false, nullptr);
}
...
bool VisibilityCheckSystem::CacheIsValid()
{
    ...
    auto currentMatrix = cam->GetViewProjMatrix(false, false);
...
void VisibilityCheckSystem::BuildCache()
{
    ...
    stateCache.viewprojMatrix = stateCache.camera->GetViewProjMatrix(false, false);
}
