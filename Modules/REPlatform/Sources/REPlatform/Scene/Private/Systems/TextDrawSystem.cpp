...
void TextDrawSystem::PushNextBatch(const Color& color)
{
    ...
    if (!fontMaterial->PreBuildMaterial(RenderSystem2D::RENDER_PASS_NAME))
        return;
...
void TextDrawSystem::DrawTextLines(const DAVA::Vector2& pos2d, const DAVA::Vector<DAVA::String>& lines, const DAVA::Color& color, Align align)
{
    Vector<WideString> wlines;
    wlines.reserve(lines.size());
    for (const DAVA::String& line : lines)
        wlines.emplace_back(UTF8Utils::EncodeToWideString(line));

    DrawTextLines(pos2d, wlines, color, align);
}

void TextDrawSystem::DrawTextLines(const DAVA::Vector2& pos2d, const DAVA::Vector<DAVA::WideString>& lines, const DAVA::Color& color, Align align)
{
    Vector2 pos = pos2d;
    for (const DAVA::WideString& line : lines)
    {
        textToDraw.emplace_back(pos, line, color, align, font->GetSize());
        pos.y += GetFont()->GetFontHeight();
    }
}
