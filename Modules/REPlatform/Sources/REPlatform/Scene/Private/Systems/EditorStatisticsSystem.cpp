...
DAVA_VIRTUAL_REFLECTION_IMPL(RenderStatsSettings)
{
    ...
    .Field("calculateSpeedTreePerFrame", &RenderStatsSettings::calculateSpeedTreePerFrame)[M::DisplayName(" Calculate SpeedTree per frame")]
    .End();
}
...
void EditorStatisticsSystem::CalculateTriangles()
{
    ...
    if (drawCamera != nullptr)
    {
        ...
        clipResult.Clear();
        GetScene()->GetRenderSystem()->GetRenderHierarchy()->Clip(drawCamera, clipResult, currVisibilityCriteria);
        triangles[eEditorMode::MODE_ALL_SCENE].renderObjects = clipResult.geometryArray; //GFX_COMPLETE - we will defenitly need a bit different scene info for deferred
