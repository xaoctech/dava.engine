...
void DebugDrawSystem::DrawLightNode(Entity* entity, bool isSelected)
{
    ...
    if (NULL != light)
    {
        ...
        else if (light->GetLightType() == Light::TYPE_POINT)
        {
        ...
        else if (light->GetLightType() == Light::TYPE_SPOT)
        {
...
void DebugDrawSystem::DrawDecal(const Matrix4& transform, const AABBox3& box, GeoDecalManager::Mapping mapping)
{
    ...
    if (mapping == GeoDecalManager::Mapping::CYLINDRICAL)
    {
    ...
    else if (mapping == GeoDecalManager::Mapping::SPHERICAL)
    {
...
void DebugDrawSystem::DrawPostEffects(Entity* entity)
{
    SceneEditor2* sceneEditor = ((SceneEditor2*)GetScene());

    PostEffectDebugComponent* component = entity->GetComponent<PostEffectDebugComponent>();
    if ((component != nullptr) && component->GetLightMeterEnabled())
    {
        PostEffectRenderer::DebugRenderer* debugRenderer = sceneEditor->GetRenderSystem()->GetPostEffectRenderer()->GetDebugRenderer();

        Vector<String> lines = {
            Format("RGB: %.3f, %.3f, %.3f",
                   debugRenderer->debugPickedLuminance.x * GetGlobalLuminanceScale(),
                   debugRenderer->debugPickedLuminance.y * GetGlobalLuminanceScale(),
                   debugRenderer->debugPickedLuminance.z * GetGlobalLuminanceScale()),

            Format("Lum: %.3f (%.3f EV)",
                   debugRenderer->debugPickedLuminance.w * GetGlobalLuminanceScale(),
                   GetEVFromLuminance(debugRenderer->debugPickedLuminance.w))
        };

        TextDrawSystem* textDrawSystem = sceneEditor->GetSystem<TextDrawSystem>();
        textDrawSystem->DrawTextLines(debugRenderer->pointOfInterest + Vector2(1.0f, 1.0f), lines, Color::Black, TextDrawSystem::Align::Center);
        textDrawSystem->DrawTextLines(debugRenderer->pointOfInterest, lines, Color::White, TextDrawSystem::Align::Center);
    }
}
} // namespace DAVA
