...
void NotPassableTerrainProxy::UpdateTexture(Heightmap* heightmap, const AABBox3& landscapeBoundingBox, const Rect2i& forRect)
{
    ...
    {
        projMatrix.BuildOrtho(0.0f, textureSize.dx, 0.0f, textureSize.dy, -1.0f, 1.0f, rhi::DeviceCaps().isZeroBaseClipRange, false);
    }
    else
    {
        projMatrix.BuildOrtho(0.0f, textureSize.dx, textureSize.dy, 0.0f, -1.0f, 1.0f, rhi::DeviceCaps().isZeroBaseClipRange, false);
    }
    ...
    passConfig.name = "NotPassableTerrainProxy_Update";
            ...
            if (Rect2i(xRect, yRect, GRID_QUAD_SIZE, GRID_QUAD_SIZE).RectIntersects(forRect))
            {
                ...
                for (int32 y = yRect; (y < yRect + GRID_QUAD_SIZE) && y < heightmapSize; ++y)
                {
                    ...
                    for (int32 x = xRect; (x < xRect + GRID_QUAD_SIZE) && x < heightmapSize; ++x)
                    {
                        ...
                        PickColor(Max(tanRight, tanBottom), color);
