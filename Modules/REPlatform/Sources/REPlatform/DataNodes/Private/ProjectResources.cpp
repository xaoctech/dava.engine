...
void ProjectResources::LoadProject(const FilePath& incomePath)
{
    ...
    if (incomePath.IsDirectoryPathname() && incomePath != data->projectPath)
    {
        ...
        engineCtx->assetManager->AddWatch(data->GetDataSourcePath().GetAbsolutePathname(), true);
        engineCtx->assetManager->AddWatch(data->GetDataPath().GetAbsolutePathname(), true);
