...
{
    Vector<String> permanentNames = ReflectedTypeDB::GetPermanentNames();
    for (const String& permanentName : permanentNames)
    {
        const ReflectedType* type = ReflectedTypeDB::GetByPermanentName(permanentName);
        DVASSERT(type != nullptr);
