...
mousePressEvent(QGraphicsSceneMouseEvent* event)
{
    ...
    [&](PortType portToCheck)
    {
        ...
        if (portIndex != INVALID)
        {
            ...
            {
                auto connectionPolicy = _node.nodeDataModel()->connectionPolicy(portToCheck, portIndex);
...
hoverMoveEvent(QGraphicsSceneHoverEvent* event)
{
    ...
    {
        if (geom.boundingRect().contains(QPoint(pos.x(), pos.y())) == true)
        {
            NodeGeometry& nodeGeometry = _node.nodeGeometry();
            auto getToolTipForPort = [&](PortType portToCheck)
            {
                int portIndex = nodeGeometry.checkHitScenePoint(portToCheck, event->scenePos(), sceneTransform());
                if (portIndex != INVALID)
                {
                    return _node.nodeDataModel()->portHint(portToCheck, portIndex);
                }
                return QString();
            };

            QString tip = getToolTipForPort(PortType::In);
            if (tip.isEmpty() == true)
            {
                tip = getToolTipForPort(PortType::Out);
            }
