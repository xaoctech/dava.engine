...
createConnection(PortType connectedPort,
                 ...
                 PortKind portKind)
{
    auto connection = std::make_shared<Connection>(connectedPort, node, portIndex, portKind);
...
restoreConnection(QJsonObject const& connectionJson)
{
    ...
    PortKind portKindIn = static_cast<PortKind>(connectionJson["in_kind"].toInt());
    PortKind portKindOut = static_cast<PortKind>(connectionJson["out_kind"].toInt());

    auto nodeIn = _nodes[nodeInId].get();
    auto nodeOut = _nodes[nodeOutId].get();

    return createConnection(*nodeIn, portIndexIn, portKindIn, *nodeOut, portIndexOut, portKindOut);
}
...
removeNode(Node& node)
{
    ...
    NodeWillBeRemoved(node.nodeDataModel());
