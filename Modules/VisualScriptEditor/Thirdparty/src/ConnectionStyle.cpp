...
QColor
ConnectionStyle::
normalColor(QString typeId, bool reduceBlue) const
{
    uint hash = qHash(typeId);

    int const hue_range = 0xFF;

    qsrand(hash);
    int hue = qrand() % hue_range;

    int sat = static_cast<int>(120 + hash % 129);

    QColor c = QColor::fromHsl(hue,
                               sat,
                               160);
