...
public:
  ...
  virtual ~DataModelRegistry() = default;
...
public:
  ...
  getTypeConverter(QString const &sourceTypeID,
    ...
    virtual const std::map<QString, std::vector<QtNodes::RegistryItemDescriptor*>>& GetStaticModels() const = 0;
    virtual const std::map<QString, std::vector<QtNodes::RegistryItemDescriptor*>>& GetDynamicModels() = 0;
    virtual std::unique_ptr<NodeDataModel> Create(const RegistryItemDescriptor* descriptor) = 0;
