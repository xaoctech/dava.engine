                     ...
                     FlowScene const& scene)
{
    ...
    [&](PortType portType)
    {
        ...
        for (size_t i = 0; i < n; ++i)
        {
            ...
            {
                painter->setBrush(connectionStyle.normalColor(dataType.id, dataType.hasData));
            }
            else
            {
            ...
            {
                double radius1_4 = radius / 4;

                QPolygon polygon;
                polygon << QPoint(p.x() - radius - radius1_4, p.y() - radius)
                        << QPoint(p.x() + radius - radius1_4, p.y() - radius)
                        << QPoint(p.x() + radius + radius1_4, p.y())
                        << QPoint(p.x() + radius - radius1_4, p.y() + radius)
                        << QPoint(p.x() - radius - radius1_4, p.y() + radius);
    ...
    [&](PortType portType)
    {
        ...
        for (size_t i = 0; i < n; ++i)
        {
            ...
            if (!state.getEntries(portType)[i].empty())
            {
                ...
                if (connectionStyle.useDataDefinedColors())
                {
                    QColor const c = connectionStyle.normalColor(dataType.id, dataType.hasData);
                    painter->setPen(c);
                    painter->setBrush(c);
                }
                else
              ...
              NodeDataModel const* model)
{
    ...
    //
    float diam = nodeStyle.ConnectionPointDiameter;
    QRectF boundary(-diam, -diam, 2.0 * diam + geom.width(), diam + geom.entryHeight());
    double const radius = 3.0;
    {
        QLinearGradient gradient(QPointF(0.0, 0.0),
                                 QPointF(geom.width(), geom.height()));

        gradient.setColorAt(0.0, model->captionColor());
        gradient.setColorAt(0.5, nodeStyle.GradientColor1);
        gradient.setColorAt(0.75, nodeStyle.GradientColor2);
        gradient.setColorAt(1.0, nodeStyle.GradientColor3);
