...
inline void memcpy_s(void* dest, size_t sizeInBytes, const void* src, size_t count)
{
    ...
    CRYPTOPP_ASSERT(dest != NULL);
    CRYPTOPP_ASSERT(src != NULL);
...
template <class T>
inline const T& STDMIN(const T& a, const T& b)
{
    return b < a ? b : a;
}
...
template <class T>
inline const T& STDMAX(const T& a, const T& b)
{
...
CRYPTOPP_DLL
std::string
IntToString<word64>(word64 value, unsigned int base);
...
CRYPTOPP_DLL
std::string
IntToString<Integer>(Integer value, unsigned int base);
...
{
    CRYPTOPP_ASSERT(inout != NULL);
    CRYPTOPP_ASSERT(size < INT_MAX);
...
template <>
inline word32 rotlFixed<word32>(word32 x, unsigned int y)
{
...
template <>
inline word32 rotrFixed<word32>(word32 x, unsigned int y)
{
    // Uses Microsoft <stdlib.h> call, bound to C/C++ language rules.
    CRYPTOPP_ASSERT(y < 8 * sizeof(x));
...
template <>
inline word32 rotlVariable<word32>(word32 x, unsigned int y)
{
    CRYPTOPP_ASSERT(y < 8 * sizeof(x));
...
template <>
inline word32 rotrVariable<word32>(word32 x, unsigned int y)
{
    CRYPTOPP_ASSERT(y < 8 * sizeof(x));
...
//!   <tt>[0, sizeof(T)*8 - 1]</tt> to avoid undefined behavior.
template <>
inline word32 rotlMod<word32>(word32 x, unsigned int y)
{
    y %= 8 * sizeof(x);
    return _lrotl(x, static_cast<byte>(y));
}
...
//!   <tt>[0, sizeof(T)*8 - 1]</tt> to avoid undefined behavior.
template <>
inline word32 rotrMod<word32>(word32 x, unsigned int y)
{
    y %= 8 * sizeof(x);
    return _lrotr(x, static_cast<byte>(y));
}
...
//! \note rotrFixed will assert in Debug builds if is outside the allowed range.
template <>
inline word64 rotrFixed<word64>(word64 x, unsigned int y)
{
    // Uses Microsoft <stdlib.h> call, bound to C/C++ language rules.
    CRYPTOPP_ASSERT(y < 8 * sizeof(x));
...
//! \note rotlVariable will assert in Debug builds if is outside the allowed range.
template <>
inline word64 rotlVariable<word64>(word64 x, unsigned int y)
{
    CRYPTOPP_ASSERT(y < 8 * sizeof(x));
...
//! \note rotrVariable will assert in Debug builds if is outside the allowed range.
template <>
inline word64 rotrVariable<word64>(word64 x, unsigned int y)
{
    CRYPTOPP_ASSERT(y < 8 * sizeof(x));
    return y ? _rotr64(x, static_cast<byte>(y)) : x;
}
...
//!   <tt>[0, sizeof(T)*8 - 1]</tt> to avoid undefined behavior.
template <>
inline word64 rotlMod<word64>(word64 x, unsigned int y)
{
    CRYPTOPP_ASSERT(y < 8 * sizeof(x));
    return y ? _rotl64(x, static_cast<byte>(y)) : x;
}
...
//!   <tt>[0, sizeof(T)*8 - 1]</tt> to avoid undefined behavior.
template <>
inline word64 rotrMod<word64>(word64 x, unsigned int y)
{
    CRYPTOPP_ASSERT(y < 8 * sizeof(x));
    return y ? _rotr64(x, static_cast<byte>(y)) : x;
}
...
template <>
inline word16 rotlFixed<word16>(word16 x, unsigned int y)
{
...
template <>
inline word16 rotrFixed<word16>(word16 x, unsigned int y)
{
    // Intrinsic, not bound to C/C++ language rules.
...
template <>
inline word16 rotlVariable<word16>(word16 x, unsigned int y)
{
    return _rotl16(x, static_cast<byte>(y));
}

template <>
inline word16 rotrVariable<word16>(word16 x, unsigned int y)
{
    return _rotr16(x, static_cast<byte>(y));
}

template <>
inline word16 rotlMod<word16>(word16 x, unsigned int y)
{
    return _rotl16(x, static_cast<byte>(y));
}

template <>
inline word16 rotrMod<word16>(word16 x, unsigned int y)
{
    return _rotr16(x, static_cast<byte>(y));
}

template <>
inline byte rotlFixed<byte>(byte x, unsigned int y)
{
    // Intrinsic, not bound to C/C++ language rules.
...
template <>
inline byte rotrFixed<byte>(byte x, unsigned int y)
{
    // Intrinsic, not bound to C/C++ language rules.
...
template <>
inline byte rotlVariable<byte>(byte x, unsigned int y)
{
    return _rotl8(x, static_cast<byte>(y));
}

template <>
inline byte rotrVariable<byte>(byte x, unsigned int y)
{
    return _rotr8(x, static_cast<byte>(y));
}

template <>
inline byte rotlMod<byte>(byte x, unsigned int y)
{
    return _rotl8(x, static_cast<byte>(y));
}

template <>
inline byte rotrMod<byte>(byte x, unsigned int y)
{
    return _rotr8(x, static_cast<byte>(y));
}
...
public:
    ...
    PutBlock(const void* xorBlock, void* block)
        : m_xorBlock((const byte*)xorBlock)
        , m_block((byte*)block)
    {
    }
...
struct BlockGetAndPut
{
    ...
    static inline GetBlock<T, B, GA> Get(const void* block)
    {
        return GetBlock<T, B, GA>(block);
    }
...
struct SafeShifter<true>
{
    ...
    {
        CRYPTOPP_UNUSED(value);
        CRYPTOPP_UNUSED(bits);
        return 0;
    }
