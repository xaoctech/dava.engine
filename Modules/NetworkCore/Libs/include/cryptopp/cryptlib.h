...
{
public:
    explicit InvalidArgument(const std::string& s)
...
{
public:
    explicit InvalidDataFormat(const std::string& s)
        : Exception(INVALID_DATA_FORMAT, s)
    {
    }
};
...
{
public:
    explicit InvalidCiphertext(const std::string& s)
        : InvalidDataFormat(s)
    {
    }
};
...
{
public:
    explicit CannotFlush(const std::string& s)
        : Exception(CANNOT_FLUSH, s)
    {
    }
};
...
struct CRYPTOPP_DLL DecodingResult
{
    ...
    explicit DecodingResult(size_t len)
        : isValidCoding(true)
        , messageLength(len)
    {
    }
...
public:
    ...
    {
        return IVRequirement() <= UNIQUE_IV;
    }
...
public:
    ...
    inline CipherDir GetCipherDirection() const
    {
...
public:
    ...
    {
    public:
        explicit BadState(const std::string& name, const char* message)
            : Exception(OTHER_ERROR, name + ": " + message)
        {
        }
        explicit BadState(const std::string& name, const char* function, const char* state)
            : Exception(OTHER_ERROR, name + ": " + function + " was called before " + state)
        {
        }
    };
...
protected:
    ...
    {
        CRYPTOPP_UNUSED(headerLength);
        CRYPTOPP_UNUSED(messageLength);
        CRYPTOPP_UNUSED(footerLength);
    }
};
...
public:
    ...
    void GetBlock(byte* output, size_t size)
    {
...
public:
    ...
    struct BlockingInputOnly : public NotImplemented
    {
        BlockingInputOnly(const std::string& s)
            : NotImplemented(s + ": Nonblocking input is not implemented by this object.")
        {
        }
    };
    //@}
...
public:
    ...
    {
        TransferTo2(target, transferMax, channel);
        return transferMax;
    }
    ...
    {
        lword i = position;
        CopyRangeTo2(target, i, i + copyMax, channel);
        return i - position;
    }
    ...
    struct NoChannelSupport : public NotImplemented
    {
        NoChannelSupport(const std::string& name)
            : NotImplemented(name + ": this object doesn't support multiple channels")
        {
        }
    };
    ...
    struct InvalidChannelName : public InvalidArgument
    {
        InvalidChannelName(const std::string& name, const std::string& channel)
            : InvalidArgument(name + ": unexpected channel name \"" + channel + "\"")
        {
        }
    };
...
public:
    ...
    {
    public:
        explicit InvalidMaterial(const std::string& s)
            : InvalidDataFormat(s)
        {
        }
    };
...
public:
    ...
    {
        return const_cast<PrivateKeyAlgorithm*>(this)->AccessPrivateKey();
    }
};
...
public:
    ...
    {
        return const_cast<KeyAgreementAlgorithm*>(this)->AccessCryptoParameters();
    }
};
...
public:
    ...
    public:
        InvalidPlaintextLength()
            : Exception(OTHER_ERROR, "PK_Encryptor: invalid plaintext length")
        {
        }
    };
...
public:
    ...
    public:
        KeyTooShort()
            : InvalidKeyLength("PK_Signer: key too short for this signature scheme")
        {
        }
    };
    ...
    {
        CRYPTOPP_UNUSED(recoverablePartLength);
...
public:
    BERDecodeErr()
        : InvalidArgument("BER decode error")
    {
    }
