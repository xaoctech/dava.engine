...
public:
    ...
    public:
        DivideByZero()
            : Exception(OTHER_ERROR, "Integer: division by zero")
        {
        }
    };
    ...
    public:
        OpenPGPDecodeErr()
            : Exception(INVALID_DATA_FORMAT, "OpenPGP decode error")
        {
        }
    };
    ...
    Integer& operator%=(word t)
    {
        return * this = Integer(POSITIVE, 0, Modulo(t));
    }
...
//!
inline bool operator==(const CryptoPP::Integer& a, const CryptoPP::Integer& b)
{
...
//!
inline CryptoPP::Integer operator+(const CryptoPP::Integer& a, const CryptoPP::Integer& b)
{
    return a.Plus(b);
}
//!
...
//!
inline CryptoPP::Integer operator/(const CryptoPP::Integer& a, CryptoPP::word b)
{
    return a.DividedBy(b);
}
//!
//! \sa a_times_b_mod_c() and a_exp_b_mod_c()
inline CryptoPP::word operator%(const CryptoPP::Integer& a, CryptoPP::word b)
{
    return a.Modulo(b);
}
