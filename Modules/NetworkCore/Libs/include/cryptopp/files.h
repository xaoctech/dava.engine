...
public:
    ...
    public:
        OpenErr(const std::string& filename)
            : Err("FileStore: error opening file for reading: " + filename)
        {
        }
    };
    ...
    public:
        ReadErr()
            : Err("FileStore: error reading file")
        {
        }
    };
...
public:
    ...
    FileSource(const wchar_t* filename, bool pumpAll, BufferedTransformation* attachment = NULL, bool binary = true)
        : SourceTemplate<FileStore>(attachment)
    {
        SourceInitialize(pumpAll, MakeParameters(Name::InputFileNameWide(), filename)(Name::InputBinaryMode(), binary));
    }
#endif
...
public:
    ...
    public:
        OpenErr(const std::string& filename)
            : Err("FileSink: error opening file for writing: " + filename)
        {
        }
    };
    ...
    public:
        WriteErr()
            : Err("FileSink: error writing file")
        {
        }
    };
