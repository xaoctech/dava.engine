...
private:
    ...
    T* GetAlignedArray()
    {
        return (CRYPTOPP_BOOL_ALIGN16 && T_Align16) ? (T*)(void*)(((byte*)m_array) + (0 - (size_t)m_array) % 16) : m_array;
    }
    CRYPTOPP_ALIGN_DATA(8)
    T m_array[(CRYPTOPP_BOOL_ALIGN16 && T_Align16) ? S + 8 / sizeof(T) : S];
#endif
...
public:
    ...
    explicit SecBlock(size_type size = 0)
        : m_size(size)
        , m_ptr(m_alloc.allocate(size, NULL))
    {
    }
    ...
    SecBlock(const SecBlock<T, A>& t)
        : m_size(t.m_size)
        , m_ptr(m_alloc.allocate(t.m_size, NULL))
    {
    ...
    typename A::const_pointer data() const
    {
        return m_ptr;
    }
...
public:
    ...
    explicit SecBlockWithHint(size_t size)
        : SecBlock<T, A>(size)
    {
    }
};
...
inline bool operator==(const CryptoPP::AllocatorWithCleanup<T, A>&, const CryptoPP::AllocatorWithCleanup<U, B>&)
{
    return (true);
}
template <class T, bool A, class U, bool B>
