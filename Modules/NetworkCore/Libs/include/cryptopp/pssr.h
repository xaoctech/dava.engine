...
template <bool USE_HASH_ID>
class PSSR_MEM_BaseWithHashId;

//! \brief PSSR Message Encoding Method with Hash Identifier
//! \tparam true flag indicating HashId is used
template <>
class PSSR_MEM_BaseWithHashId<true> : public EMSA2HashIdLookup<PSSR_MEM_Base>
{
};
...
template <>
class PSSR_MEM_BaseWithHashId<false> : public PSSR_MEM_Base
{
};
...
public:
    ...
    {
        return std::string(ALLOW_RECOVERY ? "PSSR-" : "PSS-") + MGF::StaticAlgorithmName();
    }
};
