...
protected:
    ...
    {
        MAGIC1 = 0x1f,
        MAGIC2 = 0x8b, // flags for the header
        DEFLATED = 8,
        FAST = 4,
        SLOW = 2
    };
...
public:
    ...
    public:
        HeaderErr()
            : Err(INVALID_DATA_FORMAT, "Gunzip: header decoding error")
        {
        }
    };
    //! \class TailErr
    //! \brief Exception thrown when the tail is too short
    class TailErr : public Err
    {
    public:
        TailErr()
            : Err(INVALID_DATA_FORMAT, "Gunzip: tail too short")
        {
        }
    };
    //! \class CrcErr
    ...
    public:
        CrcErr()
            : Err(DATA_INTEGRITY_CHECK_FAILED, "Gunzip: CRC check error")
        {
        }
    };
    //! \class LengthErr
    ...
    public:
        LengthErr()
            : Err(DATA_INTEGRITY_CHECK_FAILED, "Gunzip: length check error")
        {
        }
    };
