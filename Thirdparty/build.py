...
invoked_builders = {}
modules_root = os.path.join(os.getcwd(), '..', 'Modules')
...
def get_lib_root(name):
    mname, lname = os.path.split(name)
    if not mname:
        return lname
    return os.path.join(modules_root, mname, 'Libs', lname)
...
def import_library_builder_module(name):
    ...
    if name not in imported_builders:
        ...
        if not os.access(builder_file_path, os.F_OK):
            builder_file_path = os.path.join(get_lib_root(name), 'build.py')
...
def get_module_libraries():
    libs = []
    for module in os.listdir(modules_root):
        try:
            for lib in os.listdir(os.path.join(modules_root, module, 'Libs')):
                if os.access(os.path.join(modules_root, module, 'Libs', lib, 'build.py'), os.F_OK):
                    libs.append(os.path.join(module, lib))
        except OSError:
            continue
    return libs


def get_mandatory_libraries():
    return [folder_name for folder_name in os.listdir(os.getcwd())
            if os.access(os.path.join(folder_name, 'build.py'), os.F_OK)]


def get_all_libraries():
    return get_module_libraries() + get_mandatory_libraries()
