...
def _download_and_extract(working_directory_path):
    ...
    build_utils.download_and_extract(source_url, working_directory_path, source_folder_path, 'FFmpeg-n3.1.4')
    build_utils.download_and_extract(c99_to_c89_url, os.path.join(working_directory_path, 'c99_to_c89_folder_path'), c99_to_c89_folder_path)
...
def _patch_sources(source_folder_path, working_directory_path):
    ...
    shutil.copyfile('inttypes.h', os.path.join(source_folder_path, 'inttypes.h'))
    shutil.copyfile('stdint.h', os.path.join(source_folder_path, 'stdint.h'))

    _patch_sources.did = True

def _build_win32(working_directory_path, root_project_path):
    source_folder_path, c99_to_c89_folder_path = _download_and_extract(working_directory_path)
    _patch_sources(source_folder_path, working_directory_path)

    install_path_debug_x86 = os.path.join(working_directory_path, 'gen/install_win32_debug_x86')
    ...
    configure_args_debug_x86 = configure_args_debug_template.format(_path_to_msys_path(install_path_debug_x86))
    bash_arg = bash_arg_template.format(_path_to_msys_path(source_folder_path), configure_args_debug_x86)
    cmd = msys2_cmd_template[:]
    cmd.append(bash_arg)
    build_utils.run_process(cmd, process_cwd=source_folder_path, environment=env_x86)

    configure_args_release_x86 = configure_args_release_template.format(_path_to_msys_path(install_path_release_x86))
    bash_arg = bash_arg_template.format(_path_to_msys_path(source_folder_path), configure_args_release_x86)
    cmd = msys2_cmd_template[:]
    cmd.append(bash_arg)
    build_utils.run_process(cmd, process_cwd=source_folder_path, environment=env_x86)

    configure_args_debug_x64 = configure_args_debug_template.format(_path_to_msys_path(install_path_debug_x64))
    bash_arg = bash_arg_template.format(_path_to_msys_path(source_folder_path), configure_args_debug_x64)
    cmd = msys2_cmd_template[:]
    cmd.append(bash_arg)
    build_utils.run_process(cmd, process_cwd=source_folder_path, environment=env_x64)

    configure_args_release_x64 = configure_args_release_template.format(_path_to_msys_path(install_path_release_x64))
    bash_arg = bash_arg_template.format(_path_to_msys_path(source_folder_path), configure_args_release_x64)
    cmd = msys2_cmd_template[:]
    cmd.append(bash_arg)
    build_utils.run_process(cmd, process_cwd=source_folder_path, environment=env_x64)
    ...
    _copy_libraries(install_path_debug_x64, os.path.join(root_project_path, 'Libs/lib_CMake/win/x64/Debug/'))
    _copy_libraries(install_path_release_x64, os.path.join(root_project_path, 'Libs/lib_CMake/win/x64/Release/'))
