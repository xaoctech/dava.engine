    ...
    if (placeHolderRoot)
    {
        ...
        if (testedControl)
        {
            ...
            if (options->GetOption("-isFlow").AsBool())
            {
                ...
                UIFlowStateSystem* stateSystem = scene->GetSystem<UIFlowStateSystem>();
                UIFlowStateComponent* state = stateSystem->FindStateByPath(testedControlPath);
                stateSystem->ActivateState(state != nullptr ? state : testedControl->GetComponent<UIFlowStateComponent>(), false);
            }
            else
            {
