...
void InitializeScene(DAVA::Scene& scene)
{
    ...
    const char* environmentScenePath = "~res:/3d/Maps/56_pubg/landscape/56_pubg.sc2";
    ...
    if (IsServer(&scene))
    {
        for (uint32 i = 0; i < SHOOTER_NUM_CARS; ++i) //if SHOOTER_NUM_CARS is const uint32, XCode compiler generates error here
        {
