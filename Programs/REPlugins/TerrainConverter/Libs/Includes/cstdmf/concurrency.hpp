...
public:
	...
	void push( int count = 1 ) { value_+=count; }
	void pull() { value_--; }
	bool pullTry()
	{
		if (value_ <= 0)
...
#elif defined( _WIN32 ) // !MF_SINGLE_THREADED
...
#else // !defined( MF_SINGLE_THREADED) && !defined( _WIN32 )
...
/// thread local declaration - ie "static THREADLOCAL(type) blah"
// Blitz BW 2.9, _pthread style TLS is not very well support on android
