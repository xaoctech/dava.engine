...
#ifdef WINRT
    return 0;//TODO: find a way to get user id for win uap application
#elif defined( _WIN32 )
	// VS2005:
	#if _MSC_VER >= 1400
		char uid[16];
...
// It may look strange that bw_sprintf and bw_snprintf have the same number of parameters,
//  but 'count' argument for bw_snprintf seemed redundant - at the moment there's no case
//  in BW source where count != sizeOfBuffer
// On the other hand, introducing 'count' parameter could emphasise the fact, that bw_snprintf
//  is truncating function, while bw_sprintf only needs sizeOfBuffer for bounds check (right now
//  only in msvc implementation)
inline int bw_vsprintf( char * buffer, size_t sizeOfBuffer, const char * format, va_list arg );
inline int bw_sprintf( char * buffer, size_t sizeOfBuffer, const char * format, ... )
{
    va_list args;
    va_start( args, format );
    int result = bw_vsprintf( buffer, sizeOfBuffer, format, args );
    va_end( args );
    return result;
}
...
inline char * bw_strcat( char * dest, size_t destSize, const char * src )
{
    return strcat( dest, src );
}

inline char * bw_strcpy( char * dest, size_t destSize, const char * src )
{
    return strcpy( dest, src );
}

inline char * bw_strncpy( char * dest, size_t destSize, const char * src, size_t count )
{
// This implementation behaves like strncpy_s, i.e. will not pad 'dest' with zeroes if 'src'
// is shorter than 'count' and will null-terminate 'dest' in the opposite case.
// TODO: it may also be worth to assert that 'count' is less than 'destSize'.
    if (destSize > 0)
    {
        dest[0] = '\0';
...
inline char * bw_strerror( int errorCode )
{
...
inline char	* bw_strerror( int errorCode )
{
    static char errbuf[1024];//is not thread-safe, but neither strerror is
    strerror_s(errbuf, sizeof(errbuf), errorCode);
    return errbuf;
}

inline int bw_vsprintf( char * buffer, size_t sizeOfBuffer, const char * format, va_list arg )
{
    return vsprintf_s(buffer, sizeOfBuffer, format, arg);
}
		...
		va_list args )
{
	...
	if (size != 0)
	{
		result = _vsnprintf_s( dest, size, _TRUNCATE, format, args );
	}

	if (result == -1)
	{
		// If truncated, then return the count, like the standard
		...
		va_list args )
{
	...
	if (size != 0)
	{
		result = _vsnwprintf_s( dest, size - 1, _TRUNCATE, format, args );
	}
