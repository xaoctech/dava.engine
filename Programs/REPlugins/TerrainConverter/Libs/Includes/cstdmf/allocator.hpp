...
namespace Allocator
{
	...
	/// Print currently allocated memory chunks in order of allocation.
	/// If file is not NULL then output is performed to it, otherwise printing
	/// is done to debug output.
	/// \returns the number of live allocations (requires ENABLE_MEMORY_DEBUG)
	size_t debugReportOrdered(FILE* file = 0);
	...
	void saveSmartPointersToFile( const char* filename );
