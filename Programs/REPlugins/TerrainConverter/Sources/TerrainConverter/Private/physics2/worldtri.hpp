...
{
public:
	typedef DAVA::uint16 Flags;
	typedef DAVA::uint16 Padding;

	// construction

	WorldTriangle( const DAVA::Vector3 & v0,
	...
	bool intersects( const DAVA::Vector3 &	start,
	...
	// utility
    DAVA::Vector2 project( const DAVA::Vector3 & onTri ) const;

	const DAVA::Vector3 & v0() const;
	const DAVA::Vector3 & v1() const;
	const DAVA::Vector3 & v2() const;
	const DAVA::Vector3 & v(DAVA::uint32 index ) const;

    DAVA::Vector3 normal() const;

	void bounce(DAVA::Vector3 & v,
    ...
    DAVA::uint32	collisionFlags() const	{ return collisionFlags( flags_ ); }
    DAVA::uint32	materialKind() const	{ return materialKind( flags_ ); }
...
#ifdef BW_WORLDTRIANGLE_DEBUG
	static void	BeginDraw( const Matrix4& transform, uint32 lifeTime = 0,
...
private:
	...
	static Matrix4	s_debugTriangleTransform_;
