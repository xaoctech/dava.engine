 ...
 */
INLINE
PlaneEq::PlaneEq( const DAVA::Vector3 & normal,
				 const float d ) :
	normal_( normal ),
	d_( d )
{
 ...
 */
INLINE
PlaneEq::PlaneEq( const DAVA::Vector3 & point,
 ...
 */
INLINE
void PlaneEq::init( const DAVA::Vector3 & point,
				 const DAVA::Vector3 & normal )
{
	normal_ = normal;
	d_ = normal.DotProduct( point );
 ...
 *	@param Normalize	If true, the normal is normalised.
 */
INLINE
PlaneEq::PlaneEq( const DAVA::Vector3 & p0,
	const DAVA::Vector3 & p1,
	const DAVA::Vector3 & p2,
	ShouldNormalise Normalize )
{
	this->init( p0, p1, p2, Normalize );
}
 ...
 */
INLINE
void PlaneEq::init( const DAVA::Vector3 & p0,
	const DAVA::Vector3 & p1,
	const DAVA::Vector3 & p2,
	ShouldNormalise Normalize )
{
//	normal_ = p1 - p0;
//	normal_.CrossProduct(p2 - p0);

	normal_.CrossProduct(p1 - p0, p2 - p0);

	if (Normalize == SHOULD_NORMALISE)
		normal_.Normalize();
 ...
 */
INLINE
const DAVA::Vector3 & PlaneEq::normal() const
{
	return this->normal_;
}
...
float PlaneEq::y(float x, float z) const
{
	...
	return (d_ - normal_.x * x - normal_.z * z) / normal_.y;
}
 ...
 */
INLINE
float PlaneEq::distanceTo(const DAVA::Vector3 & point) const
{
 ...
 */
INLINE
bool PlaneEq::isInFrontOf(const DAVA::Vector3 & point) const
{
 ...
 */
INLINE
bool PlaneEq::isInFrontOfExact(const DAVA::Vector3 & point) const
{
	return normal_.DotProduct(point) > d_;
}
 ...
 */
INLINE
DAVA::Vector3 PlaneEq::intersectRay( const DAVA::Vector3 & source,
	const DAVA::Vector3 & dir ) const
{
	return source +
 ...
 */
INLINE
float PlaneEq::intersectRayHalf( const DAVA::Vector3 & source,
 ...
 */
INLINE
float PlaneEq::intersectRayHalfNoCheck( const DAVA::Vector3 & source,
