 ...
 */
// void BoundingBox::calculateOutcode( const Matrix4 & m ) const
// {
// 	oc_ = 0;
 ...
 */
// void BoundingBox::transformBy( const Matrix4 & transform )
// {
...
// 			size.data[axis] * transform.applyToUnitAxisVector(axis);
//
...
// 			// appropriate point.
//
// 			if (transformedAxis.data[resultDirection] > 0)
// 			{
// 				max_.data[resultDirection] += transformedAxis.data[resultDirection];
// 			}
// 			else
// 			{
// 				min_.data[resultDirection] += transformedAxis.data[resultDirection];
// 			}
// 		}
// 	}
// }
 ...
 */
bool BoundingBox::intersectsRay(const DAVA::Vector3& origin, const DAVA::Vector3& dir) const
{
    DAVA::Vector3 bounds[2] = { min_, max_ };
    DAVA::Vector3 ptOnPlane;
    ...
    for (int bound = 0; bound < 2; bound++)
    {
        for (int axis = X_AXIS; axis <= Z_AXIS; axis++)
        {
            ...
            {
                float t = (bounds[bound].data[axis] - origin.data[axis]) / dir.data[axis];
                ...
                if (t > 0.f)
                {
                    ...
                    if (min_.data[nextAxis] < ptOnPlane.data[nextAxis] && ptOnPlane.data[nextAxis] < max_.data[nextAxis] &&
                        min_.data[prevAxis] < ptOnPlane.data[prevAxis] && ptOnPlane.data[prevAxis] < max_.data[prevAxis])
                    {
                        return true;
                    }
                }
 ...
 */
bool BoundingBox::intersectsLine(const DAVA::Vector3& origin, const DAVA::Vector3& dest) const
{
    const DAVA::Vector3 direction = dest - origin;
    DAVA::Vector3 bounds[2] = { this->minBounds(), this->maxBounds() };
    DAVA::Vector3 ptOnPlane;
    ...
    for (int bound = 0; bound < 2; bound++)
    {
        for (int axis = X_AXIS; axis <= Z_AXIS; axis++)
        {
            if (direction.data[axis] != 0.f)
            {
                float t = (bounds[bound].data[axis] - origin.data[axis]) / direction.data[axis];
 ...
 *			(like intersectsLine)
 */
bool BoundingBox::clip(DAVA::Vector3& source, DAVA::Vector3& extent, float bloat) const
{
    DAVA::Vector3 mt; // min params
    DAVA::Vector3 Mt; // max params

    float sMin = 0.f;
    float eMax = 1.f;

    DAVA::Vector3 delta = extent - source;
 ...
 */
float BoundingBox::distance(const DAVA::Vector3& point) const
{
    DAVA::Vector3 p(0, 0, 0);
