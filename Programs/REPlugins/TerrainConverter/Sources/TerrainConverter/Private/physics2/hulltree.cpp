...
void HullTree::add(const HullBorder& border, const HullContents* tag)
{
    ...
    {
        DAVA::Vector<DAVA::Vector2> outline;
        ...
        for (HullBorder::const_iterator bt = border.begin(); bt != bend; bt++)
        {
            ...
            if (aline.normal() != DAVA::Vector2::Zero)
        ...
        for (unsigned int i = 0; i < lines.size(); i++)
        {
            ...
            DAVA::Vector2 lineEnd = lines[i].param(maxDist);
            // add them to the outline (but in the right order!)
            outline.push_back(lineEnd);
        }
 ...
 */
const HullContents* HullTree::testPoint(const DAVA::Vector3& point) const
{
...
long HullTree::size(int depth, bool /*back*/) const
{
    ...
    if (pFront_ != NULL)
        sz += pFront_->size(depth + 1, false);
    if (pBack_ != NULL)
        sz += pBack_->size(depth + 1, false);
    return sz;
}
...
HullTree::Portal3D::Portal3D(const PlaneEq& plane, DAVA::Vector<DAVA::Vector2>& portal)
{
    for (size_t i = 0; i < portal.size(); i++)
    {
        points_.push_back(plane.param(portal[i]));
    }
}
// hulltree.cpp
