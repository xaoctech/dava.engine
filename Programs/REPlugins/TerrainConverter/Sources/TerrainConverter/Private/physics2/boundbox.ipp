 ...
 */
INLINE BoundingBox::BoundingBox( const DAVA::Vector3 & min, const DAVA::Vector3 & max ) :
	min_( min ),
	max_( max ),
	oc_( 0 ),
	combinedOc_( 0)
 ...
 */
INLINE const DAVA::Vector3 & BoundingBox::minBounds() const
{
	DVASSERT( !this->insideOut() );
 ...
 */
INLINE const DAVA::Vector3 & BoundingBox::maxBounds() const
{
	DVASSERT( !this->insideOut() );
 ...
 */
INLINE void BoundingBox::setBounds( const DAVA::Vector3 & min, const DAVA::Vector3 & max )
{
	min_ = min;
	max_ = max;
}
 ...
 */
INLINE void BoundingBox::addBounds( const DAVA::Vector3 & v )
{
	if( this->insideOut() )
	{
		min_ = v;
	...
	if (min_.data[0] > v.data[0])
		min_.data[0] = v.data[0];

	if (min_.data[1] > v.data[1])
		min_.data[1] = v.data[1];

	if (min_.data[2] > v.data[2])
		min_.data[2] = v.data[2];

	if (max_.data[0] < v.data[0])
		max_.data[0] = v.data[0];

	if (max_.data[1] < v.data[1])
		max_.data[1] = v.data[1];

	if (max_.data[2] < v.data[2])
		max_.data[2] = v.data[2];
}
 ...
 * you'd expect.
 */
INLINE void BoundingBox::expandSymmetrically( const DAVA::Vector3 & v )
{
...
INLINE void BoundingBox::addBounds( const BoundingBox & bb )
{
	...
	if (min_.data[0] > bb.min_.data[0])
		min_.data[0] = bb.min_.data[0];

	if (min_.data[1] > bb.min_.data[1])
		min_.data[1] = bb.min_.data[1];

	if (min_.data[2] > bb.min_.data[2])
		min_.data[2] = bb.min_.data[2];

	if (max_.data[0] < bb.max_.data[0])
		max_.data[0] = bb.max_.data[0];

	if (max_.data[1] < bb.max_.data[1])
		max_.data[1] = bb.max_.data[1];

	if (max_.data[2] < bb.max_.data[2])
		max_.data[2] = bb.max_.data[2];
}
...
{
	const DAVA::Vector3 & min1 = this->min_;
	const DAVA::Vector3 & max1 = this->max_;

	const DAVA::Vector3 & min2 = box.min_;
	const DAVA::Vector3 & max2 = box.max_;

	return(
		intervalsIntersect(min1.data[0], max1.data[0], min2.data[0], max2.data[0]) &&
		intervalsIntersect(min1.data[1], max1.data[1], min2.data[1], max2.data[1]) &&
		intervalsIntersect(min1.data[2], max1.data[2], min2.data[2], max2.data[2]));
}
 ...
 */
INLINE bool BoundingBox::intersects( const DAVA::Vector3 & v ) const
{
	return ( v.data[ 0 ] >= min_.data[ 0 ] ) && ( v.data[ 1 ] >= min_.data[ 1 ] ) && ( v.data[ 2 ] >= min_.data[ 2 ] ) &&
		   ( v.data[ 0 ] <  max_.data[ 0 ] ) && ( v.data[ 1 ] <  max_.data[ 1 ] ) && ( v.data[ 2 ] <  max_.data[ 2 ] );
}
 ...
 */
INLINE bool BoundingBox::intersects( const DAVA::Vector3 &v, float bias ) const
{
	return ( v.data[ 0 ] >= ( min_.data[ 0 ] - bias ) ) && ( v.data[ 1 ] >= ( min_.data[ 1 ] - bias ) ) && ( v.data[ 2 ] >= ( min_.data[ 2 ] - bias ) ) &&
		   ( v.data[ 0 ] <  ( max_.data[ 0 ] + bias ) ) && ( v.data[ 1 ] <  ( max_.data[ 1 ] + bias ) ) && ( v.data[ 2 ] <  ( max_.data[ 2 ] + bias ) );
}
 ...
 */
INLINE DAVA::Vector3 BoundingBox::centre() const
{
	DVASSERT( !this->insideOut() );
