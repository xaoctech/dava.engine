...
DAVA::RenderObject* CreateRenderObject(const DAVA::FilePath& scenePath)
{
    ...
    if (result == SceneFileV2::ERROR_NO_ERROR)
    {
        ...
        scene->GetChildEntitiesWithComponent(entities, DAVA::Type::Instance<DAVA::MeshComponent>());
        if (entities.size() == 1)
        {
...
{
    //GFX_COMPLETE auto-convert user-node sc2?
    /*
...
const DAVA::Matrix4* UserNodeSystem::GetWorldTransformPtr(DAVA::Entity* entity) const
{
    using namespace DAVA;
