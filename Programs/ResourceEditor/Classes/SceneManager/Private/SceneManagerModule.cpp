...
void SceneManagerModule::CreateModuleActions(DAVA::UI* ui)
{
    ...
    // Save Map Action
    {
        QtAction* action = new QtAction(accessor, QString("Save Level"));
        KeyBindableActionInfo info;
        info.blockName = "File";
        info.context = Qt::WindowShortcut;
        //info.defaultShortcuts.push_back(QKeySequence("Ctrl+S"));
    ...
    // Save Map As
    {
        ...
        ActionPlacementInfo placementInfo;
        placementInfo.AddPlacementPoint(CreateMenuPoint(MenuItems::menuFile, { InsertionParams::eInsertionMethod::AfterItem, "Save Level" }));
 ...
 */
DAVA::FilePath SceneManagerModule::GetMapSavePath(const DAVA::RefPtr<DAVA::SceneEditor2>& scene)
{
    ...
    params.title = QStringLiteral("Save map as");
    params.filters = "DAVA Map (*.map)";
...
void SceneManagerModule::SaveLevel(bool saveAs)
{
    ...
    //if (saveAs == true)
    {
        saveAsPath = GetMapSavePath(data->scene);
    }
    ...
    {
        Asset<Level> level = DAVA::GetEngineContext()->assetManager->CreateNewAsset<Level>(saveAsPath);
        level->SetScene(data->scene.Get());
        level->Save(saveAsPath);
    }
