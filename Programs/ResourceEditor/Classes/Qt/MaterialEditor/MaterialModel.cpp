...
void MaterialModel::SetSelection(const DAVA::SelectableGroup* group)
{
    ...
    auto updateSelection = [&](MaterialItem* item) {
        ...
        DAVA::Vector<DAVA::Entity*> entities = curScene->GetSystem<DAVA::EditorMaterialSystem>()->GetEntities(material);
...
void MaterialModel::Sync(MaterialItem* item)
{
    ...
    for (auto it : processedList)
    {
        bool shouldAddMaterial = !it.second;
    ...
    DAVA::Vector<DAVA::ReflectedObject> owners = curScene->GetSystem<DAVA::EditorMaterialSystem>()->GetMaterialOwners(material);
    for (const DAVA::ReflectedObject& o : owners)
    {
        if (o.GetReflectedType() == DAVA::ReflectedTypeDB::Get<DAVA::RenderBatch>())
        {
            // set item lod/switch flags
            const DAVA::RenderBatch* rb = o.GetPtr<DAVA::RenderBatch>();
            if (nullptr != rb)
            {
    ...
    bool can_be_deleted = (0 == item->rowCount() && owners.empty());
