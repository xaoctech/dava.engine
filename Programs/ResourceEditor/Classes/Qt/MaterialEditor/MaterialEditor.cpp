...
private:
    ...
    void FillBase(QtPropertyData* baseRoot, DAVA::NMaterial* material)
    {
        ...
        // fill material type
        const DAVA::InspMember* typeMember = info->Member(DAVA::FastName("materialType"));
        if (nullptr != typeMember)
        {
            std::unique_ptr<QtPropertyDataInspMember> typeProperty(new QtPropertyDataInspMember(UIName::Type, material, typeMember));
            typeProperty->SetInspDescription(typeMember->Desc());
            typeProperty->SetEditable(false);
            baseRoot->ChildAdd(std::move(typeProperty));
        }
    ...
    void FillDynamicMembers(QtPropertyData* root, DAVA::InspInfoDynamic* dynamic, DAVA::NMaterial* material, bool isGlobal, DAVA::KeyedArchive* hiddenProps)
    {
        ...
        for (size_t i = 0; i < membersList.size(); ++i)
        {
            ...
            std::unique_ptr<QtPropertyDataInspDynamic> child(new QtPropertyDataInspDynamic(name, dynamic, ddata));
            DAVA::InspDesc descr = dynamic->MemberDesc(ddata, name);
            if (descr.enumMap != nullptr)
            {
                for (size_t i = 0; i < descr.enumMap->GetCount(); ++i)
                {
                    DAVA::int32 value;
                    descr.enumMap->GetValue(i, value);
                    QString visibleValue(descr.enumMap->ToString(value));
                    child->AddAllowedValue(DAVA::VariantType(value), visibleValue);
                }
            }
            root->MergeChild(std::move(child));
        }
    }
...
void MaterialEditor::initActions()
{
    ...
    QObject::connect(ui->actionRemoveMaterial, SIGNAL(triggered()), this, SLOT(onRemoveMaterial()));

    QToolButton* addMaterialButton = dynamic_cast<QToolButton*>(ui->toolbar->widgetForAction(ui->actionAddMaterial));
    addMaterialButton->removeAction(ui->actionAddMaterial);
...
void MaterialEditor::initTemplates()
{
    if (nullptr == templatesFilterModel)
    {
        ...
        for (const DAVA::MaterialTemplateInfo& t : *templates)
        {
            ...
            item->setData(int(t.materialType), Qt::UserRole + 1);
...
DAVA::Set<DAVA::NMaterial*> MaterialEditor::GetSelection() const
{
    DAVA::Set<DAVA::NMaterial*> selection;
    foreach (DAVA::NMaterial* material, curMaterials)
        selection.insert(material);
...
void MaterialEditor::commandExecuted(DAVA::SceneEditor2* scene, const DAVA::RECommandNotificationObject& commandNotification)
{
    ...
    if (commandNotification.MatchCommandTypes<DAVA::MaterialGlobalSetCommand, DAVA::MaterialAddCommand, DAVA::MaterialRemoveCommand>())
    {
    ...
    if (commandNotification.MatchCommandTypes<DAVA::MaterialAssignCommand>())
    {
        UpdateToolbar();
    }
...
void MaterialEditor::FillTemplates(const QList<DAVA::NMaterial*>& materials)
{
    ...
    if ((materials.size() == 1) && (materials.first() != nullptr))
    {
        DAVA::NMaterial* material = materials.first();
        ...
        {
            templatesFilterModel->SetSelectedMaterialType(material->GetMaterialType());
            {
    ...
    //////////////////////////////////////////////////////////////////////////
    // Some magic with Qt to hide scroll-bars in combo-box when element count is reduced

    QAbstractItemView* view = ui->templateBox->view();
    QWidget* parent = view->parentWidget();
    DVASSERT(parent != nullptr);
    QLayout* parentLayout = parent->layout();
    DVASSERT(parentLayout != nullptr);

    for (int i = 0; i < parentLayout->count(); ++i)
    {
        QLayoutItem* item = parentLayout->itemAt(i);
        QWidget* widget = item->widget();
        if (widget != nullptr && widget != view)
        {
            widget->hide();
        }
    }
    ...
    if (1 == curMaterials.size())
    {
        ...
        if (nullptr != templateMember)
        {
            ...
            else
            {
                ...
                if (!fxName.IsValid())
                    fxName = DAVA::FastName("");

                activeScene->Exec(std::unique_ptr<DAVA::Command>(new DAVA::InspMemberModifyCommand(templateMember, material, DAVA::VariantType(fxName))));
            }
...
void MaterialEditor::onAddMaterial(QAction* action)
{
    ...
    activeScene->BeginBatch("Add new material");
    activeScene->Exec(std::unique_ptr<DAVA::Command>(new DAVA::MaterialAddCommand(material)));
    activeScene->EndBatch();
...
void MaterialEditor::onRemoveMaterial()
{
    DVASSERT(activeScene != nullptr);

    DAVA::uint32 count = static_cast<DAVA::uint32>(curMaterials.size());
    activeScene->BeginBatch("Remove material", count);
    for (DAVA::uint32 i = 0; i < count; ++i)
    {
        DAVA::NMaterial* material = curMaterials[i];
...
{
    bool removeEnabled = (curMaterials.size() != 0);
    foreach (DAVA::NMaterial* material, curMaterials)
    {
    ...
    ui->actionRemoveMaterial->setEnabled(removeEnabled);
