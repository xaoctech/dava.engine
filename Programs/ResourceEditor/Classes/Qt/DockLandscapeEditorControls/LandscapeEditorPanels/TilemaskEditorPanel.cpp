...
void TilemaskEditorPanel::UpdateTileTextures()
{
    ...
    static const DAVA::FastName tileTextures[4] = {
        DAVA::Landscape::TEXTURE_ALBEDO_TILE0,
        DAVA::Landscape::TEXTURE_ALBEDO_TILE1,
        DAVA::Landscape::TEXTURE_ALBEDO_TILE2,
        DAVA::Landscape::TEXTURE_ALBEDO_TILE3
    };

    DAVA::Landscape* landscape = sceneEditor->GetSystem<DAVA::LandscapeEditorDrawSystem>()->GetBaseLandscape();
    for (DAVA::uint32 i = 0; i < landscape->GetLayersCount(); ++i)
    {
        ...
        for (DAVA::uint32 i = 0; i < sizeof(tileTextures) / sizeof(tileTextures[0]); ++i)
        {
            ...
            if (texture)
            {
                ...
                DAVA::ScopedPtr<DAVA::Image> image(DAVA::ImageSystem::LoadSingleMip(texturepath));
                if (image)
                {
                    image->ResizeCanvas(iconSize.width(), iconSize.height());
                    tileTexturePreviewWidget->AddTexture(image);
                }
            }
        }
    }
