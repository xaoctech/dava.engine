...
void CollectMaterialChildrenRecursive(DAVA::NMaterial* material, DAVA::Set<DAVA::NMaterial*>* children)
{
    if (material == nullptr)
        return;

    for (DAVA::NMaterial* child : material->GetChildren())
    {
        children->insert(child);
        CollectMaterialChildrenRecursive(child, children);
    }
}
}
...
void DeveloperTools::OnDebugFunctionsGridCopy()
{
    ...
    float32 z = 0;
    const float32 xshift = 25.0f;
    const float32 yshift = 25.0f;
    const float32 zshift = 0.0;

    FastName var0("metallnessScale");
    FastName var1("roughnessScale");
    ...
    if (selection.GetSize() == 1)
    {
        ...
        for (uint32 x = 0; x < 10; ++x)
        {
            for (uint32 y = 0; y < 10; ++y)
            {
                ...
                float32 inGlossiness = (float32)x / 9.0f;
                float32 inSpecularity = (float32)y / 9.0f;

                if (material->HasLocalProperty(var0))
                    material->SetPropertyValue(var0, &inGlossiness);
                else
                    material->AddProperty(var0, &inGlossiness, rhi::ShaderProp::TYPE_FLOAT1);

                if (material->HasLocalProperty(var1))
                    material->SetPropertyValue(var1, &inSpecularity);
                else
                    material->AddProperty(var1, &inSpecularity, rhi::ShaderProp::TYPE_FLOAT1);
...
void DeveloperTools::OnReloadLandscapeDependencies()
{
    using namespace DAVA;

    Set<NMaterial*> materialsToReload;
    SceneEditor2* currentScene = DeveloperToolsDetails::GetActiveScene();
    for (Landscape* landscape : currentScene->landscapeSystem->GetLandscapeObjects())
    {
        landscape->ReloadDecorationData();

        for (uint32 i = 0; i < landscape->GetLayersCount(); ++i)
        {
            uint32 pageMaterialCount = landscape->GetPageMaterialCount(i);
            for (uint32 j = 0; j < pageMaterialCount; ++j)
                materialsToReload.insert(landscape->GetPageMaterials(i, j));
        }

        NMaterial* landscapeMaterial = landscape->GetLandscapeMaterial();
        if (landscapeMaterial != nullptr)
        {
            materialsToReload.insert(landscapeMaterial);
            DeveloperToolsDetails::CollectMaterialChildrenRecursive(landscapeMaterial, &materialsToReload);
        }
    }
