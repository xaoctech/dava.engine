...
{
    lightViewActionGroup = new QActionGroup(ui->menuLightView);
    lightViewActionGroup->addAction(ui->actionViewBaseColor);
    lightViewActionGroup->addAction(ui->actionViewRoughness);
    lightViewActionGroup->addAction(ui->actionViewMetallness);
    lightViewActionGroup->addAction(ui->actionViewAO);
    lightViewActionGroup->addAction(ui->actionViewShadow);
    lightViewActionGroup->addAction(ui->actionViewNormals);
    lightViewActionGroup->addAction(ui->actionViewTransmittance);
    QObject::connect(lightViewActionGroup, SIGNAL(triggered(QAction*)), this, SLOT(OnMaterialLightViewChanged(QAction*)));

    QObject::connect(ui->actionViewCombinedResult, SIGNAL(triggered(bool)), this, SLOT(OnMaterialLightViewAll(bool)));
    QObject::connect(ui->actionViewCombinedDirectLighting, SIGNAL(toggled(bool)), this, SLOT(OnMaterialLightViewChanged(bool)));
    QObject::connect(ui->actionViewCombinedIndirectLighting, SIGNAL(toggled(bool)), this, SLOT(OnMaterialLightViewChanged(bool)));
    QObject::connect(ui->actionViewCombinedAmbientOcclusion, SIGNAL(toggled(bool)), this, SLOT(OnMaterialLightViewChanged(bool)));
    QObject::connect(ui->actionViewCombinedTransmittance, SIGNAL(toggled(bool)), this, SLOT(OnMaterialLightViewChanged(bool)));
    QObject::connect(ui->actionViewCombinedBaseColor, SIGNAL(toggled(bool)), this, SLOT(OnMaterialLightViewChanged(bool)));
    QObject::connect(ui->actionViewCombinedNormalMapping, SIGNAL(toggled(bool)), this, SLOT(OnMaterialLightViewChanged(bool)));
    QObject::connect(ui->actionViewCombinedDiffuse, SIGNAL(toggled(bool)), this, SLOT(OnMaterialLightViewChanged(bool)));
    QObject::connect(ui->actionViewCombinedSpecular, SIGNAL(toggled(bool)), this, SLOT(OnMaterialLightViewChanged(bool)));

    QObject::connect(ui->actionViewLightmapCanvas, SIGNAL(triggered(bool)), this, SLOT(OnMaterialLightViewLightmapCanvas(bool)));
    QObject::connect(ui->actionViewTextureMipLevel, SIGNAL(triggered(bool)), this, SLOT(OnMaterialLightViewTextureMipLevel(bool)));
...
void QtMainWindow::ApplyMaterialLightViewMode(DAVA::int32 curViewMode)
{
    ...
    {
        MainWindowDetails::GetCurrentScene()->GetSystem<EditorMaterialSystem>()->SetLightViewMode(curViewMode);
    }
}
...
{
    UpdateMaterialLightViewMode(nullptr);
}

void QtMainWindow::OnMaterialLightViewChanged(QAction* sender)
{
    UpdateMaterialLightViewMode(sender);
}

void QtMainWindow::OnMaterialLightViewAll(bool)
{
    ApplyMaterialLightViewMode(DAVA::CommonInternalSettings::RESOLVE_RESULT);
}

void QtMainWindow::OnMaterialLightViewLightmapCanvas(bool)
{
    ApplyMaterialLightViewMode(DAVA::CommonInternalSettings::VIEW_BY_COMPONENT_BIT | DAVA::CommonInternalSettings::VIEW_STATIC_SHADOWS | DAVA::CommonInternalSettings::VIEW_LIGHTMAP_CANVAS_BIT);
}
...
void QtMainWindow::OnForceFirstLod(bool enabled)
{
    ...
    landscape->SetForceMinSubdiv(enabled ? 4 : 0);
...
DAVA::uint32 QtMainWindow::GetInvalidEntries(DAVA::PolygonGroup* geometry)
{
    if (geometry == nullptr)
        return 0;

    auto floatValid = [](float t) {
        return std::isfinite(t);
    };
    auto vector3Valid = [&floatValid](const DAVA::Vector3& v) {
        return floatValid(v.x) && floatValid(v.y) && floatValid(v.z) && (v.SquareLength() > std::numeric_limits<float>::epsilon());
    };

    DAVA::uint32 result = 0;

    for (DAVA::int32 i = 0; i < geometry->GetVertexCount(); ++i)
    {
        if (geometry->GetFormat() & DAVA::EVF_NORMAL)
        {
            DAVA::Vector3 n;
            geometry->GetNormal(i, n);
            if (!vector3Valid(n))
                result |= DAVA::EVF_NORMAL;
        }
        if (geometry->GetFormat() & DAVA::EVF_TANGENT)
        {
            DAVA::Vector3 n;
            geometry->GetTangent(i, n);
            if (!vector3Valid(n))
                result |= DAVA::EVF_TANGENT;
        }
        if (geometry->GetFormat() & DAVA::EVF_BINORMAL)
        {
            DAVA::Vector3 n;
            geometry->GetBinormal(i, n);
            if (!vector3Valid(n))
                result |= DAVA::EVF_BINORMAL;
        }
    }
...
void QtMainWindow::GatherEntitiesWithCorruptGeometry(DAVA::Entity* base, DAVA::Vector<CorruptGeometryInfo>& objects)
{
    DAVA::RenderObject* ro = DAVA::GetRenderObject(base);
    if (ro != nullptr)
    {
        for (DAVA::uint32 i = 0; i < ro->GetRenderBatchCount(); ++i)
        {
            DAVA::RenderBatch* batch = ro->GetRenderBatch(i);
            DAVA::uint32 corruptFormat = GetInvalidEntries(batch->GetPolygonGroup());
            if (corruptFormat != 0)
            {
                objects.emplace_back();
                objects.back().entity = base;
                objects.back().boundingBox = ro->GetWorldBoundingBox();
                objects.back().corruptFormat = corruptFormat;
                objects.back().corruptBatchIndex = i;
                break;
            }
        }
    }

    for (DAVA::int32 i = 0; i < base->GetChildrenCount(); ++i)
        GatherEntitiesWithCorruptGeometry(base->GetChild(i), objects);
}

void QtMainWindow::OnValidateGeometry()
{
    DAVA::Vector<CorruptGeometryInfo> corruptObjects;
    corruptObjects.reserve(256);
    DAVA::RefPtr<DAVA::SceneEditor2> currentScene = MainWindowDetails::GetCurrentScene();

    GatherEntitiesWithCorruptGeometry(currentScene.Get(), corruptObjects);
    if (corruptObjects.empty())
    {
        DAVA::NotificationParams params;
        params.message.message = "Good news! Your scene does not have invalid objects";
        params.message.type = DAVA::Result::RESULT_SUCCESS;
        params.title = "Geometry Validator";
        DAVA::Deprecated::ShowNotification(params);
        return;
    }

    if (corruptGeometryObjectIndex >= corruptObjects.size())
        corruptGeometryObjectIndex = 0;

    const CorruptGeometryInfo& info = corruptObjects[corruptGeometryObjectIndex];

    DAVA::int32 printPos = 0;
    char messageBuffer[1024] = {};
    if (info.corruptFormat & DAVA::EVF_NORMAL)
        printPos += sprintf(messageBuffer + printPos, "- Normals\n");
    if (info.corruptFormat & DAVA::EVF_TANGENT)
        printPos += sprintf(messageBuffer + printPos, "- Tangents\n");
    if (info.corruptFormat & DAVA::EVF_BINORMAL)
        printPos += sprintf(messageBuffer + printPos, "- Binormals\n");

    DAVA::NotificationParams params;
    params.message.message = DAVA::Format("Entity %s has invalid geometry in batch index %u. Invalid geometry:\n%s",
                                          info.entity->GetName().c_str(), info.corruptBatchIndex, messageBuffer);
    params.message.type = DAVA::Result::RESULT_ERROR;
    params.title = "Geometry Validator";
    DAVA::Deprecated::ShowNotification(params);

    DAVA::SelectableGroup newSelection;
    newSelection.Add(info.entity, info.boundingBox);
    newSelection.RebuildIntegralBoundingBox();

    currentScene->GetSystem<DAVA::SelectionSystem>()->SetSelection(newSelection);
    currentScene->GetSystem<DAVA::SceneCameraSystem>()->LookAt(newSelection.GetIntegralBoundingBox());

    ++corruptGeometryObjectIndex;
}
