...
public slots:
    ...
    void OnTiledTextureRetreived(DAVA::Landscape* landscape, DAVA::Asset<DAVA::Texture> landscapeTexture);
    ...
    void OnMaterialLightViewChanged(QAction*);
    void OnMaterialLightViewAll(bool);
    void OnMaterialLightViewLightmapCanvas(bool);
    void OnMaterialLightViewTextureMipLevel(bool);
    ...
    void OnValidateGeometry();

private:
    struct CorruptGeometryInfo
    {
        DAVA::Entity* entity = nullptr;
        DAVA::AABBox3 boundingBox;
        DAVA::uint32 corruptFormat = 0;
        DAVA::uint32 corruptBatchIndex = DAVA::uint32(-1);
    };
    DAVA::uint32 GetInvalidEntries(DAVA::PolygonGroup* geometry);
    void GatherEntitiesWithCorruptGeometry(DAVA::Entity* base, DAVA::Vector<CorruptGeometryInfo>& objects);
...
private:
    ...
    void UpdateMaterialLightViewMode(QAction* sender);
    void ApplyMaterialLightViewMode(DAVA::int32 mode);
    ...
    size_t corruptGeometryObjectIndex = size_t(-1);
};
