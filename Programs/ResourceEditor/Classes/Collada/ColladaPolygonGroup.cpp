...
namespace DAVA
{
static const int32 VertexColorAlphaStreamSet = 9;
...
ColladaPolygonGroup::ColladaPolygonGroup(ColladaMesh* _parentMesh, FCDGeometryPolygons* _polygons, ColladaVertexWeight* vertexWeightArray, uint32 maxVertexInfluence)
{
    ...
    FCDGeometryPolygonsInput* pColorInput = polygons->FindInput(FUDaeGeometryInput::COLOR);
    FCDGeometryPolygonsInput* pColorAlphaInput = nullptr;
    ...
    FCDGeometrySource* pColorSource = polygons->GetParent()->FindSourceByType(FUDaeGeometryInput::COLOR);
    FCDGeometrySource* pColorAlphaSource = nullptr;
    ...
    if (pColorInput && pColorSource)
        vertexFormat |= EVF_COLOR;
    ...
    {
        for (size_t i = 1; i < texCoordInputList.size(); ++i)
        {
            if (texCoordInputList[i]->GetSet() != VertexColorAlphaStreamSet)
            {
                pTexCoordInput1 = texCoordInputList[i];
                pTexCoordSource1 = texCoordSourcesList[i];
        ...
        for (size_t i = 1; i < texCoordInputList.size(); ++i)
        {
            if (texCoordInputList[i]->GetSet() == VertexColorAlphaStreamSet)
            {
                pColorAlphaInput = texCoordInputList[i];
                pColorAlphaSource = texCoordSourcesList[i];
                break;
            }
        }
    }
    ...
    int colorIndexCount = -1;
    int colorAlphaIndexCount = -1;
    uint32 *vertexIndeces = 0, *normalIndeces = 0, *texIndices0 = 0, *texIndices1 = 0, *tangentIndices = 0, *binormalIndices = 0, *colorIndices = 0, *colorAlphaIndices = 0;
    ...
    if (pColorInput)
    {
        colorIndexCount = (int)pColorInput->GetIndexCount();
        colorIndices = (uint32*)pColorInput->GetIndices();
    }

    if (pColorAlphaInput)
    {
        colorAlphaIndexCount = (int)pColorAlphaInput->GetIndexCount();
        colorAlphaIndices = (uint32*)pColorAlphaInput->GetIndices();
    }

    DAVA::Logger::FrameworkDebug("ColladaPolygonGroup (V: %u, N: %u, T: %u)",
                                 vertexIndexCount, normalIndexCount, texIndexCount0);
    ...
    for (int v = 0; v < vertexIndexCount; ++v)
    {
        ...
        if (pColorSource)
        {
            colorIndex = colorIndices[v];
            p = &pColorSource->GetData()[colorIndex * 3];
            tv.color.x = p[0];
            tv.color.y = p[1];
            tv.color.z = p[2];
            tv.color.w = 0.0f;
            if (pColorAlphaSource)
                tv.color.w = pColorAlphaSource->GetData()[colorIndex * 3];
        }
	...
	*/
    ...
    DAVA::Logger::FrameworkDebug("Building optimized array: %u", uint32_t(unoptimizedVerteces.size()));

    std::vector<ColladaVertex> optVertexArray;
    optVertexArray.reserve(unoptimizedVerteces.size());
    ...
    for (int index = 0; index < (int)triangleCount * 3; ++index)
    {
        ...
        for (oIndex = 0; oIndex < optSize; ++oIndex)
        {
            ...
            if (pColorSource)
            {
                if (!FLOAT_EQUAL_EPS(tvi->color.x, tvj->color.x, EPS))
                {
                    equalIJ = false;
                }
                if (!FLOAT_EQUAL_EPS(tvi->color.y, tvj->color.y, EPS))
                {
                    equalIJ = false;
                }
                if (!FLOAT_EQUAL_EPS(tvi->color.z, tvj->color.z, EPS))
                {
                    equalIJ = false;
                }
                if (!FLOAT_EQUAL_EPS(tvi->color.w, tvj->color.w, EPS))
                {
                    equalIJ = false;
                }
            }
