...
RenderObject* ColladaImporter::GetMeshFromCollada(ColladaMeshInstance* mesh, const FastName& name)
{
    ...
    for (auto polygonGroupInstance : mesh->polyGroupInstances)
    {
        ...
        NMaterial* davaMaterial = library.GetOrCreateMaterialParent(polygonGroupInstance->material, isShadow, polygonGroupInstance->polyGroup->maxVertexInfluenceCount);
        if (isSkinnedMesh)
        {
...
void ColladaImporter::LoadMaterialParents(ColladaScene* colladaScene)
{
    ...
    {
        NMaterial* globalMaterial = library.GetOrCreateMaterialParent(cmaterial, false, 0);
...
eColladaErrorCodes ColladaImporter::SavePrefab(ColladaScene* colladaScene, const FilePath& scenePath)
{
...
bool ColladaImporter::RemoveEmptySceneNodes(DAVA::Entity* currentNode)
{
...
bool ColladaImporter::RemoveEmptyHierarchy(Entity* currentNode)
{
    for (int32 c = 0; c < currentNode->GetChildrenCount(); ++c)
    {
        Entity* childNode = currentNode->GetChild(c);
