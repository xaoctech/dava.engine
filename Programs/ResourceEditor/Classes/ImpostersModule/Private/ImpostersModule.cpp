...
#define IMPOSTER_MODULE_CREATE_DEBUG_OBJECT 0
    ...
    {
        Vector3 dx = Vector3(std::numeric_limits<float32>::epsilon(), 0.0f, 0.0f);
        Vector3 dy = Vector3(0.0f, std::numeric_limits<float32>::epsilon(), 0.0f);
    ...
    key.width = superSamplingScale * frameSize.dx * IMPOSTER_FRAME_COUNT;
    key.width = superSamplingScale * frameSize.dy;
    key.format = PixelFormat::FORMAT_RGBA8888;
    key.mipLevelsCount = 1;
    key.isDepth = true;
    key.needPixelReadback = false;
    key.ensurePowerOf2 = false;

    Asset<Texture> maskDepthTexture = assetManager->GetAsset<Texture>(key, AssetManager::SYNC);

    key.isDepth = false;
    Asset<Texture> maskTexture = assetManager->GetAsset<Texture>(key, AssetManager::SYNC);
    Asset<Texture> buffer0Texture = assetManager->GetAsset<Texture>(key, AssetManager::SYNC);
    buffer0Texture->SetWrapMode(rhi::TEXADDR_CLAMP, rhi::TEXADDR_CLAMP);

    Asset<Texture> buffer1Texture = assetManager->GetAsset<Texture>(key, AssetManager::SYNC);
    buffer1Texture->SetWrapMode(rhi::TEXADDR_CLAMP, rhi::TEXADDR_CLAMP);

    key.width = frameSize.dx * IMPOSTER_FRAME_COUNT;
    key.height = frameSize.dy;
    key.mipLevelsCount = levelCount;
    key.needPixelReadback = true;
    Asset<Texture> albedoTexture = assetManager->GetAsset<Texture>(key, AssetManager::SYNC); // frameSize.dx * IMPOSTER_FRAME_COUNT, frameSize.dy, PixelFormat::FORMAT_RGBA8888, false));
    Asset<Texture> normalTexture = assetManager->GetAsset<Texture>(key, AssetManager::SYNC); // frameSize.dx * IMPOSTER_FRAME_COUNT, frameSize.dy, PixelFormat::FORMAT_RGBA8888, false));
    ...
    Renderer::RegisterSyncCallback(sync, [albedoPath, normalPath, albedoTexture, normalTexture, material, levelCount](rhi::HSyncObject sync) mutable {
        ...
        AssetManager* assetManager = GetEngineContext()->assetManager;
        Texture::PathKey albedoPathKey(albedoPath);
        material->AddTexture(FastName("albedo"), assetManager->GetAsset<Texture>(albedoPath, AssetManager::SYNC));
        ...
        Texture::PathKey normalPathKey(albedoPath);
        material->AddTexture(FastName("normalmap"), assetManager->GetAsset<Texture>(normalPathKey, AssetManager::SYNC));
    ...
    passConfig.depthStencilBuffer.texture = maskDepthTexture->handle;
