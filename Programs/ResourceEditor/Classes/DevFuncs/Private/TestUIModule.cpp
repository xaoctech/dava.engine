...
struct ImageViewTestData : public ReflectionBase
{
    ImageViewTestData()
        : ReflectionBase()
    {
        redImage = Image::Create(256, 256, PixelFormat::FORMAT_RGBA8888);
        for (uint32 pixelPos = 0; pixelPos < redImage->dataSize; pixelPos += 4)
        {
            redImage->data[pixelPos + 0] = 255; //r;
            redImage->data[pixelPos + 1] = 0; //g;
            redImage->data[pixelPos + 2] = 0; //b;
            redImage->data[pixelPos + 3] = 255; //a;
        }

        blueImage = Image::Create(256, 512, PixelFormat::FORMAT_RGBA8888);
        for (uint32 pixelPos = 0; pixelPos < redImage->dataSize; pixelPos += 4)
        {
            blueImage->data[pixelPos + 0] = 0; //r;
            blueImage->data[pixelPos + 1] = 0; //g;
            blueImage->data[pixelPos + 2] = 255; //b;
            blueImage->data[pixelPos + 3] = 255; //a;
        }
    }

    ~ImageViewTestData() override
    {
        SafeRelease(redImage);
        SafeRelease(blueImage);
    }

    Image* zeroImage = nullptr;
    Image* redImage = nullptr;
    Image* blueImage = nullptr;
    bool zeroImageIsActive = true;
    bool isRedImageActive = true;

    DAVA_VIRTUAL_REFLECTION_IN_PLACE(ImageViewTestData, ReflectionBase)
    {
        ReflectionRegistrator<ImageViewTestData>::Begin()
        .Field("image", [](const ImageViewTestData* pThis) { return pThis->zeroImageIsActive ? pThis->zeroImage : pThis->redImage; }, nullptr)
        .Field("isZeroImage", &ImageViewTestData::zeroImageIsActive)
        .Field("imageText", [](const ImageViewTestData* pThis) { return pThis->zeroImageIsActive ? "Zero Image Enabled" : "Red Image Enabled"; }, nullptr)

        .Field("colorImage", [](const ImageViewTestData* pThis) { return pThis->isRedImageActive ? pThis->redImage : pThis->blueImage; }, nullptr)
        .Field("imageSize", [](const ImageViewTestData* pThis) { return pThis->isRedImageActive ? 32 : 256; }, nullptr)
        .Field("isRedImageActive", &ImageViewTestData::isRedImageActive)
        .Field("colorImageText", [](const ImageViewTestData* pThis) { return pThis->isRedImageActive ? "Red Image Active" : "Blue Image Active"; }, nullptr)
    ...
    static Result Create(UI* ui, ContextAccessor* accessor, QWidget* parent)
    {
        ...
        ImageViewTestData* data = new ImageViewTestData();
...

        {
            QVBoxLayout* vLayout = new QVBoxLayout();
            {
                ...
                params.fields[CheckBox::Fields::Checked] = "isZeroImage";
                params.fields[CheckBox::Fields::TextHint] = "imageText";
                CheckBox* checkBox = new CheckBox(params, accessor, Reflection::Create(data), parent);
                vLayout->addWidget(checkBox->ToWidgetCast());
            }
            {
                QHBoxLayout* lineLayout = new QHBoxLayout();
                lineLayout->addWidget(new QLabel("Simlpe Image: ", parent));

                ImageView::Params params(accessor, ui, DAVA::mainWindowKey);
                params.fields[ImageView::Fields::Image] = "image";
                ImageView* imageView = new ImageView(params, accessor, Reflection::Create(data), parent);
                lineLayout->addWidget(imageView->ToWidgetCast());
...

        {
            ...
            {
                CheckBox::Params params(accessor, ui, DAVA::mainWindowKey);
                params.fields[CheckBox::Fields::Checked] = "isRedImageActive";
                params.fields[CheckBox::Fields::TextHint] = "colorImageText";
                CheckBox* checkBox = new CheckBox(params, accessor, Reflection::Create(data), parent);
                vLayout->addWidget(checkBox->ToWidgetCast());
            }
            {
                QHBoxLayout* lineLayout = new QHBoxLayout();
                lineLayout->addWidget(new QLabel("Image with size: ", parent));

                ImageView::Params params(accessor, ui, DAVA::mainWindowKey);
                params.fields[ImageView::Fields::Image] = "colorImage";
                params.fields[ImageView::Fields::Size] = "imageSize";
                ImageView* imageView = new ImageView(params, accessor, Reflection::Create(data), parent);
                lineLayout->addWidget(imageView->ToWidgetCast());
                vLayout->addLayout(lineLayout);
            }
...
void TestUIModule::ShowDialog()
{
    ...
    {
      { &ImageViewTestData::Create, "ImageView Test" },
