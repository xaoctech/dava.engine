...
void REApplication::CreateModules(DAVA::Core* tarcCore) const
{
    ...
    bool rhiThreadedMode = item.Get("rhiThreadedMode", false);
    ...
    descr.fieldName = DAVA::FastName("rhiThreadedMode");
    renderBackEndListener->BindField(descr, [accessor](const DAVA::Any& v) {
        if (v.IsEmpty() == true)
        {
            return;
...
void REApplication::Init(const DAVA::EngineContext* engineContext)
{
    ...
    DAVA::QualitySettingsSystem::Instance()->SetForceRescale(true);
    ...
    DAVA::ProfilerOverlay::globalProfilerOverlay->SetInputEnabled(true);

    DAVA::ProfilerCPU::globalProfiler->Start();
    DAVA::ProfilerGPU::globalProfiler->Start();
