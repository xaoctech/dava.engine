...
RootProperty::~RootProperty()
{
    ...
    for (VirtualPropertiesSection* section : virtualProperties)
    {
        section->SetParent(nullptr);
        section->Release();
    }
...
{
    return static_cast<uint32>(baseProperties.size() + controlProperties.size() + componentProperties.size() + virtualProperties.size());
}
...
AbstractProperty* RootProperty::GetProperty(int index) const
{
    ...
    if (index < static_cast<int>(virtualProperties.size()))
        return virtualProperties[index];
    index -= static_cast<int>(virtualProperties.size());

    DVASSERT(false, "Index out of range!");
    return nullptr;
}
...
void RootProperty::AddVirtualSection(ComponentPropertiesSection* localSection)
{
    ...
    //if (componentType == Type::Instance<UIUserPropertiesComponent>() || componentType == Type::Instance<UIVisualScriptComponent>())
    {
        ...
        {
            VarTableValueProperty* propertiesProperty = dynamic_cast<VarTableValueProperty*>(localSection->GetProperty(idx));
            if (propertiesProperty != nullptr)
            {
                DAVA::UIComponent* component = localSection->GetComponent();
                ...
                if (it == virtualProperties.end())
                {
                    ...
                    String sectionName(Format("[%s.%s]", localSection->GetName().c_str(), propertiesProperty->GetName().c_str()));
...
void RootProperty::RemoveVirtualSection(ComponentPropertiesSection* section)
{
    const DAVA::Type* componentType = section->GetComponentType();
...
void RootProperty::RefreshProperty(AbstractProperty* property, DAVA::int32 refreshFlags)
{
    ...
    {
        VarTableValueProperty* propertiesProperty = dynamic_cast<VarTableValueProperty*>(property);
        if (propertiesProperty != nullptr)
        {
...
void RootProperty::RebuildVirtualProperties()
{
    // Rebuild virtual properties section
    for (auto& vSection : virtualProperties)
    {
