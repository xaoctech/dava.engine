...
/**
    ...
    inline FMVector3(float _x, float _y, float _z)
    {
        x = _x;
        y = _y;
        z = _z;
    ...
    inline FMVector3(const FMVector3& rhs)
    {
        x = rhs.x;
        y = rhs.y;
        z = rhs.z;
    ...
    inline FMVector3 Normalize() const
    {
        float l = Length();
        return (l > 0.0f) ? FMVector3(x / l, y / l, z / l) : FMVector3::XAxis;
    }

    /** Project this FMVector3 onto another FMVector3.
		@param unto The FMVector3 to project onto. */
    inline void Project(const FMVector3& unto)
    {
        (*this) = Projected(unto);
    }
    ...
    /** Assign this FMVector3 to the given float array.
		...
		@return This vector. */
    inline FMVector3& operator=(const float* v)
    {
        x = *v;
        y = *(v + 1);
        z = *(v + 2);
    ...
    inline FMVector3& operator=(const FMVector3& rhs)
    {
        x = rhs.x;
        y = rhs.y;
        z = rhs.z;
    ...
    inline void ComponentMinimum(const FMVector3& min)
    {
        if (x < min.x)
            x = min.x;
        if (y < min.y)
    ...
    inline float ComponentMinimum() const
    {
        return min(fabsf(x), min(fabsf(y), fabsf(z)));
    }
    ...
    inline void ComponentMaximum(const FMVector3& max)
    {
        if (x > max.x)
            x = max.x;
        if (y > max.y)
    ...
    inline float ComponentMaximum() const
    {
    ...
    inline void ComponentClamp(const FMVector3& min, const FMVector3& max)
    {
    ...
    inline float ComponentAverage() const
    {
        return (fabsf(x) + fabsf(y) + fabsf(z)) / 3.0f;
    }
...
inline FMVector3 operator+(const FMVector3& a, const FMVector3& b)
{
    return FMVector3(a.x + b.x, a.y + b.y, a.z + b.z);
}
...
/** Positive operator of the given FMVector3.
	...
	@return The FMVector3 representation of the resulting vector. */
inline FMVector3 operator+(const FMVector3& a)
{
    return FMVector3(+a.x, +a.y, +a.z);
}
...
inline float operator*(const FMVector3& a, const FMVector3& b)
{
...
inline FMVector3 operator*(const FMVector3& a, float b)
{
    return FMVector3(a.x * b, a.y * b, a.z * b);
}
...
/** Scalar multiplication with a FMVector3.
	...
	@param b The vector.
	@return The FMVector3 representing the resulting vector. */
inline FMVector3 operator*(float a, const FMVector3& b)
{
    return FMVector3(a * b.x, a * b.y, a * b.z);
}
...
inline FMVector3&
operator+=(FMVector3& b, const FMVector3& a)
{
    b.x += a.x;
    b.y += a.y;
    b.z += a.z;
...
/**	Assignment of the subtraction of two FMVector3.
	...
	@param a The second vector.
	@return The first vector, after it has been assigned new values. */
inline FMVector3& operator-=(FMVector3& b, const FMVector3& a)
{
    b.x -= a.x;
    b.y -= a.y;
    b.z -= a.z;
...
inline FMVector3& operator*=(FMVector3& b, float a)
{
    b.x *= a;
    b.y *= a;
    b.z *= a;
...
inline bool IsEquivalent(const FMVector3& p, const FMVector3& q)
{
    return IsEquivalent(p.x, q.x) && IsEquivalent(p.y, q.y) && IsEquivalent(p.z, q.z);
}
inline bool operator==(const FMVector3& p, const FMVector3& q)
{
    return IsEquivalent(p, q);
} /**< See above. */

// Already documented above.
inline FMVector3 FMVector3::Projected(const FMVector3& unto) const
{
    return ((*this) * unto) / unto.LengthSquared() * unto;
}
