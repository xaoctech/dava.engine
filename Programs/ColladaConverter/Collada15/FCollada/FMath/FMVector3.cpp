...
FMVector4 FMVector4::FromHSVColor(float hue, float saturation, float value)
{
    ...
    if (!IsEquivalent(saturation, 0.0f))
    {
        ...
        {
        case 0:
            return FMVector4(value, t, p, 1.0f);
        case 1:
            return FMVector4(q, value, p, 1.0f);
        case 2:
            return FMVector4(p, value, t, 1.0f);
        case 3:
            return FMVector4(p, q, value, 1.0f);
        case 4:
            return FMVector4(t, p, value, 1.0f);
        case 5:
        default:
            return FMVector4(value, p, q, 1.0f);
        }
    }
    else
        return FMVector4(value, value, value, 1.0f); // Achromatic (grey)
}
...
FMVector3 FMVector4::ToHSVColor()
{
    ...
    if (!IsEquivalent(largest, 0.0f))
    {
        ...
        if (IsEquivalent(x, largest))
            hsv.x = (y - z) / delta; // between yellow & magenta
        else if (IsEquivalent(y, largest))
            hsv.x = 2.0f + (z - x) / delta; // between cyan & yellow
        else
            hsv.x = 4.0f + (x - y) / delta; // between magenta & cyan
