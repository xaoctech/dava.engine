...
public:
    ...
    iterator find(const KEY& key)
    {
        ...
        while (out != NULL)
        {
            if (key < out->data.first)
                out = out->left;
            else if (key == out->data.first)
                return iterator(out);
            else
                out = out->right;
        }
    ...
    {
        return const_iterator(const_cast<tree<KEY, DATA>*>(this)->find(key));
    } /**< See above. */
...
template <class KEY, class DATA>
bool tree<KEY, DATA>::iterator::operator==(const const_iterator& other) const
{
    return other.currentNode == currentNode;
}
template <class KEY, class DATA>
