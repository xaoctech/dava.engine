...
inline double RadToDeg(double val)
{
    return (val * 180.0 / Pi);
}
...
inline T Clamp(T val, T2 mn, T3 mx)
{
...
template <class T, class T2, class T3>
inline T Wrap(T val, T2 mn, T3 mx)
{
    T d = (T)(mx - mn);
    while (val > (T)mx) val -= d;
    while (val < (T)mn) val += d;
    return val;
}
...
{
    if (val > mx)
        return (mn + fmodf(val - mx, mx - mn));
    else if (val < mn)
        return (mx - fmodf(mn - val, mx - mn));
    return val;
}
/** See above.*/
template <>
...
{
    if (val > mx)
        return (mn + fmod(val - mx, mx - mn));
    else if (val < mn)
        return (mx - fmod(mn - val, mx - mn));
    return val;
}
/** See above.*/
template <>
...
{
    if (val > mx)
        return mn + ((val - mx) % (mx - mn));
    else if (val < mn)
        return mx - ((mn - val) % (mx - mn));
    return val;
}
/** See above.*/
template <>
