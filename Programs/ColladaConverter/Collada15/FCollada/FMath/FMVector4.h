...
/** Creates an empty 4D vector. */
    ...
    inline FMVector4(const float* source, uint32 startIndex = 0)
    {
    ...
    inline FMVector4(const FMVector3& v, float _w)
    {
        x = v.x;
        y = v.y;
        z = v.z;
    ...
    inline FMVector4(const FMVector2& v1, const FMVector2& v2)
    {
        x = v1.x;
        y = v1.y;
        z = v2.x;
    ...
    inline FMVector4(float _x, float _y, float _z, float _w)
    {
        x = _x;
        y = _y;
        z = _z;
...
public:
    ...
    inline FMVector4& operator=(const float* v)
    {
        x = *v;
        y = *(v + 1);
        z = *(v + 2);
    ...
    inline void ComponentMinimum(const FMVector4& min)
    {
        if (x < min.x)
            x = min.x;
        if (y < min.y)
    ...
    inline void ComponentMaximum(const FMVector4& max)
    {
        if (x > max.x)
            x = max.x;
        if (y > max.y)
...
inline FMVector4 operator*(const FMVector4& a, float b)
{
    return FMVector4(a.x * b, a.y * b, a.z * b, a.w * b);
}
...
/** Scalar multiplication with a FMVector4.
	...
	@param b The vector.
	@return The FMVector4 representing the resulting vector. */
inline FMVector4 operator*(float a, const FMVector4& b)
{
    return FMVector4(a * b.x, a * b.y, a * b.z, a * b.w);
}
...
inline FMVector4 operator+(const FMVector4& a, const FMVector4& b)
{
    return FMVector4(a.x + b.x, a.y + b.y, a.z + b.z, a.w + b.w);
}
...
inline FMVector4& operator+=(FMVector4& b, const FMVector4& a)
{
    b.x += a.x;
    b.y += a.y;
    b.z += a.z;
...
inline FMVector4& operator*=(FMVector4& b, float a)
{
    b.x *= a;
    b.y *= a;
    b.z *= a;
...
inline float operator*(const FMVector4& a, const FMVector4& b)
{
...
inline bool IsEquivalent(const FMVector4& p, const FMVector4& q)
{
    return IsEquivalent(p.x, q.x) && IsEquivalent(p.y, q.y) && IsEquivalent(p.z, q.z) && IsEquivalent(p.w, q.w);
}
inline bool operator==(const FMVector4& p, const FMVector4& q)
{
    return IsEquivalent(p.x, q.x) && IsEquivalent(p.y, q.y) && IsEquivalent(p.z, q.z) && IsEquivalent(p.w, q.w);
} /**< See above. */
