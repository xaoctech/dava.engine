...
bool CheckPhysicsMaterialLibrary(FULogFile& fileOut, FCDPhysicsMaterialLibrary* library)
{
    ...
    for (size_t i = 0; i < library->GetEntityCount(); ++i)
    {
        ...
        if (m->GetDaeId() == physicsMaterialId1)
        {
            FailIf(material1 != NULL);
            material1 = m;
        }
        else if (m->GetDaeId() == physicsMaterialId2)
        {
            FailIf(material2 != NULL);
            material2 = m;
        }
        else if (m->GetDaeId() == physicsMaterialId3)
        {
            FailIf(material3 != NULL);
            material3 = m;
        }
        else
        {
...
bool CheckPhysicsModelLibrary(FULogFile& fileOut, FCDPhysicsModelLibrary* library)
{
    ...
    for (size_t i = 0; i < library->GetEntityCount(); ++i)
    {
        ...
        if (m->GetDaeId() == physicsModelId1)
        {
            FailIf(model1 != NULL);
            model1 = m;
        }
        else if (m->GetDaeId() == physicsModelId2)
        {
            FailIf(model2 != NULL);
            model2 = m;
        }
        else if (m->GetDaeId() == physicsModelId3)
        {
            FailIf(model3 != NULL);
            model3 = m;
        }
...
bool CheckPhysicsModel(FULogFile& fileOut, FCDPhysicsModel* model)
{
    ...
    for (size_t i = 0; i < 2; ++i)
    {
        ...
        if (c->GetSubId() == "Pour")
        {
            FailIf(constraint1 != NULL);
            constraint1 = c;
        }
        else if (c->GetSubId() == "Contre")
        {
            FailIf(constraint2 != NULL);
            constraint2 = c;
        }
        else
            Fail;
