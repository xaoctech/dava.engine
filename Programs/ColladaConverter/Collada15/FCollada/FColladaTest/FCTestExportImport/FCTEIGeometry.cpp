...
bool CheckGeometryLibrary(FULogFile& fileOut, FCDGeometryLibrary* library)
{
    ...
    FCDGeometryMesh* mesh = NULL;
    FCDGeometrySpline* spline = NULL;
...
bool CheckGeometryMesh(FULogFile& fileOut, FCDGeometryMesh* mesh)
{
    ...
    for (size_t i = 0; i < 3; ++i)
    {
        ...
        case FUDaeGeometryInput::POSITION:
            posSource = source;
            PassIf(source->GetName() == FC("TestPositionSource"));
            break;
        case FUDaeGeometryInput::COLOR:
            colorSource = source;
            PassIf(source->GetName() == FC("TestColorSource"));
            break;
        case FUDaeGeometryInput::EXTRA:
            dummySource = source;
            PassIf(source->GetName() == FC("TestDummySource"));
            break;
        default:
            Fail;
            break;
...
bool CheckControllerSkin(FULogFile& fileOut, FCDSkinController* controller)
{
    ...
    for (size_t i = 0; i < 2; ++i)
    {
        ...
        if (joint->GetId() == jointId1)
        {
            FailIf(joint1 != NULL);
            joint1 = &joint->GetBindPoseInverse();
        }
        else if (joint->GetId() == jointId2)
        {
            FailIf(joint2 != NULL);
            joint2 = &joint->GetBindPoseInverse();
        }
        else
            Fail;
    }
