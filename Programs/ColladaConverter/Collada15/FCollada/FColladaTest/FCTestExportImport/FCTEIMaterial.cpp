...
bool FillEffectStandard(FULogFile& fileOut, FCDEffectStandard* profile)
{
    ...
    UNUSED(FCDTexture* texture3 = )
    profile->AddTexture(FUDaeTextureChannel::FILTER);
    return true;
}
...
bool CheckEffectFX(FULogFile& fileOut, FCDEffectProfileFX* profile)
{
    ...
    for (size_t i = 0; i < profile->GetCodeCount(); ++i)
    {
        ...
        case FCDEffectCode::INCLUDE:
            PassIf(filenameCode == NULL);
            filenameCode = c;
            break;
        case FCDEffectCode::CODE:
            PassIf(inlineCode == NULL);
            inlineCode = c;
            break;
        default:
            Fail;
    ...
    for (size_t i = 0; i < 3; ++i)
    {
        ...
        if (p->GetObjectType() == FCDEffectParameterBool::GetClassType())
        {
            FailIf(booleanParameter != NULL);
            booleanParameter = (FCDEffectParameterBool*)p;
        }
        else if (p->GetObjectType() == FCDEffectParameterSampler::GetClassType())
        {
            FailIf(samplerParameter != NULL);
            samplerParameter = (FCDEffectParameterSampler*)p;
        }
        else if (p->GetObjectType() == FCDEffectParameterSurface::GetClassType())
        {
            FailIf(surfaceParameter != NULL);
            surfaceParameter = (FCDEffectParameterSurface*)p;
        }
        else
            Fail;
    }
    ...
    for (size_t i = 0; i < float2Parameter->GetAnnotationCount(); ++i)
    {
        ...
        if (a->type == FCDEffectParameter::FLOAT)
        {
            PassIf(floatAnnotation == NULL);
            floatAnnotation = a;
        }
        else if (a->type == FCDEffectParameter::INTEGER)
        {
            PassIf(integerAnnotation == NULL);
            integerAnnotation = a;
        }
        else if (a->type == FCDEffectParameter::STRING)
        {
            PassIf(stringAnnotation == NULL);
            stringAnnotation = a;
        }
        else if (a->type == FCDEffectParameter::BOOLEAN)
        {
            PassIf(booleanAnnotation == NULL);
            booleanAnnotation = a;
        }
        else
            Fail;
...
bool CheckImageLibrary(FULogFile& fileOut, FCDImageLibrary* library)
{
    ...
    for (size_t i = 0; i < 3; ++i)
    {
        ...
        if (IsEquivalent(image->GetDaeId(), wantedImageId))
        {
            FailIf(image1 != NULL);
            image1 = image;
        }
        else if (IsEquivalent(image->GetDaeId(), wantedImage2Id))
        {
            FailIf(image2 != NULL);
            image2 = image;
        }
