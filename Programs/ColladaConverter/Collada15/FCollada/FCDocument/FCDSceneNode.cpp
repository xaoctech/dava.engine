...
bool FCDSceneNode::AddChildNode(FCDSceneNode* sceneNode)
{
    ...
    if (children.contains(sceneNode))
        return false;
...
void FCDSceneNode::SetSubId(const fm::string& subId)
{
    ...
    do
    {
        for (size_t i = 0; i < curNode->GetParentCount(); i++)
        {
            ...
            if (!parentSubId.empty())
                parentSubIds.push_back(parentSubId);
        }
        // Continue iteration.
...
FCDEntity* FCDSceneNode::Clone(FCDEntity* _clone, bool cloneChildren) const
{
    ...
    if (_clone == NULL)
        _clone = clone = new FCDSceneNode(const_cast<FCDocument*>(GetDocument()));
    else if (_clone->HasType(FCDSceneNode::GetClassType()))
        clone = (FCDSceneNode*)_clone;
