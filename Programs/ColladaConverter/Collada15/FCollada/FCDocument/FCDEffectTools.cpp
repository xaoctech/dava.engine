...
void LinkAnimatedParamCommonFloat(const fm::string& semantic, FCDEffectParameterFloat* param, FCDGeometryInstance* geometry, FCDMaterial* material, FCDEffect* effect, FCDEffectProfile* profile)
{
    ...
    if (!geometryParam && !materialParam && !effectParam && !profileParam)
        return;
...
FCDParameterAnimatableFloat* GetAnimatedFloat(FCDGeometryInstance* geometryInstance, FCDMaterial* material, const fm::string& semantic)
{
    ...
    //Find out if the parameter is animated
    FCDEffectParameter* effectStandardParam = effectStandard->GetParam(semantic, &isFloat);
    if (!effectStandardParam)
        return NULL;
    const fm::string& reference = effectStandardParam->GetReference();
    if (reference.empty())
    {
        if (isFloat)
            return &(((FCDEffectParameterFloat*)effectStandardParam)->GetValue());
        else
            return NULL;
    }
    FCDEffectParameter* geometryParam = geometryInstance != NULL ? FindEffectParameterBySemantic(geometryInstance, semantic) : NULL;
    ...
    if (isFloat)
    {
        if (geometryParam)
        {
            if (geometryParam->GetType() != FCDEffectParameter::FLOAT)
                return NULL;
            else
                return &((FCDEffectParameterFloat*)geometryParam)->GetValue();
        }
        //From this on out, could call GetDefaultFloat... and return that?
        else if (materialParam)
        {
            if (materialParam->GetType() != FCDEffectParameter::FLOAT)
                return NULL;
            else
                return &((FCDEffectParameterFloat*)materialParam)->GetValue();
        }
        else if (effectParam)
        {
            if (effectParam->GetType() != FCDEffectParameter::FLOAT)
                return NULL;
            else
                return &((FCDEffectParameterFloat*)effectParam)->GetValue();
        }
        else if (effectProfileParam)
        {
            if (effectProfileParam->GetType() != FCDEffectParameter::FLOAT)
                return NULL;
            else
                return &((FCDEffectParameterFloat*)effectProfileParam)->GetValue();
        }
        else
            return &((FCDEffectParameterFloat*)effectStandardParam)->GetValue();
    }
...
float* GetDefaultFloat(FCDMaterial* material, const fm::string& semantic)
{
    ...
    {
        if (isFloat)
            return &(float&)((FCDEffectParameterFloat*)effectStandardParam)->GetValue();
        else
            return NULL;
    }
    FCDEffectParameter* materialParam = FindEffectParameterByReference(material, reference, true);
    ...
    if (isFloat)
    {
        if (materialParam)
        {
            if (materialParam->GetType() != FCDEffectParameter::FLOAT)
                ...
                return &(float&)((FCDEffectParameterFloat*)materialParam)->GetValue();
        }
        else if (effectParam)
        {
            if (effectParam->GetType() != FCDEffectParameter::FLOAT)
                ...
                return &(float&)((FCDEffectParameterFloat*)effectParam)->GetValue();
        }
        else if (effectProfileParam)
        {
            if (effectProfileParam->GetType() != FCDEffectParameter::FLOAT)
                ...
                return &(float&)((FCDEffectParameterFloat*)effectProfileParam)->GetValue();
        }
        else
            return &(float&)((FCDEffectParameterFloat*)effectStandardParam)->GetValue();
    }
...
{
    if (geometryInstance == NULL || semantic == NULL || *semantic == 0)
        return NULL;
    ...
    for (size_t p = 0; p < count; ++p)
    {
        ...
        if (IsEquivalent(effectParameter->GetSemantic(), semantic))
...
{
    if (materialInstance == NULL || semantic == NULL || *semantic == 0)
        return NULL;
...
{
    if (material == NULL || semantic == NULL || *semantic == 0)
        return NULL;
...
{
    if (effect == NULL || semantic == NULL || *semantic == 0)
        return NULL;
...
const FCDEffectParameter* FindEffectParameterBySemantic(const FCDEffectProfile* profile, const char* semantic, bool localOnly)
{
    ...
    if (profile == NULL || semantic == NULL || *semantic == 0)
        return NULL;
    ...
    else if (profile->HasType(FCDEffectStandard::GetClassType()))
    {
        ...
        for (uint32 i = 0; i < FUDaeTextureChannel::COUNT; ++i)
        {
            ...
            for (size_t t = 0; t < bucketSize; ++t)
            {
                ...
                if (IsEquivalent(texture->GetSet()->GetSemantic(), semantic))
                    return texture->GetSet();
            }
        }
    }
    return NULL;
...
{
    if (technique == NULL || semantic == NULL || *semantic == 0)
        return NULL;
...
{
    if (geometryInstance == NULL || reference == NULL || *reference == 0)
        return NULL;
    size_t count = geometryInstance->GetEffectParameterCount();
    for (size_t p = 0; p < count; ++p)
    {
        const FCDEffectParameter* effectParameter = geometryInstance->GetEffectParameter(p);
        if (IsEquivalent(effectParameter->GetReference(), reference))
            return effectParameter;
    }
    return NULL;
}
...
{
    if (materialInstance == NULL || reference == NULL || *reference == 0)
        return NULL;
    const FCDGeometryInstance* geometryInstance = (FCDGeometryInstance*)materialInstance->GetParent();
    size_t count = geometryInstance->GetEffectParameterCount();
    for (size_t p = 0; p < count; ++p)
...
{
    if (material == NULL || reference == NULL || *reference == 0)
        return NULL;
    size_t count = material->GetEffectParameterCount();
    for (size_t p = 0; p < count; ++p)
    {
...
{
    if (effect == NULL || reference == NULL || *reference == 0)
        return NULL;
    size_t count = effect->GetEffectParameterCount();
    for (size_t p = 0; p < count; ++p)
    {
...
{
    // Look within the local parameters.
    if (profile == NULL || reference == NULL || *reference == 0)
        return NULL;
    size_t count = profile->GetEffectParameterCount();
    for (size_t p = 0; p < count; ++p)
    {
    ...
    else if (profile->HasType(FCDEffectStandard::GetClassType()))
    {
        ...
        for (uint32 i = 0; i < FUDaeTextureChannel::COUNT; ++i)
        {
            ...
            {
                FCDTexture* texture = std->GetTexture(i, t);
                if (IsEquivalent(texture->GetSet()->GetReference(), reference))
                    return texture->GetSet();
            }
        }
    }
...
{
    if (technique == NULL || reference == NULL || *reference == 0)
        return NULL;
    size_t count = technique->GetEffectParameterCount();
    for (size_t p = 0; p < count; ++p)
    {
...
void FindEffectParametersBySemantic(const FCDMaterialInstance* materialInstance, const char* semantic, FCDEffectParameterList& parameters, bool localOnly)
{
    ...
    for (size_t p = 0; p < count; ++p)
    {
        const FCDEffectParameter* effectParameter = geometryInstance->GetEffectParameter(p);
        if (IsEquivalent(effectParameter->GetSemantic(), semantic))
            parameters.push_back(effectParameter);
    }
    if (!localOnly)
    {
    ...
    else if (profile->HasType(FCDEffectStandard::GetClassType()))
    {
        ...
        for (uint32 i = 0; i < FUDaeTextureChannel::COUNT; ++i)
        {
            ...
            for (size_t t = 0; t < bucketSize; ++t)
            {
                FCDTexture* texture = std->GetTexture(i, t);
                if (IsEquivalent(texture->GetSet()->GetSemantic(), semantic))
                    parameters.push_back(texture->GetSet());
            }
        }
    }
}
