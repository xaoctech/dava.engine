...
public:
    ...
    inline FCDAnimation* GetAnimation(size_t index) const
    {
        FUAssert(GetAnimationCount() > index, return NULL);
        return (FCDAnimation*)animations[index]->GetEntity();
    };
    ...
    inline void SetAnimationName(const fm::string& name, size_t index)
    {
        if (names.size() <= index)
    ...
    inline fm::string GetAnimationName(size_t index) const
    {
        return names[index];
    }
