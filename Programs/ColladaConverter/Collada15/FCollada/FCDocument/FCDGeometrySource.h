...
public:
    ...
    {
        return !sourceData.empty() ? &sourceData.front() : NULL;
    }
	...
	*/
    inline FCDParameterListAnimatableFloat& GetSourceData()
    {
        return sourceData;
    }
    inline const FCDParameterListAnimatableFloat& GetSourceData() const
    {
        return sourceData;
    }
    ...
    {
        return (float**)sourceData.GetDataPtr();
    }
    inline const float** GetDataPtr() const
    {
        return (const float**)sourceData.GetDataPtr();
    } /**< See above. */
    ...
    {
        return sourceData.size() / stride;
    }

    /** Retrieves the max number of values this input can handle before memory is reallocated.
		@return The number of source values. */
    inline size_t GetValueReserved() const
    {
        return sourceData.capacity() / stride;
    }
    ...
    inline void SetValueCount(size_t count)
    {
        FUAssert(stride > 0, return );
        SetDataCount(count * stride);
    }
    ...
    inline const float* GetValue(size_t index) const
    {
        ...
        return &sourceData.at(index * stride);
    } /**< See above. */

    /** Sets one source value out of this source.
		@param index The index of the source value.
		@param value The new value. */
    inline void SetValue(size_t index, const float* value)
    {
        FUAssert(index < GetValueCount(), return );
        for (size_t i = 0; i < stride; ++i) sourceData.set(stride * index + i, value[i]);
    }
    ...
    inline FUDaeGeometryInput::Semantic GetType() const
    {
        return (FUDaeGeometryInput::Semantic)*sourceType;
    }
    ...
    inline void SetType(FUDaeGeometryInput::Semantic type)
    {
        sourceType = type;
    }
    ...
    inline FUObjectContainer<FCDAnimated>& GetAnimatedValues()
    {
        return sourceData.GetAnimatedValues();
    }
    inline const FUObjectContainer<FCDAnimated>& GetAnimatedValues() const
    {
        return sourceData.GetAnimatedValues();
    } /**< See above. */
    ...
    inline void SetStride(uint32 _stride)
    {
        stride = _stride;
        SetDirtyFlag();
    }
