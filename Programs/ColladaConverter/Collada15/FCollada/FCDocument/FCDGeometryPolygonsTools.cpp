...
void Triangulate(FCDGeometryPolygons* polygons, bool recalculate)
{
    if (polygons == NULL)
        return;
    if (polygons->GetPrimitiveType() == FCDGeometryPolygons::LINE_STRIPS || polygons->GetPrimitiveType() == FCDGeometryPolygons::LINES || polygons->GetPrimitiveType() == FCDGeometryPolygons::POINTS)
        return;
...
static uint32 CompressSortedVector(FMVector3& toInsert, FloatList& insertedList, UInt32List& compressIndexReferences)
{
    ...
    for (mid = (start + end) / 2; start < end; mid = (start + end) / 2)
    {
        ...
        if (toInsert.x == insertedList[3 * index])
            break;
        else if (toInsert.x < insertedList[3 * index])
            end = mid;
        else
            start = mid + 1;
    }
    ...
    for (rangeStart = mid; rangeStart > 0; --rangeStart)
    {
        ...
        if (!IsEquivalent(insertedList[3 * index], toInsert.x))
            break;
    }
    ...
    for (size_t g = rangeStart; g < rangeEnd; ++g)
    {
        ...
        if (IsEquivalent(toInsert, *(const FMVector3*)&insertedList[3 * index]))
            return index;
    }
...
{
    if (texcoordSource == NULL || mesh == NULL)
        return;
    ...
    for (size_t i = 0; i < polygonsCount; ++i)
    {
        ...
        if (positionInput == NULL || normalsInput == NULL)
            continue;
        ...
        if (positionSource == NULL || normalsSource == NULL || texcoordSource == NULL)
            continue;
        ...
        if (positionStride < 3 || normalsStride < 3 || texcoordStride < 2)
            continue;
        ...
        if (positionIndices == NULL || normalsIndices == NULL || texcoordIndices == NULL)
            continue;
        if (indexCount == 0 || indexCount != normalsInput->GetIndexCount() || indexCount != texcoordInput->GetIndexCount())
            continue;
        ...
        if (positionDataLength == 0 || normalsDataLength == 0 || texcoordDataLength == 0)
            continue;
        ...
        for (size_t faceIndex = 0; faceIndex < faceCount; ++faceIndex)
        {
            ...
            for (size_t vertexIndex = 0; vertexIndex < faceVertexCount; ++vertexIndex)
            {
                ...
                if (IsEquivalent(crossEdge.x, 0.0f))
                    crossEdge.x = 1.0f; // degenerate
    ...
    for (size_t i = 0; i < polygonsCount; ++i)
    {
        ...
        FCDGeometrySource* normalsSource = normalsInput->GetSource();
        FCDGeometrySource* texcoordSource = texcoordInput->GetSource();
        if (normalsSource == NULL || texcoordSource == NULL)
            continue;
        uint32 normalsStride = normalsSource->GetStride();
        uint32 texcoordStride = texcoordSource->GetStride();
        if (normalsStride < 3 || texcoordStride < 2)
            continue;
        uint32* positionIndices = positionInput->GetIndices();
        uint32* normalsIndices = normalsInput->GetIndices();
        uint32* texcoordIndices = texcoordInput->GetIndices();
        ...
        size_t normalsDataLength = normalsSource->GetDataCount();
        size_t texcoordDataLength = texcoordSource->GetDataCount();
        if (normalsDataLength == 0 || texcoordDataLength == 0)
            continue;
    ...
    if (tangentSource != NULL)
        tangentSource->SetData(tangentData, 3);
    if (binormalSource != NULL)
        binormalSource->SetData(binormalData, 3);
}

struct HashIndexMapItem
{
    UInt32List allValues;
    UInt32List newIndex;
};
...
void GenerateUniqueIndices(FCDGeometryMesh* mesh, FCDGeometryPolygons* polygonsToProcess, FCDNewIndicesList& outIndices, FCDGeometryIndexTranslationMapList& outTranslationMaps)
{
    ...
    if (outIndices.size() < outIndicesMinSize)
        outIndices.resize(outIndicesMinSize);
    ...
    for (size_t p = 0; p < polygonsCount; ++p)
    {
        ...
        if (polygons->GetPrimitiveType() == FCDGeometryPolygons::POINTS)
            return;
        if (polygonsToProcess != NULL && polygons != polygonsToProcess)
            continue;
        ...
        for (size_t i = 0; i < originalIndexCount; ++i)
        {
            ...
            if (it != hashMap.end())
            {
                ...
                for (size_t r = 0; r < repeatCount && newIndex == totalVertexCount; ++r)
                {
                    ...
                    {
                        if (indices[l][i] != hashItem->allValues[r * listCount + l])
                            break;
                    }
                    if (l == listCount)
                    {
    ...
    if (outTranslationMaps.size() < meshSourceCount)
        outTranslationMaps.resize(meshSourceCount);
    for (size_t d = 0; d < meshSourceCount; ++d)
    {
        ...
        {
            FCDGeometryPolygons* polygons = mesh->GetPolygons(p);
            if (polygonsToProcess != NULL && polygonsToProcess != polygons)
                continue;
            ...
            if (oldIndexList == NULL || oldIndexCount == 0)
                continue;
            ...
            for (size_t i = 0; i < indexCount; ++i)
            {
                ...
                if (oldIndex >= oldSource->GetValueCount())
                    continue;
                ...
                if (itU == thisMap->end())
                {
                    itU = thisMap->insert(oldIndex, UInt32List());
                }
                UInt32List::iterator itF = itU->second.find(newIndex);
                if (itF == itU->second.end())
                    itU->second.push_back(newIndex);
            }
        }
    }
}
...
void GenerateUniqueIndices(FCDGeometryMesh* mesh, FCDGeometryPolygons* polygonsToProcess, FCDGeometryIndexTranslationMap* translationMap)
{
    ...
    if (polygonsCount == 0)
        return;
    UInt32ListList indexBuffers;
    indexBuffers.resize(polygonsCount);
    size_t totalVertexCount = 0;
    ...
    for (size_t p = 0; p < polygonsCount; ++p)
    {
        ...
        for (size_t i = 0; i < inputCount; ++i)
        {
            ...
            if (input->OwnsIndices())
            {
                ...
                if (idxOwners.find(input) == idxOwners.end())
                    idxOwners.push_back(input);
            }
        }
        size_t listCount = idxOwners.size();
        if (listCount == 0)
        ...
        for (size_t p = 0; p < polygonsCount; ++p)
        {
            ...
            for (size_t i = 0; i < indexCount; ++i)
            {
                ...
                if (isPositionSource)
                {
                    ...
                    if (itU == translationMap->end())
                    {
                        itU = translationMap->insert(oldIndex, UInt32List());
                    }
                    UInt32List::iterator itF = itU->second.find(newIndex);
                    if (itF == itU->second.end())
                        itU->second.push_back(newIndex);
    ...
    for (size_t p = 0; p < polygonsCount; ++p)
    {
        ...
        for (size_t i = 0; i < inputCount; i++)
        {
            ...
            if (anyInput->GetSource()->GetDataCount() == 0)
                continue;
...
uint16 GenerateVertexPackingMap(size_t maxIndex, size_t maxIndices, size_t maxVertices, const uint32* inIndices, uint16* outIndices, UInt16List* outPackingMap, uint16* outNVertices /*=NULL*/)
{
    ...
    if (outNVertices != NULL)
        *outNVertices = nVertices;
    return nIndices;
}
...
void FitIndexBuffers(FCDGeometryMesh* mesh, size_t maximumIndexCount)
{
    ...
    for (size_t i = 0; i < originalPolygonCount; ++i)
    {
        ...
        {
            if (faceVertexCount + (*splitIt) > maximumIndexCount)
                break;
            faceVertexCount += (*splitIt);
        }
        if (splitIt == faceVertexCounts.end())
            continue; // Polygons sets fit correctly.
        ...
        while (faceCopyEnd < faceCount)
        {
            ...
            for (; faceCopyEnd < faceCount; ++faceCopyEnd)
            {
                ...
                if (faceVertexCount + localCount > maximumIndexCount)
                    break;
            ...
            for (size_t j = 0; j < inputCount; ++j)
            {
                ...
                if (!mesh->IsVertexSource(source))
                    inputCopy = polygonsCopy->AddInput(source, input->GetOffset());
                else
                    inputCopy = polygonsCopy->FindInput(source);
