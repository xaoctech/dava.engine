...
FCDTransform* FCDPhysicsShape::AddTransform(FCDTransform::Type type, size_t index)
{
    ...
    if (transform != NULL)
    {
        if (index > transforms.size())
            ...
            transforms.insert(transforms.begin() + index, transform);
    }
    SetNewChildFlag();
    return transform;
}
...
{
    if (clone == NULL)
        clone = new FCDPhysicsShape(const_cast<FCDocument*>(GetDocument()));
...
float FCDPhysicsShape::CalculateVolume() const
{
    if (IsGeometryInstance())
    {
        ...
        if (geom->IsMesh())
        {
            ...
            if (mesh == NULL)
                return 1.0f; // missing convex hull or of spline
