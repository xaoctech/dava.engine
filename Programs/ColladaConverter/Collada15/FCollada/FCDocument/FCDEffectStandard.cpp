    ...
    , InitializeParameterNoArg(filterTextures)
{
    ...
    // factor of 0.0.
    if (GetDocument()->GetVersion() < FCDVersion(1, 4, 1))
...
const FCDTexture** FCDEffectStandard::GetTextureBucket(uint32 bucket) const
{
    switch (bucket)
    {
    case FUDaeTextureChannel::AMBIENT:
        return ambientTextures.begin();
    case FUDaeTextureChannel::BUMP:
        return bumpTextures.begin();
    case FUDaeTextureChannel::DIFFUSE:
        return diffuseTextures.begin();
    case FUDaeTextureChannel::DISPLACEMENT:
        return displacementTextures.begin();
    case FUDaeTextureChannel::EMISSION:
        return emissionTextures.begin();
    case FUDaeTextureChannel::FILTER:
        return filterTextures.begin();
    case FUDaeTextureChannel::REFLECTION:
        return reflectivityTextures.begin();
    case FUDaeTextureChannel::REFRACTION:
        return refractionTextures.begin();
    case FUDaeTextureChannel::SHININESS:
        return shininessTextures.begin();
    case FUDaeTextureChannel::SPECULAR:
        return specularTextures.begin();
    case FUDaeTextureChannel::SPECULAR_LEVEL:
        return specularFactorTextures.begin();
    case FUDaeTextureChannel::TRANSPARENT:
        return translucencyTextures.begin();
...
size_t FCDEffectStandard::GetTextureCount(uint32 bucket) const
{
    ...
    {
    case FUDaeTextureChannel::AMBIENT:
        return ambientTextures.size();
    case FUDaeTextureChannel::BUMP:
        return bumpTextures.size();
    case FUDaeTextureChannel::DIFFUSE:
        return diffuseTextures.size();
    case FUDaeTextureChannel::DISPLACEMENT:
        return displacementTextures.size();
    case FUDaeTextureChannel::EMISSION:
        return emissionTextures.size();
    case FUDaeTextureChannel::FILTER:
        return filterTextures.size();
    case FUDaeTextureChannel::REFLECTION:
        return reflectivityTextures.size();
    case FUDaeTextureChannel::REFRACTION:
        return refractionTextures.size();
    case FUDaeTextureChannel::SHININESS:
        return shininessTextures.size();
    case FUDaeTextureChannel::SPECULAR:
        return specularTextures.size();
    case FUDaeTextureChannel::SPECULAR_LEVEL:
        return specularFactorTextures.size();
    case FUDaeTextureChannel::TRANSPARENT:
        return translucencyTextures.size();
...
FCDTexture* FCDEffectStandard::AddTexture(uint32 bucket)
{
    ...
    case FUDaeTextureChannel::AMBIENT:
        ...
        break;
    case FUDaeTextureChannel::BUMP:
        bumpTextures.push_back(texture);
        break;
    case FUDaeTextureChannel::DIFFUSE:
    ...
    case FUDaeTextureChannel::EMISSION:
        ...
        break;
    case FUDaeTextureChannel::FILTER:
        filterTextures.push_back(texture);
        break;
    case FUDaeTextureChannel::REFLECTION:
        reflectivityTextures.push_back(texture);
        break;
    case FUDaeTextureChannel::REFRACTION:
        refractionTextures.push_back(texture);
        break;
    case FUDaeTextureChannel::SHININESS:
        shininessTextures.push_back(texture);
        break;
    case FUDaeTextureChannel::SPECULAR:
        specularTextures.push_back(texture);
        break;
    case FUDaeTextureChannel::SPECULAR_LEVEL:
        specularFactorTextures.push_back(texture);
        break;
    case FUDaeTextureChannel::TRANSPARENT:
        translucencyTextures.push_back(texture);
        break;
...
FCDEffectProfile* FCDEffectStandard::Clone(FCDEffectProfile* _clone) const
{
    ...
    if (_clone == NULL)
        _clone = clone = new FCDEffectStandard(const_cast<FCDocument*>(GetDocument()), const_cast<FCDEffect*>(GetParent()));
    else if (_clone->GetObjectType() == FCDEffectStandard::GetClassType())
        clone = (FCDEffectStandard*)_clone;

    if (_clone != NULL)
        FCDEffectProfile::Clone(_clone);
    if (clone != NULL)
    {
        clone->type = type;
...
#define CLONE_ANIMATED(value) clone->value->SetValue(value->GetValue()); if (value->GetValue().IsAnimated()) value->GetValue().GetAnimated()->Clone(clone->value->GetValue().GetAnimated());
        ...
        CLONE_ANIMATED(emissionColor);
        CLONE_ANIMATED(emissionFactor);
        clone->isEmissionFactor = isEmissionFactor;
        CLONE_ANIMATED(translucencyColor);
        CLONE_ANIMATED(translucencyFactor);
        CLONE_ANIMATED(diffuseColor);
        CLONE_ANIMATED(ambientColor);
        CLONE_ANIMATED(specularColor);
        CLONE_ANIMATED(specularFactor);
        CLONE_ANIMATED(shininess);
        CLONE_ANIMATED(reflectivityColor);
        CLONE_ANIMATED(reflectivityFactor);
        CLONE_ANIMATED(indexOfRefraction);
...
void FCDEffectStandard::AddExtraAttribute(const char* profile, const char* key, const fchar* value)
{
    ...
    if (extraTech == NULL)
        extraTech = GetParent()->GetExtra()->GetDefaultType()->AddTechnique(profile);
