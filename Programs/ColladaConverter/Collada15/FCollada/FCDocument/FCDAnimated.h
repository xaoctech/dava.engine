...
public:
    ...
    inline size_t GetCurveCount(size_t index) const
    {
        FUAssert(index < GetValueCount(), return 0);
        return curves[index].size();
    }
    ...
    inline FCDAnimationCurve* GetCurve(size_t index, size_t curveIndex = 0)
    {
        FUAssert(index < GetValueCount(), return NULL);
        return curveIndex < curves.at(index).size() ? curves.at(index).at(curveIndex) : NULL;
    }
    inline const FCDAnimationCurve* GetCurve(size_t index, size_t curveIndex = 0) const
    {
        FUAssert(index < GetValueCount(), return NULL);
        return curveIndex < curves.at(index).size() ? curves.at(index).at(curveIndex) : NULL;
    } /**< See above. */
    ...
    inline FCDAnimationCurveListList& GetCurves()
    {
        return curves;
    }
    inline const FCDAnimationCurveListList& GetCurves() const
    {
        return curves;
    } /**< See above. */
    ...
    inline float* GetValue(size_t index)
    {
        FUAssert(index < GetValueCount(), return NULL);
        return values.at(index);
    }
    inline const float* GetValue(size_t index) const
    {
        FUAssert(index < GetValueCount(), return NULL);
        return values.at(index);
    } /**< See above. */
    ...
    inline void SetValue(size_t index, float* value)
    {
        FUAssert(index < GetValueCount(), return );
	...
	*/
    ...
    inline FCDAnimationCurve* FindCurve(const char* qualifier)
    {
        size_t index = FindQualifier(qualifier);
        return index < GetValueCount() ? GetCurve(index) : NULL;
    }
    inline FCDAnimationCurve* FindCurve(const fm::string& qualifier)
    {
        return FindCurve(qualifier.c_str());
    } /**< See above. */
    inline const FCDAnimationCurve* FindCurve(const char* qualifier) const
    {
        size_t index = FindQualifier(qualifier);
        return index < GetValueCount() ? GetCurve(index) : NULL;
    } /**< See above. */
    inline const FCDAnimationCurve* FindCurve(const fm::string& qualifier) const
    {
        return FindCurve(qualifier.c_str());
    } /**< See above. */
    ...
    inline FCDAnimationCurve* FindCurve(const float* value)
    {
        size_t index = FindValue(value);
        return index < GetValueCount() ? GetCurve(index) : NULL;
    }
    inline const FCDAnimationCurve* FindCurve(const float* value) const
    {
        size_t index = FindValue(value);
        return index < GetValueCount() ? GetCurve(index) : NULL;
    } /**< See above. */
    ...
    inline size_t FindQualifier(const fm::string& qualifier) const
    {
        return FindQualifier(qualifier.c_str());
    } /**< See above. */
    ...
    inline int32 GetArrayElement() const
    {
        return arrayElement;
    }
    ...
    /** Sets the array index for an animated element.
		...
		@param index The array index. This value should be -1 to indicate that
			the animated element does not belong to a list. */
    inline void SetArrayElement(int32 index)
    {
        arrayElement = index;
        SetDirtyFlag();
    }
