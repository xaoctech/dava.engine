...
{
    return topDocuments.size();
}
FCOLLADA_EXPORT FCDocument* GetTopDocument(size_t index)
{
    FUAssert(index < topDocuments.size(), return NULL);
    return topDocuments.at(index);
}
FCOLLADA_EXPORT bool IsTopDocument(FCDocument* document)
{
    return topDocuments.contains(document);
}
...
FCOLLADA_EXPORT void GetAllDocuments(FCDocumentList& documents)
{
    ...
    for (size_t index = 0; index < topDocuments.size(); ++index)
    {
        ...
        for (size_t p = 0; p < placeHolderCount; ++p)
        {
            ...
            if (target != NULL && !documents.contains(target))
...
FCOLLADA_EXPORT FCDocument* LoadDocument(const fchar* filename)
{
    ...
    if (!LoadDocumentFromFile(document, filename))
        document->Release();
    return document;
}
