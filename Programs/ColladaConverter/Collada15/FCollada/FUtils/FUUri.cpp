    ...
    path(FC(""))
{
    ...
    if (scheme != FUUri::NONE)
    {
        ...
        if (hostDelimiterIndex == fstring::npos)
            hostDelimiterIndex = _uri.length();
    ...
    if (queryDelimiter != fstring::npos)
        query = _uri.substr(queryDelimiter + 1, fragmentDelimiter - queryDelimiter);
    if (fragmentDelimiter != fstring::npos)
        fragment = _uri.substr(fragmentDelimiter + 1);
...
fstring FUUri::MakeRelative(const fstring& _path) const
{
    ...
    if (!filePath.empty())
    {
        ...
        {
            if (!IsEquivalent(documentPaths[matchIndex], localPaths[matchIndex]))
                break;
        }
