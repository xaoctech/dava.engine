...
inline bool IsSomeFolderChar(fchar c)
{
    return c == FOLDER_CHAR || c == UNWANTED_FOLDER_CHAR;
}
...
void FUFileManager::PushRootPath(const fstring& path)
{
    ...
    if (absolutePath.length() > 0 && absolutePath.back() != '\\' && absolutePath.back() != '/')
        absolutePath.append((fchar)'/');
...
#ifdef WIN32
    if (_mkdir(TO_STRING(absoluteDirectory).c_str()) == 0)
        return true;
    errno_t err;
    _get_errno(&err);
    if (err == EEXIST)
        return true;
#elif defined(LINUX)
    if (mkdir(TO_STRING(absoluteDirectory).c_str(), ~0u) == 0)
        return true; // I think this means all permissions..
#elif defined(__APPLE__)
    if (mkdir(TO_STRING(absoluteDirectory).c_str(), 0777) == 0)
        return true;
#endif // WIN32
...
fstring FUFileManager::CleanUri(const FUUri& uri)
{
    ...
    if (uri.GetScheme() == FUUri::NONE)
        out = FS("#") + uri.GetFragment();
    else if (forceAbsolute)
        out = uri.GetAbsoluteUri();
    else
        out = uri.GetRelativeUri(GetCurrentUri());
    return out;
}
