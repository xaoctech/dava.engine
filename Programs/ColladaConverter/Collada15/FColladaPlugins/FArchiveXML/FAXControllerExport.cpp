...
xmlNode* FArchiveXML::WriteController(FCDObject* object, xmlNode* parentNode)
{
    ...
    if (controller->GetSkinController() != NULL)
        FArchiveXML::LetWriteObject(controller->GetSkinController(), controllerNode);
    else if (controller->GetMorphController() != NULL)
        FArchiveXML::LetWriteObject(controller->GetMorphController(), controllerNode);
...
xmlNode* FArchiveXML::WriteSkinController(FCDObject* object, xmlNode* parentNode)
{
    ...
    jointSubIds.reserve(jointCount);
    FMMatrix44List jointBindPoses;
    jointBindPoses.reserve(jointCount);
    ...
    FUSStringBuilder jointSourceId(skinController->GetParent()->GetDaeId());
    jointSourceId += "-joints";
    AddSourceString(skinNode, jointSourceId.ToCharPtr(), jointSubIds, DAE_JOINT_SKIN_INPUT);

    // Create the joint bind matrix source
    FUSStringBuilder jointBindSourceId(skinController->GetParent()->GetDaeId());
    jointBindSourceId += "-bind_poses";
    ...
    for (size_t i = 0; i < influenceCount; ++i)
    {
        ...
        for (size_t i = 0; i < vertex->GetPairCount(); ++i)
        {
            ...
            if (!IsEquivalent(w, 1.0f))
                weights.push_back(w);
        }
    }
    FUSStringBuilder weightSourceId(skinController->GetParent()->GetDaeId());
    weightSourceId += "-weights";
    ...
    FUSStringBuilder vertexCounts;
    vertexCounts.reserve(1024);
    FUSStringBuilder vertexMatches;
    vertexMatches.reserve(1024);
    ...
    for (size_t j = 0; j < influenceCount; ++j)
    {
        ...
        vertexCounts.append((uint32)vertex->GetPairCount());
        vertexCounts.append(' ');
        for (size_t i = 0; i < vertex->GetPairCount(); ++i)
        {
            const FCDJointWeightPair* pair = vertex->GetPair(i);
            vertexMatches.append(pair->jointIndex);
            vertexMatches.append(' ');
            if (!IsEquivalent(pair->weight, 1.0f))
                vertexMatches.append(weightOffset++);
            else
                vertexMatches.append('0');
            vertexMatches.append(' ');
        }
    }
    if (!vertexMatches.empty())
        vertexMatches.pop_back();
...
xmlNode* FArchiveXML::WriteMorphController(FCDObject* object, xmlNode* parentNode)
{
    ...
    FUSStringBuilder targetSourceId(morphController->GetParent()->GetDaeId());
    targetSourceId.append("-targets");
    AddSourceIDRef(morphNode, targetSourceId.ToCharPtr(), targetIds, DAE_TARGET_MORPH_INPUT);

    // Export the weight source
    FUSStringBuilder weightSourceId(morphController->GetParent()->GetDaeId());
    weightSourceId.append("-morph_weights");
