...
xmlNode* FArchiveXML::WriteEffectCode(FCDObject* object, xmlNode* parentNode)
{
    ...
    for (xmlNode* n = parentNode->children; n != NULL; n = n->next)
    {
        if (n->type != XML_ELEMENT_NODE)
            continue;
        else if (IsEquivalent(n->name, DAE_ASSET_ELEMENT))
            continue;
        else if (IsEquivalent(n->name, DAE_FXCMN_CODE_ELEMENT))
            continue;
        else if (IsEquivalent(n->name, DAE_FXCMN_INCLUDE_ELEMENT))
            continue;
        else
        {
            includeAtNode = n;
    ...
    case FCDEffectCode::CODE:
        if (includeAtNode == NULL)
            codeNode = AddChild(parentNode, DAE_FXCMN_CODE_ELEMENT);
        else
            codeNode = InsertChild(parentNode, includeAtNode, DAE_FXCMN_CODE_ELEMENT);
    ...
    case FCDEffectCode::INCLUDE:
    {
        if (includeAtNode == NULL)
            ...
            codeNode = InsertChild(parentNode, includeAtNode, DAE_FXCMN_INCLUDE_ELEMENT);
        if (_sid.empty())
            _sid = "include";
        AddNodeSid(codeNode, _sid);
...
xmlNode* FArchiveXML::WriteEffectParameter(FCDObject* object, xmlNode* parentNode)
{
    ...
    for (size_t i = 0; i < effectParameter->GetAnnotationCount(); ++i)
    {
        ...
        case FCDEffectParameter::BOOLEAN:
            AddChild(annotateNode, DAE_FXCMN_BOOL_ELEMENT, *annotation->value);
            break;
        case FCDEffectParameter::STRING:
            AddChild(annotateNode, DAE_FXCMN_STRING_ELEMENT, *annotation->value);
            break;
        case FCDEffectParameter::INTEGER:
            AddChild(annotateNode, DAE_FXCMN_INT_ELEMENT, *annotation->value);
            break;
        case FCDEffectParameter::FLOAT:
            AddChild(annotateNode, DAE_FXCMN_FLOAT_ELEMENT, *annotation->value);
            break;
        default:
            break;
        }
...
xmlNode* FArchiveXML::WriteEffectParameterFloat(FCDObject* object, xmlNode* parentNode)
{
    ...
    else
    {
        ...
        if (*wantedSubId == 0)
            wantedSubId = effectParameterFloat->GetSemantic().c_str();
        if (*wantedSubId == 0)
            wantedSubId = "flt";
...
xmlNode* FArchiveXML::WriteEffectParameterFloat2(FCDObject* object, xmlNode* parentNode)
{
    ...
    builder.set(effectParameterFloat2->GetValue()->x);
    builder.append(' ');
    builder.append(effectParameterFloat2->GetValue()->y);
...
xmlNode* FArchiveXML::WriteEffectParameterFloat3(FCDObject* object, xmlNode* parentNode)
{
    ...
    if (*wantedSubId == 0)
        wantedSubId = effectParameterFloat3->GetSemantic().c_str();
    if (*wantedSubId == 0)
        wantedSubId = "flt3";
...
xmlNode* FArchiveXML::WriteEffectParameterSampler(FCDObject* object, xmlNode* parentNode)
{
    ...
    case FCDEffectParameterSampler::SAMPLER1D:
        samplerName = DAE_FXCMN_SAMPLER1D_ELEMENT;
        break;
    case FCDEffectParameterSampler::SAMPLER2D:
        samplerName = DAE_FXCMN_SAMPLER2D_ELEMENT;
        break;
    case FCDEffectParameterSampler::SAMPLER3D:
        samplerName = DAE_FXCMN_SAMPLER3D_ELEMENT;
        break;
    case FCDEffectParameterSampler::SAMPLERCUBE:
        samplerName = DAE_FXCMN_SAMPLERCUBE_ELEMENT;
        break;
    default:
        samplerName = DAEERR_UNKNOWN_ELEMENT;
        break;
    }
...
xmlNode* FArchiveXML::WriteEffectParameterSurface(FCDObject* object, xmlNode* parentNode)
{
    ...
    if (effectParameterSurface->GetFormatHint())
    {
        ...
        if (formatHint->channels == FCDFormatHint::CHANNEL_RGB)
            AddContent(channelsNode, DAE_FORMAT_HINT_RGB_VALUE);
        else if (formatHint->channels == FCDFormatHint::CHANNEL_RGBA)
            AddContent(channelsNode, DAE_FORMAT_HINT_RGBA_VALUE);
        else if (formatHint->channels == FCDFormatHint::CHANNEL_L)
            AddContent(channelsNode, DAE_FORMAT_HINT_L_VALUE);
        else if (formatHint->channels == FCDFormatHint::CHANNEL_LA)
            AddContent(channelsNode, DAE_FORMAT_HINT_LA_VALUE);
        else if (formatHint->channels == FCDFormatHint::CHANNEL_D)
            AddContent(channelsNode, DAE_FORMAT_HINT_D_VALUE);
        else if (formatHint->channels == FCDFormatHint::CHANNEL_XYZ)
            AddContent(channelsNode, DAE_FORMAT_HINT_XYZ_VALUE);
        else if (formatHint->channels == FCDFormatHint::CHANNEL_XYZW)
            AddContent(channelsNode, DAE_FORMAT_HINT_XYZW_VALUE);
        else
            AddContent(channelsNode, DAEERR_UNKNOWN_ELEMENT);

        xmlNode* rangeNode = AddChild(formatHintNode, DAE_RANGE_ELEMENT);
        if (formatHint->range == FCDFormatHint::RANGE_SNORM)
            AddContent(rangeNode, DAE_FORMAT_HINT_SNORM_VALUE);
        else if (formatHint->range == FCDFormatHint::RANGE_UNORM)
            AddContent(rangeNode, DAE_FORMAT_HINT_UNORM_VALUE);
        else if (formatHint->range == FCDFormatHint::RANGE_SINT)
            AddContent(rangeNode, DAE_FORMAT_HINT_SINT_VALUE);
        else if (formatHint->range == FCDFormatHint::RANGE_UINT)
            AddContent(rangeNode, DAE_FORMAT_HINT_UINT_VALUE);
        else if (formatHint->range == FCDFormatHint::RANGE_FLOAT)
            AddContent(rangeNode, DAE_FORMAT_HINT_FLOAT_VALUE);
        else if (formatHint->range == FCDFormatHint::RANGE_LOW)
            AddContent(rangeNode, DAE_FORMAT_HINT_LOW_VALUE);
        else
            AddContent(rangeNode, DAEERR_UNKNOWN_ELEMENT);

        xmlNode* precisionNode = AddChild(formatHintNode, DAE_PRECISION_ELEMENT);
        if (formatHint->precision == FCDFormatHint::PRECISION_LOW)
            AddContent(precisionNode, DAE_FORMAT_HINT_LOW_VALUE);
        else if (formatHint->precision == FCDFormatHint::PRECISION_MID)
            AddContent(precisionNode, DAE_FORMAT_HINT_MID_VALUE);
        else if (formatHint->precision == FCDFormatHint::PRECISION_HIGH)
            AddContent(precisionNode, DAE_FORMAT_HINT_HIGH_VALUE);
        else
            AddContent(precisionNode, DAEERR_UNKNOWN_ELEMENT);
        ...
        for (fm::vector<FCDFormatHint::optionValue>::iterator it = formatHint->options.begin(); it != formatHint->options.end(); ++it)
        {
            ...
            if (*it == FCDFormatHint::OPT_SRGB_GAMMA)
                AddContent(optionNode, DAE_FORMAT_HINT_SRGB_GAMMA_VALUE);
            else if (*it == FCDFormatHint::OPT_NORMALIZED3)
                AddContent(optionNode, DAE_FORMAT_HINT_NORMALIZED3_VALUE);
            else if (*it == FCDFormatHint::OPT_NORMALIZED4)
                AddContent(optionNode, DAE_FORMAT_HINT_NORMALIZED4_VALUE);
            else if (*it == FCDFormatHint::OPT_COMPRESSABLE)
                AddContent(optionNode, DAE_FORMAT_HINT_COMPRESSABLE_VALUE);
        }
    }
...
xmlNode* FArchiveXML::WriteEffectParameterVector(FCDObject* object, xmlNode* parentNode)
{
    ...
    else
    {
        ...
        if (*wantedSubId == 0)
            wantedSubId = effectParameterVector->GetSemantic().c_str();
        if (*wantedSubId == 0)
            wantedSubId = "flt4";
...
xmlNode* FArchiveXML::WriteEffectPassShader(FCDObject* object, xmlNode* parentNode)
{
    ...
    if (!effectPassShader->GetCompilerTarget().empty())
        AddChild(shaderNode, DAE_FXCMN_COMPILERTARGET_ELEMENT, effectPassShader->GetCompilerTarget());
    if (!effectPassShader->GetCompilerOptions().empty())
        AddChild(shaderNode, DAE_FXCMN_COMPILEROPTIONS_ELEMENT, effectPassShader->GetCompilerOptions());
    ...
    if (!effectPassShader->GetName().empty())
    {
        ...
        if (effectPassShader->GetCode() != NULL)
            AddAttribute(nameNode, DAE_SOURCE_ATTRIBUTE, effectPassShader->GetCode()->GetSubId());
    }
...
#define CHILD_NODE_ENUM(offset, elementName, nameSpace, enumName) { \
    ...
    case FUDaePassState::COLOR_MASK:
        builder.set(*(bool*)(data + 0));
        builder.append(' ');
        builder.append(*(bool*)(data + 1));
        builder.append(' ');
        builder.append(*(bool*)(data + 2));
        builder.append(' ');
    ...
    case FUDaePassState::LINE_STIPPLE:
        builder.set((uint32) * (uint16*)(data + 0));
        builder.append(' ');
...
xmlNode* FArchiveXML::WriteEffectProfileFX(FCDObject* object, xmlNode* parentNode)
{
    ...
    if (!effectProfileFX->GetPlatform().empty())
        AddAttribute(profileNode, DAE_PLATFORM_ATTRIBUTE, effectProfileFX->GetPlatform());
...
xmlNode* FArchiveXML::WriteEffectStandard(FCDObject* object, xmlNode* parentNode)
{
    ...
    case FCDEffectStandard::CONSTANT:
        materialName = DAE_FXSTD_CONSTANT_ELEMENT;
        break;
    case FCDEffectStandard::LAMBERT:
        materialName = DAE_FXSTD_LAMBERT_ELEMENT;
        break;
    case FCDEffectStandard::PHONG:
        materialName = DAE_FXSTD_PHONG_ELEMENT;
        break;
    case FCDEffectStandard::BLINN:
        materialName = DAE_FXSTD_BLINN_ELEMENT;
        break;
    case FCDEffectStandard::UNKNOWN:
    default:
        materialName = DAEERR_UNKNOWN_ELEMENT;
        break;
    }
...
xmlNode* FArchiveXML::WriteEffectTechnique(FCDObject* object, xmlNode* parentNode)
{
    ...
    {
        UNUSED(xmlNode* dummyPassNode = )
        AddChild(techniqueNode, DAE_PASS_ELEMENT);
    }
...
xmlNode* FArchiveXML::WriteImage(FCDObject* object, xmlNode* parentNode)
{
    ...
    if (image->GetWidth() > 0)
        AddAttribute(imageNode, DAE_WIDTH_ELEMENT, image->GetWidth());
    if (image->GetHeight() > 0)
        AddAttribute(imageNode, DAE_HEIGHT_ELEMENT, image->GetHeight());
    if (image->GetDepth() > 0)
        AddAttribute(imageNode, DAE_DEPTH_ELEMENT, image->GetDepth());
...
xmlNode* FArchiveXML::WriteTextureParameter(FCDEffectStandard* effectStandard, xmlNode* parentNode, uint32 bucketIndex)
{
    ...
    if (bucketIndex != FUDaeTextureChannel::UNKNOWN)
    {
        ...
        for (size_t t = 0; t < textureCount; ++t)
        {
            ...
            if (newTextureNode != NULL && textureNode == NULL)
