...
{
    if (!FArchiveXML::LoadEntityInstance(object, instanceNode))
        return false;
...
{
    if (!FArchiveXML::LoadGeometryInstance(object, instanceNode))
        return false;
...
{
    if (IsEquivalent(node->name, DAE_INSTANCE_CAMERA_ELEMENT))
        return FCDEntity::CAMERA;
    else if (IsEquivalent(node->name, DAE_INSTANCE_CONTROLLER_ELEMENT))
        return FCDEntity::CONTROLLER;
    else if (IsEquivalent(node->name, DAE_INSTANCE_EMITTER_ELEMENT))
        return FCDEntity::EMITTER;
    else if (IsEquivalent(node->name, DAE_INSTANCE_FORCE_FIELD_ELEMENT))
        return FCDEntity::FORCE_FIELD;
    else if (IsEquivalent(node->name, DAE_INSTANCE_GEOMETRY_ELEMENT))
        return FCDEntity::GEOMETRY;
    else if (IsEquivalent(node->name, DAE_INSTANCE_SPRITE_ELEMENT))
        return FCDEntity::GEOMETRY; // same class does 2 jobs
    else if (IsEquivalent(node->name, DAE_INSTANCE_LIGHT_ELEMENT))
        return FCDEntity::LIGHT;
    else if (IsEquivalent(node->name, DAE_INSTANCE_NODE_ELEMENT))
        return FCDEntity::SCENE_NODE;
    else
        return (uint32)~0;
}
