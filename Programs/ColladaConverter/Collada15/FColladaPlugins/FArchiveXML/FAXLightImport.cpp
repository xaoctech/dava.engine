...
{
    if (!FArchiveXML::LoadTargetedEntity(object, lightNode))
        return false;
    ...
    for (xmlNode* child = commonTechniqueNode->children; child != NULL; child = child->next)
    {
        if (child->type != XML_ELEMENT_NODE)
            continue;
        if (IsEquivalent(child->name, DAE_LIGHT_POINT_ELEMENT))
        {
            lightParameterNode = child;
            light->SetLightType(FCDLight::POINT);
            break;
        }
        else if (IsEquivalent(child->name, DAE_LIGHT_SPOT_ELEMENT))
        {
            lightParameterNode = child;
            light->SetLightType(FCDLight::SPOT);
            break;
        }
        else if (IsEquivalent(child->name, DAE_LIGHT_AMBIENT_ELEMENT))
        {
            lightParameterNode = child;
            light->SetLightType(FCDLight::AMBIENT);
            break;
        }
        else if (IsEquivalent(child->name, DAE_LIGHT_DIRECTIONAL_ELEMENT))
        {
            lightParameterNode = child;
            light->SetLightType(FCDLight::DIRECTIONAL);
            break;
        }
        else
        {
