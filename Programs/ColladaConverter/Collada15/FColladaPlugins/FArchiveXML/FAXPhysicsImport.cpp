...
{
    if (!FArchiveXML::LoadEntity(object, physicsMaterialNode))
        return false;
...
{
    if (!FArchiveXML::LoadEntity(object, physicsModelNode))
        return false;
...
{
    if (!FArchiveXML::LoadEntity(object, physicsRigidBodyNode))
        return false;
...
{
    if (!FArchiveXML::LoadEntity(object, physicsRigidConstraintNode))
        return false;
...
#undef PARSE_TRANSFORM
    ...
    if (spring)
    {
        ...
        if (linearSpring)
        {
            ...
            if (param)
                physicsRigidConstraint->SetSpringLinearDamping(FUStringConversion::ToFloat(ReadNodeContentDirect(param)));
            param = FindChildByType(linearSpring, DAE_STIFFNESS_ELEMENT);
            if (param)
                physicsRigidConstraint->SetSpringLinearStiffness(FUStringConversion::ToFloat(ReadNodeContentDirect(param)));
            param = FindChildByType(linearSpring, DAE_TARGET_VALUE_ELEMENT);
            if (!param)
                param = FindChildByType(linearSpring, DAE_REST_LENGTH_ELEMENT1_3); // COLLADA 1.3 backward compatibility
            if (param)
                physicsRigidConstraint->SetSpringLinearTargetValue(FUStringConversion::ToFloat(ReadNodeContentDirect(param)));
        }
        ...
        if (angularSpring)
        {
            ...
            if (param)
                physicsRigidConstraint->SetSpringAngularDamping(FUStringConversion::ToFloat(ReadNodeContentDirect(param)));
            param = FindChildByType(angularSpring, DAE_STIFFNESS_ELEMENT);
            if (param)
                physicsRigidConstraint->SetSpringAngularStiffness(FUStringConversion::ToFloat(ReadNodeContentDirect(param)));
            param = FindChildByType(angularSpring, DAE_TARGET_VALUE_ELEMENT);
            if (!param)
                param = FindChildByType(angularSpring, DAE_REST_LENGTH_ELEMENT1_3); // COLLADA 1.3 backward compatibility
            if (param)
                physicsRigidConstraint->SetSpringAngularTargetValue(FUStringConversion::ToFloat(ReadNodeContentDirect(param)));
        }
    }
...
{
    if (!FArchiveXML::LoadEntity(object, sceneNode))
        return false;
