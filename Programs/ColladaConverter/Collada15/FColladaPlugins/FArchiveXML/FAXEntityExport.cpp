...
xmlNode* FArchiveXML::WriteExtraType(FCDObject* object, xmlNode* parentNode)
{
    ...
    if (eType->GetName().empty() && eType->GetTechniqueCount() == 0)
        return NULL;

    // Add the <extra> element and its techniques
    xmlNode* extraNode = AddChild(parentNode, DAE_EXTRA_ELEMENT);
    if (!eType->GetName().empty())
        AddAttribute(extraNode, DAE_TYPE_ATTRIBUTE, eType->GetName());
...
xmlNode* FArchiveXML::WriteAsset(FCDObject* object, xmlNode* parentNode)
{
    ...
    if (!asset->GetKeywords().empty())
        AddChild(assetNode, DAE_KEYWORDS_ASSET_PARAMETER, asset->GetKeywords());
    AddChild(assetNode, DAE_MODIFIED_ASSET_PARAMETER, FUStringConversion::ToString(asset->GetModifiedDateTime()));
    if (!asset->GetRevision().empty())
        AddChild(assetNode, DAE_REVISION_ASSET_PARAMETER, asset->GetRevision());
    if (!asset->GetSubject().empty())
        AddChild(assetNode, DAE_SUBJECT_ASSET_PARAMETER, asset->GetSubject());
    if (!asset->GetTitle().empty())
        AddChild(assetNode, DAE_TITLE_ASSET_PARAMETER, asset->GetTitle());
    ...
    if (asset->GetHasUpAxisFlag())
    {
        ...
        if (IsEquivalent(asset->GetUpAxis(), FMVector3::YAxis))
            upAxisString = DAE_Y_UP;
        else if (IsEquivalent(asset->GetUpAxis(), FMVector3::XAxis))
            upAxisString = DAE_X_UP;
        else if (IsEquivalent(asset->GetUpAxis(), FMVector3::ZAxis))
            upAxisString = DAE_Z_UP;
...
xmlNode* FArchiveXML::WriteAssetContributor(FCDObject* object, xmlNode* parentNode)
{
    ...
    if (!assetContributor->IsEmpty())
    {
        ...
        if (!assetContributor->GetAuthor().empty())
            AddChild(contributorNode, DAE_AUTHOR_ASSET_PARAMETER, assetContributor->GetAuthor());
        if (!assetContributor->GetAuthoringTool().empty())
            AddChild(contributorNode, DAE_AUTHORINGTOOL_ASSET_PARAMETER, assetContributor->GetAuthoringTool());
        if (!assetContributor->GetComments().empty())
            AddChild(contributorNode, DAE_COMMENTS_ASSET_PARAMETER, assetContributor->GetComments());
        if (!assetContributor->GetCopyright().empty())
            AddChild(contributorNode, DAE_COPYRIGHT_ASSET_PARAMETER, assetContributor->GetCopyright());
