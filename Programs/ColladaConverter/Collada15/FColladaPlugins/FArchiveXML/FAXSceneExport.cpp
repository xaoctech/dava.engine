...
xmlNode* FArchiveXML::WriteSceneNode(FCDObject* object, xmlNode* parentNode)
{
    ...
    else
    {
        ...
        if (sceneNode->GetSubId().length() > 0)
            AddAttribute(node, DAE_SID_ATTRIBUTE, sceneNode->GetSubId());
    ...
    if (isVisualScene)
        FArchiveXML::WriteVisualScene(sceneNode, node);
    ...
    if (extraTechnique != NULL)
    {
        ...
        if (extraTechnique->GetChildNodeCount() == 0)
            SAFE_RELEASE(extraTechnique);
    }
...
xmlNode* FArchiveXML::WriteTransformLookAt(FCDObject* object, xmlNode* parentNode)
{
    ...
    FUStringConversion::ToString(builder, tLookAt->GetPosition());
    builder.append(' ');
    FUStringConversion::ToString(builder, tLookAt->GetTarget());
    builder.append(' ');
...
xmlNode* FArchiveXML::WriteTransformRotation(FCDObject* object, xmlNode* parent)
{
    ...
    FUSStringBuilder builder;
    FUStringConversion::ToString(builder, tRotation->GetAxis());
    builder += ' ';
    builder += tRotation->GetAngle();
...
xmlNode* FArchiveXML::WriteTransformSkew(FCDObject* object, xmlNode* parentNode)
{
    ...
    FUSStringBuilder builder;
    builder.set(tSkew->GetAngle());
    builder += ' ';
    FUStringConversion::ToString(builder, tSkew->GetRotateAxis());
    builder += ' ';
...
xmlNode* FArchiveXML::WriteToEntityXMLFCDEntity(FCDEntity* entity, xmlNode* parentNode, const char* nodeName, bool writeId)
{
    ...
    if (const_cast<const FCDEntity*>(entity)->GetAsset() != NULL)
        FArchiveXML::LetWriteObject(entity->GetAsset(), entityNode);
...
void FArchiveXML::WriteTargetedEntityExtra(FCDTargetedEntity* targetedEntity, xmlNode* entityNode)
{
    ...
    if (targetedEntity->GetTargetNode() != NULL)
    {
        ...
        if (technique->GetChildNodeCount() == 0)
    ...
    if (sceneNode->GetDocument()->GetVisualSceneInstance() == sceneNode)
    {
        ...
        if (!layers.empty())
        {
            ...
            for (FCDLayerList::const_iterator itL = layers.begin(); itL != layers.end(); ++itL)
            {
                ...
                if (!(*itL)->name.empty())
                    AddAttribute(layerNode, DAE_NAME_ATTRIBUTE, (*itL)->name);
        ...
        if (sceneNode->GetDocument()->HasStartTime() || sceneNode->GetDocument()->HasEndTime())
        {
            ...
            if (sceneNode->GetDocument()->HasStartTime())
                AddChild(techniqueNode, DAEMAYA_STARTTIME_PARAMETER, sceneNode->GetDocument()->GetStartTime());
            if (sceneNode->GetDocument()->HasEndTime())
                AddChild(techniqueNode, DAEMAYA_ENDTIME_PARAMETER, sceneNode->GetDocument()->GetEndTime());
        }
    }
}
