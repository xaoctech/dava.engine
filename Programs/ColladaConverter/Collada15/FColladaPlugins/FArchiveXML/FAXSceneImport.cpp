...
bool FArchiveXML::LoadEntity(FCDObject* object, xmlNode* entityNode)
{
    ...
    for (xmlNodeList::iterator it = extraNodes.begin(); it != extraNodes.end(); ++it)
    {
        ...
        if (maxTechnique != NULL)
            maxTechnique->FindParameters(parameterNodes, parameterNames);
        if (fcTechnique != NULL)
            fcTechnique->FindParameters(parameterNodes, parameterNames);
...
{
    if (!FArchiveXML::LoadEntity(object, entityNode))
        return false;
...
{
    if (!FArchiveXML::LoadEntity(object, node))
        return false;
    ...
    if (nodeType == DAE_JOINT_NODE_TYPE)
        sceneNode->SetJointFlag(true);
    else if (nodeType.length() == 0 || nodeType == DAE_NODE_NODE_TYPE)
    {
...
bool FArchiveXML::LoadTransformSkew(FCDObject* object, xmlNode* skewNode)
{
    ...
    if (IsEquivalent(tSkew->GetRotateAxis(), FMVector3::Origin) || IsEquivalent(tSkew->GetAroundAxis(), FMVector3::Origin))
        return false;
...
{
    if (IsEquivalent(node->name, DAE_ROTATE_ELEMENT))
        return FCDTransform::ROTATION;
    else if (IsEquivalent(node->name, DAE_TRANSLATE_ELEMENT))
        return FCDTransform::TRANSLATION;
    else if (IsEquivalent(node->name, DAE_SCALE_ELEMENT))
        return FCDTransform::SCALE;
    else if (IsEquivalent(node->name, DAE_SKEW_ELEMENT))
        return FCDTransform::SKEW;
    else if (IsEquivalent(node->name, DAE_MATRIX_ELEMENT))
        return FCDTransform::MATRIX;
    else if (IsEquivalent(node->name, DAE_LOOKAT_ELEMENT))
        return FCDTransform::LOOKAT;
    return (uint32)~0;
}
