...
xmlNode* FArchiveXML::WriteGeometryMesh(FCDObject* object, xmlNode* parentNode)
{
    ...
    else
    {
        ...
        if (verticesInputExtraNode != NULL)
            AddChild(verticesNode, verticesInputExtraNode);
...
xmlNode* FArchiveXML::WriteGeometry(FCDObject* object, xmlNode* parentNode)
{
    ...
    if (geometry->GetMesh() != NULL)
        FArchiveXML::LetWriteObject(geometry->GetMesh(), geometryNode);
    else if (geometry->GetSpline() != NULL)
        FArchiveXML::LetWriteObject(geometry->GetSpline(), geometryNode);
...
xmlNode* FArchiveXML::WriteGeometryPolygons(FCDObject* object, xmlNode* parentNode)
{
    ...
    case FCDGeometryPolygons::POLYGONS:
        ...
        if (!hasHoles)
            hasNPolys = (geometryPolygons->TestPolyType() != 3);

        if (hasHoles)
            polygonNodeType = DAE_POLYGONS_ELEMENT;
        else if (hasNPolys)
            polygonNodeType = DAE_POLYLIST_ELEMENT;
        else
            polygonNodeType = DAE_TRIANGLES_ELEMENT;
        break;

    case FCDGeometryPolygons::LINES:
        hasHoles = true;
        polygonNodeType = DAE_LINES_ELEMENT;
        break;
    case FCDGeometryPolygons::LINE_STRIPS:
        hasHoles = true;
        polygonNodeType = DAE_LINESTRIPS_ELEMENT;
        break;
    case FCDGeometryPolygons::TRIANGLE_FANS:
        hasHoles = true;
        polygonNodeType = DAE_TRIFANS_ELEMENT;
        break;
    case FCDGeometryPolygons::TRIANGLE_STRIPS:
        hasHoles = true;
        polygonNodeType = DAE_TRISTRIPS_ELEMENT;
        break;
    case FCDGeometryPolygons::POINTS:
        polygonNodeType = DAE_POINTS_ELEMENT;
        break;
    default:
        polygonNodeType = emptyCharString;
        FUFail(break);
        break;
    }
    ...
    FUSStringBuilder verticesNodeId(geometryPolygons->GetParent()->GetDaeId());
    verticesNodeId.append("-vertices");
    ...
    for (size_t i = 0; i < geometryPolygons->GetInputCount(); ++i)
    {
        ...
        {
            if (input->GetOffset() >= idxOwners.size())
                idxOwners.resize(input->GetOffset() + 1);
    ...
    for (size_t faceIndex = 0; faceIndex < faceCount; ++faceIndex)
    {
        ...
        for (size_t holeIndex = 0; holeIndex < holeCount + 1; ++holeIndex)
        {
            ...
            {
                if (!builder.empty())
                    builder.pop_back(); // take out the last space
...
xmlNode* FArchiveXML::WriteNURBSSpline(FCDNURBSSpline* nURBSSpline, xmlNode* parentNode, const fm::string& parentId, const fm::string& splineId)
{
    ...
    FUSStringBuilder controlPointSourceId(parentId);
    controlPointSourceId += "-cvs-" + splineId;
    AddSourcePosition(splineNode, controlPointSourceId.ToCharPtr(), nURBSSpline->GetCVs());
    FUSStringBuilder weightSourceId(parentId);
    weightSourceId += "-weights-" + splineId;
    AddSourceFloat(splineNode, weightSourceId.ToCharPtr(), nURBSSpline->GetWeights(), "WEIGHT");
    FUSStringBuilder knotSourceId(parentId);
    knotSourceId += "-knots-" + splineId;
