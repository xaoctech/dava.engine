...
bool FArchiveXML::LoadSkinController(FCDObject* object, xmlNode* skinNode)
{
    ...
    if (bindShapeTransformNode == NULL)
        skinController->SetBindShapeTransform(FMMatrix44::Identity);
    else
    {
    ...
    for (xmlNode* child = combinerNode->children; child != NULL; child = child->next)
    {
        if (child->type != XML_ELEMENT_NODE)
            continue;
        if (IsEquivalent(child->name, DAE_INPUT_ELEMENT))
            skinningInputNodes.push_back(child);
    ...
    for (xmlNodeList::iterator it = skinningInputNodes.begin(); it != skinningInputNodes.end(); ++it)
    {
        ...
        if (semantic == DAE_JOINT_SKIN_INPUT)
        {
            ...
            if (!idx.empty())
                jointIdx = FUStringConversion::ToInt32(idx);
            if (!jointSubIds.empty())
                continue;
        ...
        else if (semantic == DAE_WEIGHT_SKIN_INPUT)
        {
            fm::string idx = ReadNodeProperty(*it, DAE_OFFSET_ATTRIBUTE);
            if (!idx.empty())
                weightIdx = FUStringConversion::ToInt32(idx);
    ...
    UInt32List combinerVertexCounts;
    combinerVertexCounts.reserve(combinerValueCount);
    Int32List combinerVertexIndices;
    combinerVertexIndices.reserve(combinerValueCount * 5);
    ...
    for (size_t j = 0; j < combinerValueCount; ++j)
    {
        ...
        if (IsEquivalent(weightSum, 0.0f) || IsEquivalent(weightSum, 1.0f))
            continue;
...
bool FArchiveXML::LoadMorphController(FCDObject* object, xmlNode* morphNode)
{
    ...
    for (int32 i = 0; i < (int32)targetCount; ++i)
    {
        ...
        if (morphTarget->GetWeight().IsAnimated())
            morphTarget->GetWeight().GetAnimated()->SetArrayElement(-1);
    }
...
FCDSkinController* FArchiveXML::FindSkinController(FCDControllerInstance* controllerInstance, FCDEntity* entity)
{
    if (entity != NULL && entity->GetType() == FCDEntity::CONTROLLER)
    {
        ...
        if (controller->IsSkin())
        {
            ...
            return FArchiveXML::FindSkinController(controllerInstance, controller->GetBaseTarget());
    }
    return NULL;
}
