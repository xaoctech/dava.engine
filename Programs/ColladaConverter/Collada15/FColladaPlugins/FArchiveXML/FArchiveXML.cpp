...
bool FArchiveXML::ImportFile(const fchar* filePath, FCDocument* fcdocument)
{
    ...
    if (status)
        FUError::Error(FUError::DEBUG_LEVEL, FUError::DEBUG_LOAD_SUCCESSFUL);
    return status;
}
...
bool FArchiveXML::Import(FCDocument* theDocument, xmlNode* colladaNode)
{
    ...
    for (xmlNode* child = colladaNode->children; child != NULL; child = child->next)
    {
        ...
        n.node = child;
        n.order = UNKNOWN;
        if (IsEquivalent(child->name, DAE_LIBRARY_ANIMATION_ELEMENT))
            n.order = ANIMATION;
        else if (IsEquivalent(child->name, DAE_LIBRARY_ANIMATION_CLIP_ELEMENT))
            n.order = ANIMATION_CLIP;
        else if (IsEquivalent(child->name, DAE_LIBRARY_CAMERA_ELEMENT))
            n.order = CAMERA;
        else if (IsEquivalent(child->name, DAE_LIBRARY_CONTROLLER_ELEMENT))
            n.order = CONTROLLER;
        else if (IsEquivalent(child->name, DAE_LIBRARY_EFFECT_ELEMENT))
            n.order = EFFECT;
        else if (IsEquivalent(child->name, DAE_LIBRARY_GEOMETRY_ELEMENT))
            n.order = GEOMETRY;
        else if (IsEquivalent(child->name, DAE_LIBRARY_IMAGE_ELEMENT))
            n.order = IMAGE;
        else if (IsEquivalent(child->name, DAE_LIBRARY_LIGHT_ELEMENT))
            n.order = LIGHT;
        else if (IsEquivalent(child->name, DAE_LIBRARY_MATERIAL_ELEMENT))
            n.order = MATERIAL;
        else if (IsEquivalent(child->name, DAE_LIBRARY_VSCENE_ELEMENT))
            n.order = VISUAL_SCENE;
        else if (IsEquivalent(child->name, DAE_LIBRARY_FFIELDS_ELEMENT))
            n.order = FORCE_FIELD;
        else if (IsEquivalent(child->name, DAE_LIBRARY_NODE_ELEMENT))
            n.order = VISUAL_SCENE; // Process them as visual scenes.
        else if (IsEquivalent(child->name, DAE_LIBRARY_PMATERIAL_ELEMENT))
            n.order = PHYSICS_MATERIAL;
        ...
        {
            if ((uint32)n.order < (uint32)(*it).order)
                break;
        }
        ...
        if (IsEquivalent(extraType, DAEFC_LIBRARIES_TYPE))
        {
            ...
            for (xmlNodeList::iterator itT = techniqueNodes.begin(); itT != techniqueNodes.end(); ++itT)
            {
                for (xmlNode* child = (*itT)->children; child != NULL; child = child->next)
                {
                    ...
                    if (IsEquivalent(child->name, DAE_LIBRARY_EMITTER_ELEMENT))
                        n.order = EMITTER;
                    else
    ...
    for (size_t i = 0; i < libraryNodeCount; ++i)
    {
        ...
        {
        case ANIMATION:
            status &= (FArchiveXML::LoadAnimationLibrary(theDocument->GetAnimationLibrary(), n.node));
            break;
        case ANIMATION_CLIP:
            status &= (FArchiveXML::LoadAnimationClipLibrary(theDocument->GetAnimationClipLibrary(), n.node));
            break;
        case CAMERA:
            status &= (FArchiveXML::LoadCameraLibrary(theDocument->GetCameraLibrary(), n.node));
            break;
        case CONTROLLER:
            status &= (FArchiveXML::LoadControllerLibrary(theDocument->GetControllerLibrary(), n.node));
            break;
        case EFFECT:
            status &= (FArchiveXML::LoadEffectLibrary(theDocument->GetEffectLibrary(), n.node));
            break;
        case EMITTER:
            status &= (FArchiveXML::LoadEmitterLibrary(theDocument->GetEmitterLibrary(), n.node));
            break;
        case FORCE_FIELD:
            status &= (FArchiveXML::LoadForceFieldLibrary(theDocument->GetForceFieldLibrary(), n.node));
            break;
        case GEOMETRY:
            status &= (FArchiveXML::LoadGeometryLibrary(theDocument->GetGeometryLibrary(), n.node));
            break;
        case IMAGE:
            status &= (FArchiveXML::LoadImageLibrary(theDocument->GetImageLibrary(), n.node));
            break;
        case LIGHT:
            status &= (FArchiveXML::LoadLightLibrary(theDocument->GetLightLibrary(), n.node));
            break;
        case MATERIAL:
            status &= (FArchiveXML::LoadMaterialLibrary(theDocument->GetMaterialLibrary(), n.node));
            break;
        case PHYSICS_MODEL:
        {
        ...
        case PHYSICS_MATERIAL:
            status &= (FArchiveXML::LoadPhysicsMaterialLibrary(theDocument->GetPhysicsMaterialLibrary(), n.node));
            break;
        case PHYSICS_SCENE:
            status &= (FArchiveXML::LoadPhysicsSceneLibrary(theDocument->GetPhysicsSceneLibrary(), n.node));
            break;
        case VISUAL_SCENE:
            status &= (FArchiveXML::LoadVisualSceneNodeLibrary(theDocument->GetVisualSceneLibrary(), n.node));
            break;
        case UNKNOWN:
        default:
            break;
    ...
    for (size_t i = 0; i < theDocument->GetGeometryLibrary()->GetEntityCount(); ++i)
    {
        ...
        if (mesh)
            FArchiveXML::LinkGeometryMesh(mesh);
    }
...
#undef EXPORT_LIBRARY
        ...
        if (theDocument->GetPhysicsSceneInstanceCount() > 0)
        {
            ...
            if (sceneNode == NULL)
                sceneNode = AddChild(colladaNode, DAE_SCENE_ELEMENT);
...
extern "C"
{
PLUGIN_EXPORT uint32 GetPluginCount()
{
    return 1;
}
PLUGIN_EXPORT const FUObjectType* GetPluginType(uint32)
{
    return &FArchiveXML::GetClassType();
}
PLUGIN_EXPORT FUPlugin* CreatePlugin(uint32)
{
