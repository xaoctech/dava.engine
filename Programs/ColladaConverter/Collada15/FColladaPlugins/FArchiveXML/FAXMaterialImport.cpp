...
{
    if (!FArchiveXML::LoadEntity(object, materialNode))
        return false;
...
bool FArchiveXML::LoadEffectCode(FCDObject* object, xmlNode* codeNode)
{
    ...
    bool status = true;
    if (IsEquivalent(codeNode->name, DAE_FXCMN_INCLUDE_ELEMENT))
        effectCode->SetType(FCDEffectCode::INCLUDE);
    else if (IsEquivalent(codeNode->name, DAE_FXCMN_CODE_ELEMENT))
        effectCode->SetType(FCDEffectCode::CODE);
    else
    {
...
bool FArchiveXML::LoadEffectParameter(FCDObject* object, xmlNode* parameterNode)
{
    ...
    for (xmlNodeList::iterator itN = annotationNodes.begin(); itN != annotationNodes.end(); ++itN)
    {
        ...
        for (xmlNode* valueNode = annotateNode->children; valueNode != NULL; valueNode = valueNode->next)
        {
            if (valueNode->type != XML_ELEMENT_NODE)
                continue;
            if (IsEquivalent(valueNode->name, DAE_FXCMN_STRING_ELEMENT))
            {
                annotation->type = FCDEffectParameter::STRING;
                annotation->value = TO_FSTRING(ReadNodeContentFull(valueNode));
            }
            else if (IsEquivalent(valueNode->name, DAE_FXCMN_BOOL_ELEMENT))
            {
                annotation->type = FCDEffectParameter::BOOLEAN;
                annotation->value = TO_FSTRING(ReadNodeContentDirect(valueNode));
            }
            else if (IsEquivalent(valueNode->name, DAE_FXCMN_INT_ELEMENT))
            {
                annotation->type = FCDEffectParameter::INTEGER;
                annotation->value = TO_FSTRING(ReadNodeContentDirect(valueNode));
            }
            else if (IsEquivalent(valueNode->name, DAE_FXCMN_FLOAT_ELEMENT))
            {
                annotation->type = FCDEffectParameter::FLOAT;
                annotation->value = TO_FSTRING(ReadNodeContentDirect(valueNode));
            }
            else
            {
                FUError::Error(FUError::WARNING_LEVEL, FUError::WARNING_INVALID_ANNO_TYPE, valueNode->line);
            }
            break;
        }
    }
    ...
    if (IsEquivalent(parameterNode->name, DAE_FXCMN_NEWPARAM_ELEMENT))
        effectParameter->SetGenerator();
    else if (IsEquivalent(parameterNode->name, DAE_PARAMETER_ELEMENT))
        effectParameter->SetAnimator();
    else if (IsEquivalent(parameterNode->name, DAE_FXCMN_SETPARAM_ELEMENT))
        effectParameter->SetModifier();
...
{
    if (!FArchiveXML::LoadEffectParameter(object, parameterNode))
        return false;
...
bool FArchiveXML::LoadEffectParameterFloat(FCDObject* object, xmlNode* parameterNode)
{
    ...
    if (!effectParameterFloat->IsAnimator())
    {
        ...
        if (valueNode == NULL)
        {
            ...
            effectParameterFloat->SetFloatType(FCDEffectParameterFloat::FLOAT);
...
bool FArchiveXML::LoadEffectParameterFloat2(FCDObject* object, xmlNode* parameterNode)
{
    ...
    if (valueNode == NULL)
    {
        ...
        effectParameterFloat2->SetFloatType(FCDEffectParameterFloat2::FLOAT);
...
bool FArchiveXML::LoadEffectParameterFloat3(FCDObject* object, xmlNode* parameterNode)
{
    ...
    if (!effectParameterFloat3->IsAnimator())
    {
        ...
        if (valueNode == NULL)
        {
            ...
            effectParameterFloat3->SetFloatType(FCDEffectParameterFloat3::FLOAT);
...
bool FArchiveXML::LoadEffectParameterMatrix(FCDObject* object, xmlNode* parameterNode)
{
    ...
    if (valueNode == NULL)
    {
        ...
        effectParameterMatrix->SetFloatType(FCDEffectParameterMatrix::FLOAT);
...
bool FArchiveXML::LoadEffectParameterVector(FCDObject* object, xmlNode* parameterNode)
{
    ...
    if (!effectParameterVector->IsAnimator())
    {
        ...
        if (valueNode == NULL)
        {
            ...
            effectParameterVector->SetFloatType(FCDEffectParameterVector::FLOAT);
...
bool FArchiveXML::LoadEffectParameterSampler(FCDObject* object, xmlNode* parameterNode)
{
    ...
    for (xmlNode* child = parameterNode->children; child != NULL; child = child->next)
    {
        ...
        if (IsEquivalent(child->name, DAE_FXCMN_SAMPLER1D_ELEMENT))
        {
            effectParameterSampler->SetSamplerType(FCDEffectParameterSampler::SAMPLER1D);
            samplerNode = child;
            break;
        }
        else if (IsEquivalent(child->name, DAE_FXCMN_SAMPLER2D_ELEMENT))
        {
            effectParameterSampler->SetSamplerType(FCDEffectParameterSampler::SAMPLER2D);
            samplerNode = child;
            break;
        }
        else if (IsEquivalent(child->name, DAE_FXCMN_SAMPLER3D_ELEMENT))
        {
            effectParameterSampler->SetSamplerType(FCDEffectParameterSampler::SAMPLER3D);
            samplerNode = child;
            break;
        }
        else if (IsEquivalent(child->name, DAE_FXCMN_SAMPLERCUBE_ELEMENT))
        {
            effectParameterSampler->SetSamplerType(FCDEffectParameterSampler::SAMPLERCUBE);
            samplerNode = child;
            break;
        }
    }
    ...
    if (wrapNode)
        effectParameterSampler->SetWrapS(FUDaeTextureWrapMode::FromString(ReadNodeContentDirect(wrapNode)));
    wrapNode = FindChildByType(samplerNode, DAE_WRAP_T_ELEMENT);
    if (wrapNode)
        effectParameterSampler->SetWrapT(FUDaeTextureWrapMode::FromString(ReadNodeContentDirect(wrapNode)));
    wrapNode = FindChildByType(samplerNode, DAE_WRAP_P_ELEMENT);
    if (wrapNode)
        effectParameterSampler->SetWrapP(FUDaeTextureWrapMode::FromString(ReadNodeContentDirect(wrapNode)));

    xmlNode* filterNode = FindChildByType(samplerNode, DAE_MIN_FILTER_ELEMENT);
    if (filterNode)
        effectParameterSampler->SetMinFilter(FUDaeTextureFilterFunction::FromString(ReadNodeContentDirect(filterNode)));
    filterNode = FindChildByType(samplerNode, DAE_MAG_FILTER_ELEMENT);
    if (filterNode)
        effectParameterSampler->SetMagFilter(FUDaeTextureFilterFunction::FromString(ReadNodeContentDirect(filterNode)));
    filterNode = FindChildByType(samplerNode, DAE_MIP_FILTER_ELEMENT);
    if (filterNode)
        effectParameterSampler->SetMipFilter(FUDaeTextureFilterFunction::FromString(ReadNodeContentDirect(filterNode)));
...
bool FArchiveXML::LoadEffectParameterSurface(FCDObject* object, xmlNode* parameterNode)
{
    ...
    if (!typeAttr.empty())
        effectParameterSurface->SetSurfaceType(typeAttr);
        ...
        if (valueNode)
        {
            ...
            if (refNode)
            {
                ...
                if (name.empty())
                {
                    ...
                    effectParameterSurface->GetNames().push_back(name);
            }
    ...
    if (formatHintNode)
    {
        ...
        else
        {
            ...
            if (IsEquivalent(contents, DAE_FORMAT_HINT_RGB_VALUE))
                formatHint->channels = FCDFormatHint::CHANNEL_RGB;
            else if (IsEquivalent(contents, DAE_FORMAT_HINT_RGBA_VALUE))
                formatHint->channels = FCDFormatHint::CHANNEL_RGBA;
            else if (IsEquivalent(contents, DAE_FORMAT_HINT_L_VALUE))
                formatHint->channels = FCDFormatHint::CHANNEL_L;
            else if (IsEquivalent(contents, DAE_FORMAT_HINT_LA_VALUE))
                formatHint->channels = FCDFormatHint::CHANNEL_LA;
            else if (IsEquivalent(contents, DAE_FORMAT_HINT_D_VALUE))
                formatHint->channels = FCDFormatHint::CHANNEL_D;
            else if (IsEquivalent(contents, DAE_FORMAT_HINT_XYZ_VALUE))
                formatHint->channels = FCDFormatHint::CHANNEL_XYZ;
            else if (IsEquivalent(contents, DAE_FORMAT_HINT_XYZW_VALUE))
                formatHint->channels = FCDFormatHint::CHANNEL_XYZW;
            else
            {
        ...
        {
            fm::string contents = FUStringConversion::ToString(ReadNodeContentDirect(hintChild));
            if (IsEquivalent(contents, DAE_FORMAT_HINT_SNORM_VALUE))
                formatHint->range = FCDFormatHint::RANGE_SNORM;
            else if (IsEquivalent(contents, DAE_FORMAT_HINT_UNORM_VALUE))
                formatHint->range = FCDFormatHint::RANGE_UNORM;
            else if (IsEquivalent(contents, DAE_FORMAT_HINT_SINT_VALUE))
                formatHint->range = FCDFormatHint::RANGE_SINT;
            else if (IsEquivalent(contents, DAE_FORMAT_HINT_UINT_VALUE))
                formatHint->range = FCDFormatHint::RANGE_UINT;
            else if (IsEquivalent(contents, DAE_FORMAT_HINT_FLOAT_VALUE))
                formatHint->range = FCDFormatHint::RANGE_FLOAT;
            else if (IsEquivalent(contents, DAE_FORMAT_HINT_LOW_VALUE))
                formatHint->range = FCDFormatHint::RANGE_LOW;
            else
            {
        ...
        {
            fm::string contents = FUStringConversion::ToString(ReadNodeContentDirect(hintChild));
            if (IsEquivalent(contents, DAE_FORMAT_HINT_LOW_VALUE))
                formatHint->precision = FCDFormatHint::PRECISION_LOW;
            else if (IsEquivalent(contents, DAE_FORMAT_HINT_MID_VALUE))
                formatHint->precision = FCDFormatHint::PRECISION_MID;
            else if (IsEquivalent(contents, DAE_FORMAT_HINT_HIGH_VALUE))
                formatHint->precision = FCDFormatHint::PRECISION_HIGH;
            else
            {
        ...
        for (xmlNodeList::iterator it = optionNodes.begin(); it != optionNodes.end(); ++it)
        {
            ...
            if (IsEquivalent(contents, DAE_FORMAT_HINT_SRGB_GAMMA_VALUE))
                formatHint->options.push_back(FCDFormatHint::OPT_SRGB_GAMMA);
            else if (IsEquivalent(contents, DAE_FORMAT_HINT_NORMALIZED3_VALUE))
                formatHint->options.push_back(FCDFormatHint::OPT_NORMALIZED3);
            else if (IsEquivalent(contents, DAE_FORMAT_HINT_NORMALIZED4_VALUE))
                formatHint->options.push_back(FCDFormatHint::OPT_NORMALIZED4);
            else if (IsEquivalent(contents, DAE_FORMAT_HINT_COMPRESSABLE_VALUE))
                formatHint->options.push_back(FCDFormatHint::OPT_COMPRESSABLE);
            else
            {
...
bool FArchiveXML::LoadEffectPassShader(FCDObject* object, xmlNode* shaderNode)
{
    ...
    if (isFragment)
        effectPassShader->AffectsFragments();
    else if (isVertex)
        effectPassShader->AffectsVertices();
    else
    {
    ...
    if (effectPassShader->GetCode() == NULL)
        effectPassShader->SetCode(effectPassShader->GetParent()->GetParent()->GetParent()->FindCode(codeSource));
...
{
    if (!FArchiveXML::LoadEffectProfile(object, profileNode))
        return false;
...
{
    if (!FArchiveXML::LoadEffectProfile(object, baseNode))
        return false;
    ...
    {
        if (commonParameterNode->type != XML_ELEMENT_NODE)
            continue;
        if (IsEquivalent(commonParameterNode->name, DAE_FXSTD_CONSTANT_ELEMENT))
        {
            effectStandard->SetLightingType(FCDEffectStandard::CONSTANT);
            break;
        }
        else if (IsEquivalent(commonParameterNode->name, DAE_FXSTD_LAMBERT_ELEMENT))
        {
            effectStandard->SetLightingType(FCDEffectStandard::LAMBERT);
            break;
        }
        else if (IsEquivalent(commonParameterNode->name, DAE_FXSTD_PHONG_ELEMENT))
        {
            effectStandard->SetLightingType(FCDEffectStandard::PHONG);
            break;
        }
        else if (IsEquivalent(commonParameterNode->name, DAE_FXSTD_BLINN_ELEMENT))
        {
            effectStandard->SetLightingType(FCDEffectStandard::BLINN);
            break;
        }
    }
...
{
    if (!FArchiveXML::LoadEntity(object, effectNode))
        return false;
...
{
    if (!FArchiveXML::LoadEntity(object, imageNode))
        return false;
...
bool FArchiveXML::LoadTexture(FCDObject* object, xmlNode* textureNode)
{
    ...
    if (!data.samplerSid.empty())
        data.samplerSid = FCDObjectWithId::CleanSubId(data.samplerSid);
...
uint32 FArchiveXML::GetEffectParameterType(xmlNode* parameterNode)
{
    ...
    if (parameterNode->children == NULL)
    {
        ...
        if (IsEquivalent(parent->name, DAE_BINDMATERIAL_ELEMENT))
        {
            ...
            if (IsEquivalent(type, DAE_FXCMN_BOOL_ELEMENT))
                return FCDEffectParameter::BOOLEAN;
            else if (IsEquivalent(type, DAE_FXCMN_FLOAT_ELEMENT))
                return FCDEffectParameter::FLOAT;
            else if (IsEquivalent(type, DAE_FXCMN_FLOAT2_ELEMENT))
                return FCDEffectParameter::FLOAT2;
            else if (IsEquivalent(type, DAE_FXCMN_FLOAT3_ELEMENT))
                return FCDEffectParameter::FLOAT3;
            else if (IsEquivalent(type, DAE_FXCMN_FLOAT4_ELEMENT))
                return FCDEffectParameter::VECTOR;
            else if (IsEquivalent(type, DAE_FXCMN_FLOAT4X4_ELEMENT))
                return FCDEffectParameter::MATRIX;
            else if (IsEquivalent(type, DAE_FXCMN_HALF_ELEMENT))
                return FCDEffectParameter::FLOAT;
            else if (IsEquivalent(type, DAE_FXCMN_HALF2_ELEMENT))
                return FCDEffectParameter::FLOAT2;
            else if (IsEquivalent(type, DAE_FXCMN_HALF3_ELEMENT))
                return FCDEffectParameter::FLOAT3;
            else if (IsEquivalent(type, DAE_FXCMN_HALF4_ELEMENT))
                return FCDEffectParameter::VECTOR;
            else if (IsEquivalent(type, DAE_FXCMN_HALF4X4_ELEMENT))
                return FCDEffectParameter::MATRIX;
            else if (IsEquivalent(type, DAE_FXCMN_INT_ELEMENT))
                return FCDEffectParameter::INTEGER;
            else if (IsEquivalent(type, DAE_FXCMN_SAMPLER1D_ELEMENT))
                return FCDEffectParameter::SAMPLER;
            else if (IsEquivalent(type, DAE_FXCMN_SAMPLER2D_ELEMENT))
                return FCDEffectParameter::SAMPLER;
            else if (IsEquivalent(type, DAE_FXCMN_SAMPLER3D_ELEMENT))
                return FCDEffectParameter::SAMPLER;
            else if (IsEquivalent(type, DAE_FXCMN_SAMPLERCUBE_ELEMENT))
                return FCDEffectParameter::SAMPLER;
            else if (IsEquivalent(type, DAE_FXCMN_SURFACE_ELEMENT))
                return FCDEffectParameter::SURFACE;
            else if (IsEquivalent(type, DAE_FXCMN_STRING_ELEMENT))
                return FCDEffectParameter::STRING;
        }
    }
    else
    {
        for (xmlNode* child = parameterNode->children; child != NULL; child = child->next)
        {
            ...
            if (IsEquivalent(child->name, DAE_FXCMN_BOOL_ELEMENT))
                return FCDEffectParameter::BOOLEAN;
            else if (IsEquivalent(child->name, DAE_FXCMN_FLOAT_ELEMENT))
                return FCDEffectParameter::FLOAT;
            else if (IsEquivalent(child->name, DAE_FXCMN_FLOAT2_ELEMENT))
                return FCDEffectParameter::FLOAT2;
            else if (IsEquivalent(child->name, DAE_FXCMN_FLOAT3_ELEMENT))
                return FCDEffectParameter::FLOAT3;
            else if (IsEquivalent(child->name, DAE_FXCMN_FLOAT4_ELEMENT))
                return FCDEffectParameter::VECTOR;
            else if (IsEquivalent(child->name, DAE_FXCMN_FLOAT4X4_ELEMENT))
                return FCDEffectParameter::MATRIX;
            else if (IsEquivalent(child->name, DAE_FXCMN_HALF_ELEMENT))
                return FCDEffectParameter::FLOAT;
            else if (IsEquivalent(child->name, DAE_FXCMN_HALF2_ELEMENT))
                return FCDEffectParameter::FLOAT2;
            else if (IsEquivalent(child->name, DAE_FXCMN_HALF3_ELEMENT))
                return FCDEffectParameter::FLOAT3;
            else if (IsEquivalent(child->name, DAE_FXCMN_HALF4_ELEMENT))
                return FCDEffectParameter::VECTOR;
            else if (IsEquivalent(child->name, DAE_FXCMN_HALF4X4_ELEMENT))
                return FCDEffectParameter::MATRIX;
            else if (IsEquivalent(child->name, DAE_FXCMN_INT_ELEMENT))
                return FCDEffectParameter::INTEGER;
            else if (IsEquivalent(child->name, DAE_FXCMN_SAMPLER1D_ELEMENT))
                return FCDEffectParameter::SAMPLER;
            else if (IsEquivalent(child->name, DAE_FXCMN_SAMPLER2D_ELEMENT))
                return FCDEffectParameter::SAMPLER;
            else if (IsEquivalent(child->name, DAE_FXCMN_SAMPLER3D_ELEMENT))
                ...
                return FCDEffectParameter::SAMPLER;
            else if (IsEquivalent(child->name, DAE_FXCMN_SURFACE_ELEMENT))
                return FCDEffectParameter::SURFACE;
            else if (IsEquivalent(child->name, DAE_FXCMN_STRING_ELEMENT))
                return FCDEffectParameter::STRING;
        }
    }
