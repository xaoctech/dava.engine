...
{
    if (!FArchiveXML::LoadTargetedEntity(object, cameraNode))
        return false;
    ...
    if (opticsNode != NULL)
        FArchiveXML::LoadExtra(extra, opticsNode); // backward compatibility-only, the extra information has been moved into the <camera><extra> element.
    ...
    if (cameraOthNode != NULL)
        camera->SetProjectionType(FCDCamera::ORTHOGRAPHIC);
    if (cameraPersNode != NULL)
        camera->SetProjectionType(FCDCamera::PERSPECTIVE);
